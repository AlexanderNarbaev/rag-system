...
- fastapi-proxy: FastAPI Proxy [type: fastapi] [domain: proxy] — OpenAI-compatible API with hybrid retrieval, reranking,
...
- etl-pipeline: ETL Pipeline [type: python] [domain: etl] — Data extraction, chunking, embedding, indexing for
  Confluence, Jira, GitLab
- hitl-dashboard: HITL Dashboard [type: streamlit] [domain: hitl] — Streamlit expert dashboard for feedback and quality
  control
- mcp-server: MCP Server [type: python] [domain: integration] — Model Context Protocol server for OpenCode/Claude
  Desktop integration
- llm-backend: LLM Backend [type: vllm] [domain: inference] — Multi-provider LLM backend: vLLM, llama.cpp,
  OpenAI-compatible
- slm-router: SLM Router [type: python] [domain: proxy] — Lightweight SLM for intent classification, query

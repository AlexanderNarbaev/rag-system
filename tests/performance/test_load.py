...
def _send_chat_request (service_url: str, query: str = "What is RAG?") -> tuple [float, int, str]:
  ...
  try:
    resp = requests.post (f"{service_url}/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": query}], "stream": False,
    }, timeout = 60, )
...
class TestLatency:
  ...
  def test_latency_at_10_users (self, service_url: str, benchmark_report: list):
    ...
    result = {
        "test": "test_latency_at_10_users", "concurrent_users": num_workers, "successful_requests": len (latencies),
        "errors": errors, "mean_ms": round (mean_latency, 2), **{k: round (v, 2) for k, v in percentiles.items ()},
    }
  ...
  def test_warm_start_latency (self, service_url: str, benchmark_report: list):
    ...
    for _ in range (3):
      with contextlib.suppress (requests.exceptions.ConnectionError):
        requests.post (f"{service_url}/v1/chat/completions", json = {
            "model": "rag-proxy", "messages": [{"role": "user", "content": "Warm-up request"}], "stream": False,
        }, timeout = 30, )
      time.sleep (0.5)
    ...
    result = {
        "test": "test_warm_start_latency", "successful_requests": len (latencies),
...
class TestThroughput:
  ...
  def test_throughput_at_50_users (self, service_url: str, benchmark_report: list):
    ...
    result = {
        "test": "test_throughput_at_50_users", "concurrent_users": num_workers, "elapsed_seconds": round (elapsed, 2),
        "rps": round (rps, 2), "successful_requests": len (latencies), "errors": errors,
        "error_rate": round (error_rate, 4), "mean_ms": round (statistics.mean (latencies), 2) if latencies else 0,
...
class TestStress:
  ...
  def test_stress_at_100_users (self, service_url: str, benchmark_report: list):
    ...
    with ThreadPoolExecutor (max_workers = num_workers) as executor:
      futures = [executor.submit (_send_chat_request, service_url, f"Stress test query {i}") for i in
          range (num_workers)]
      for future in as_completed (futures):
        latency_ms, status, _ = future.result ()
        if status == 200:
    ...
    result = {
        "test": "test_stress_at_100_users", "concurrent_users": num_workers, "elapsed_seconds": round (elapsed, 2),
        "rps": round (rps, 2), "successful_requests": len (latencies), "errors": errors,
        "mean_ms": round (statistics.mean (latencies), 2) if latencies else 0,
        **{k: round (v, 2) for k, v in percentiles.items ()},
    }

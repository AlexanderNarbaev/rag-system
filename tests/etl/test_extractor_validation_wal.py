...
class TestConfluenceInputValidation:
  ...
  def test_empty_url_raises_value_error (self, tmp_path):
    config = {
        "url": "", "token": "test-token", "output_dir": str (tmp_path / "out"),
  ...
  def test_missing_url_raises_value_error (self, tmp_path):
    config = {
        "token": "test-token", "output_dir": str (tmp_path / "out"), "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "'url' is required"):
      ConfluenceExtractor (config)
  ...
  def test_invalid_url_format_raises_value_error (self, tmp_path):
    config = {
        "url": "ftp://confluence.test", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
  ...
  def test_url_without_scheme_raises_value_error (self, tmp_path):
    config = {
        "url": "confluence.test.local", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "must start with http:// or https://"):
      ConfluenceExtractor (config)
  ...
  def test_empty_token_raises_value_error (self, tmp_path):
    config = {
        "url": "https://confluence.test", "token": "", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
  ...
  def test_missing_token_raises_value_error (self, tmp_path):
    config = {
        "url": "https://confluence.test", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "'token' or 'api_key' is required"):
      ConfluenceExtractor (config)
  ...
  def test_valid_http_url_does_not_raise (self, tmp_path):
    config = {
        "url": "http://confluence.test", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
  ...
  def test_valid_https_url_does_not_raise (self, tmp_path):
    config = {
        "url": "https://confluence.test", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    extractor = ConfluenceExtractor (config)
...
class TestJiraInputValidation:
  ...
  def test_invalid_url_format_raises_value_error (self, tmp_path):
    config = {
        "url": "ftp://jira.test", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "must start with http:// or https://"):
      JiraExtractor (config)
  ...
  def test_url_without_scheme_raises_value_error (self, tmp_path):
    config = {
        "url": "jira.test.local", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "must start with http:// or https://"):
      JiraExtractor (config)
  ...
  def test_empty_token_raises_value_error (self, tmp_path):
    config = {
        "url": "https://jira.test", "token": "", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "'token' or 'api_key' is required"):
      JiraExtractor (config)
  ...
  def test_missing_token_raises_value_error (self, tmp_path):
    config = {
        "url": "https://jira.test", "output_dir": str (tmp_path / "out"), "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "'token' or 'api_key' is required"):
      JiraExtractor (config)
  ...
  def test_valid_http_url_does_not_raise (self, tmp_path):
    config = {
        "url": "http://jira.test", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
  ...
  def test_valid_https_url_does_not_raise (self, tmp_path):
    config = {
        "url": "https://jira.test", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    extractor = JiraExtractor (config)
...
class TestGitLabInputValidation:
  ...
  def test_invalid_url_format_raises_value_error (self, tmp_path):
    config = {
        "url": "ftp://gitlab.test", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "must start with http:// or https://"):
      GitLabExtractor (config)
  ...
  def test_url_without_scheme_raises_value_error (self, tmp_path):
    config = {
        "url": "gitlab.test.local", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "must start with http:// or https://"):
      GitLabExtractor (config)
  ...
  def test_empty_token_raises_value_error (self, tmp_path):
    config = {
        "url": "https://gitlab.test", "token": "", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "'token' or 'api_key' is required"):
      GitLabExtractor (config)
  ...
  def test_missing_token_raises_value_error (self, tmp_path):
    config = {
        "url": "https://gitlab.test", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    with pytest.raises (ValueError, match = "'token' or 'api_key' is required"):
      GitLabExtractor (config)
  ...
  def test_valid_http_url_does_not_raise (self, tmp_path):
    config = {
        "url": "http://gitlab.test", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
  ...
  def test_valid_https_url_does_not_raise (self, tmp_path):
    config = {
        "url": "https://gitlab.test", "token": "test-token", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    extractor = GitLabExtractor (config)
...
class TestConfluenceWalCorruption:
  ...
  def test_valid_wal_file_returns_parsed_data (self, tmp_path):
    ...
    wal_path.parent.mkdir (parents = True, exist_ok = True)
    wal_data = {
        "last_run": "2025-06-01T00:00:00", "pages_hash": {"page_1": "abc123", "page_2": "def456"},
    }
...
class TestJiraWalCorruption:
  ...
  def test_valid_wal_file_returns_parsed_data (self, tmp_path):
    ...
    wal_path.parent.mkdir (parents = True, exist_ok = True)
    wal_data = {
        "last_run": "2025-06-01T00:00:00", "last_issue_id": "PROJ-100", "processed_issues": ["PROJ-1", "PROJ-2"],
    }
    wal_path.write_text (json.dumps (wal_data))
...
class TestGitLabWalCorruption:
  ...
  def test_valid_wal_file_returns_parsed_data (self, tmp_path):
    ...
    wal_path.parent.mkdir (parents = True, exist_ok = True)
    wal_data = {
        "last_run": "2025-06-01T00:00:00", "projects": {

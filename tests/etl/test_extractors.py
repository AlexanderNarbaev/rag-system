...
CONFLUENCE_BASE_CONFIG = {
    "url": "https://confluence.test.local", "username": "bot", "token": "test-token", "space_keys": None,
    "output_dir": "/tmp/test_confluence", "wal_file": "/tmp/test_confluence/wal.json",
}
...
class TestConfluenceExtractorInit:
  def test_init_basic (self, tmp_path):
    config = {
        "url": "https://confluence.test", "username": "bot", "token": "tok", "output_dir": str (tmp_path / "output"),
  ...
  def test_init_with_all_options (self, tmp_path):
    config = {
        ...
        "output_dir": str (tmp_path / "out"), "wal_file": str (tmp_path / "w" / "w.json"), "incremental": False,
        "download_attachments": False, "max_versions": 5, "api_version": "1",
    }
    extractor = ConfluenceExtractor (config)
...
class TestConfluenceCalculatePageHash:
  ...
  def test_same_page_same_hash (self, tmp_path):
    ...
    page = {
        "body": {"storage": {"value": "hello"}}, "version": {"number": 1, "when": "2025-01-01T00:00:00Z"},
    }
  ...
  def test_different_body_different_hash (self, tmp_path):
    ex = self._make_extractor (tmp_path)
    page1 = {
        "body": {"storage": {"value": "hello"}}, "version": {"number": 1, "when": "2025-01-01"},
    }
    page2 = {
  ...
  def test_hash_changes_with_version (self, tmp_path):
    ex = self._make_extractor (tmp_path)
    page1 = {
        "body": {"storage": {"value": "same"}}, "version": {"number": 1, "when": "2025-01-01"},
    }
    page2 = {
        "body": {"storage": {"value": "same"}}, "version": {"number": 2, "when": "2025-01-02"},
    }
    assert ex._calculate_page_hash (page1) != ex._calculate_page_hash (page2)
...
class TestConfluenceGetPageVersions:
  ...
  def test_get_page_versions (self, mock_request, tmp_path):
    ...
    mock_request.return_value = {
        "results": [
            {"number": 1, "when": "2025-01-01T00:00:00Z"}, {"number": 2, "when": "2025-01-02T00:00:00Z"},
        ]
    }
    ...
    mock_get_pages.return_value = [
        {
            ...
            "version": {"number": 1, "when": "2025-01-01"}, "space": {"key": "DEV"},
        },
    ]
...
JIRA_BASE_CONFIG = {
    "url": "https://jira.test.local", "username": "bot", "token": "test-token", "output_dir": "/tmp/test_jira",
...
class TestJiraExtractorInit:
  def test_init_basic (self, tmp_path):
    config = {
        "url": "https://jira.test", "username": "bot", "token": "tok", "output_dir": str (tmp_path / "output"),
        "wal_file": str (tmp_path / "wal" / "wal.json"),
    }
  ...
  def test_init_with_custom_jql (self, tmp_path):
    config = {
        ...
        "output_dir": str (tmp_path / "out"), "wal_file": str (tmp_path / "wal" / "w.json"),
    }
    extractor = JiraExtractor (config)
...
class TestJiraProcessIssue:
  ...
  def test_process_issue_minimal (self, mock_sprints, tmp_path):
    ...
    issue = {
        "key": "PROJ-123", "fields": {
            "summary": "Test issue", "description": "Description text", "status": {"name": "Open"},
            "priority": {"name": "High"}, "issuetype": {"name": "Bug"}, "created": "2025-01-01T00:00:00Z",
            "updated": "2025-01-02T00:00:00Z", "labels": ["backend"], "comment": {"comments": []},
        },
    }
  ...
  def test_process_issue_with_links (self, mock_sprints, tmp_path):
    ...
    issue = {
        "key": "PROJ-456", "fields": {
            "summary": "Test", "issuelinks": [
                {
                    "outwardIssue": {"key": "PROJ-100"}, "type": {"outward": "blocks"},
                }, {
                    "inwardIssue": {"key": "PROJ-200"}, "type": {"inward": "is blocked by"},
                },
...
class TestJiraRun:
  ...
  def test_run_respects_max_issues (self, mock_paginated, tmp_path):
    ...
    issues = [
        {"key": "PROJ-1", "fields": {}}, {"key": "PROJ-2", "fields": {}},
    ]
...
GITLAB_BASE_CONFIG = {
    "url": "https://gitlab.test.local", "token": "test-token", "output_dir": "/tmp/test_gitlab",
...
class TestGitLabExtractorInit:
  ...
  def test_init_disabled_features (self, tmp_path):
    config = {
        "url": "https://gitlab.test", "token": "tok", "output_dir": str (tmp_path / "out"),
        "wal_file": str (tmp_path / "wal" / "w.json"), "fetch_commits": False, "fetch_files": False,

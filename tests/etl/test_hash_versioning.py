...
from etl.chunker.hash_versioning import (
  ChunkVersionStore, compute_chunk_hash, get_incremental_chunks,
)
...
class TestChunkVersionStore:
  ...
  def _make_store (self, tmp_path):
    return ChunkVersionStore (hot_dir = tmp_path / "hot", cold_dir = tmp_path / "cold",
  ...
  def test_init_creates_dirs_and_wal (self, tmp_path):
    ...
    assert (
          tmp_path / "cold").is_dir ()  # WAL file is not created at init, only on _save_wal()  # _load_wal returns
    # empty dict without writing
  ...
  def test_init_loads_existing_wal (self, tmp_path):
    ...
    store = ChunkVersionStore (hot_dir = tmp_path / "hot", cold_dir = tmp_path / "cold", wal_path = wal_path, )
  ...
  def test_update_new_document_returns_all_chunks (self, tmp_path):
    store = self._make_store (tmp_path)
    chunks = [
        self._chunk ("aaa", "chunk 1"), self._chunk ("bbb", "chunk 2"),
    ]
  ...
  def test_update_modified_document_returns_only_changed (self, tmp_path):
    store = self._make_store (tmp_path)
    v1 = [
        self._chunk ("aaa", "old text"), self._chunk ("bbb", "unchanged text"),
    ]
    store.update_document_chunks ("doc_1", v1)
    v2 = [
        self._chunk ("ccc", "new text"), self._chunk ("bbb", "unchanged text"),
    ]
  ...
  def test_save_and_load_hot_chunks_round_trip (self, tmp_path):
    store = self._make_store (tmp_path)
    chunks = [
        self._chunk ("aaa", "text a", title = "A"), self._chunk ("bbb", "text b", title = "B"),
    ]
...
class TestGetIncrementalChunks:
  ...
  def test_returns_only_changed_chunks (self, tmp_path):
    store = ChunkVersionStore (hot_dir = tmp_path / "hot", cold_dir = tmp_path / "cold",
        wal_path = tmp_path / "wal" / "wal.json", )

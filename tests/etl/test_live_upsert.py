...
class TestLiveUpsert:
  def test_live_upsert_single_chunk (self, indexer, mock_qdrant_client):
    chunk = {
        "hash": _make_chunk_hash ("test content"), "text": "test content for live upsert", "title": "Test Chunk",
        "source_type": "confluence", "source_id": "conf_123", "version": "1.0", "doc_title": "Test Doc",
        "keywords": ["test"], "entities": [], "summary": "test summary",
    }
  ...
  def test_live_upsert_uses_hash_as_point_id (self, indexer, mock_qdrant_client):
    ...
    chunk = {
        "hash": chunk_hash, "text": "idempotent content", "title": "Idempotent", "source_type": "gitlab_commit",
        "source_id": "gl_1",
    }
  ...
  def test_live_upsert_preserves_payload_fields (self, indexer, mock_qdrant_client):
    chunk = {
        "hash": _make_chunk_hash ("payload test"), "text": "payload test content", "title": "Payload",
        "source_type": "jira", "source_id": "jira_1", "version": "2.0", "doc_title": "Jira Doc",
        "keywords": ["k1", "k2"], "entities": ["E1"], "summary": "sum", "position": 3, "semantic_key": "sk1",
  ...
  def test_live_upsert_qdrant_error_returns_false (self, indexer, mock_qdrant_client):
    ...
    chunk = {
        "hash": _make_chunk_hash ("error test"), "text": "error test content", "title": "Error",
        "source_type": "confluence", "source_id": "conf_err",
    }
    result = indexer.live_upsert (chunk)
    assert result is False

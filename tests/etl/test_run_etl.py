...
class TestCollectAllDocuments:
  ...
  def test_confluence_doc (self, tmp_path):
    ...
    page_data = {
        "id": "page123", "title": "Test Page", "body_view_html": "<p>Hello</p>", "version": "3", "space": "TEST",
  ...
  def test_jira_doc (self, tmp_path):
    ...
    issue_data = {
        "key": "PROJ-123", "summary": "Bug report", "description": "Something broke", "status": "Open",
        "priority": "High", "assignee": "dev1", "created": "2025-01-01", "updated": "2025-06-01",
  ...
  def test_gitlab_commits (self, tmp_path):
    ...
    commits_data = [
        {
            "id": "abc123def456", "title": "Fix bug", "message": "Fixed the thing", "author_name": "dev1",
            "created_at": "2025-01-01", "diff": [{"new_path": "src/main.py", "diff": "+print('fixed')"}],
        }
    ]
  ...
  def test_gitlab_merge_requests (self, tmp_path):
    ...
    mrs_data = [
        {
            "iid": 42, "title": "Add feature", "description": "New feature implementation", "state": "merged",
            "author": {"username": "dev2"}, "discussions": [{"notes": [{"author": "reviewer1", "body": "LGTM"}]}],
        }
    ]
...
class TestRunChunking:
  def test_basic_chunking (self, tmp_path):
    ...
    docs = [
        {
            "id": "doc1", "source_type": "wiki", "title": "T", "content": "content", "content_type": "markdown",
...
class TestMainFunction:
  ...
  def test_main_dry_run (self, MockWal, mock_config, tmp_path):
    ...
    mock_config.return_value = {
        ...
        "gitlab": {"base_url": "http://gitlab", "output_dir": "/tmp/out"}, "chunking": {"max_chunk_size": 1000},
    }

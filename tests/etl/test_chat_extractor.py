...
def chat_config (tmp_path):
  return ExtractorConfig (source_name = "test-chats", source_type = "chat", base_url = str (tmp_path), )


@pytest.fixture
def sample_deepseek_conversation ():
  return {
      "id": "conv-001", "title": "Python async patterns", "messages": [
...
def sample_chatgpt_conversation ():
  return {
      "conversation_id": "chat-001", "title": "TypeScript tips", "messages": [
...
def sample_claude_conversation ():
  return {
      "uuid": "claude-001", "name": "System design", "chat_messages": [
...
class TestChatExtractorValidateConnection:
  ...
  def test_with_exclude_patterns (self, tmp_path):
    config = ExtractorConfig (source_name = "test", source_type = "chat", base_url = str (tmp_path),
...
class TestChatExtractorParsing:
  ...
  def test_parse_claude (self, chat_config, sample_claude_conversation):
    ext = ChatExtractor (chat_config)
    conv_data = {
        "uuid": "claude-001", "name": "System design", "messages": sample_claude_conversation ["chat_messages"],
    }
...
class TestChatExtractorShouldProcess:
  ...
  def test_same_content (self, chat_config):
    ext = ChatExtractor (chat_config)
    doc = ExtractedDocument (source_id = "d", source_type = "chat", title = "T", content = "same",
                             content_type = "text")

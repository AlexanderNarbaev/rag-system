...
def sample_confluence_event ():
  return {
      "source": "confluence", "event_type": "page_created", "doc_id": "123456",
      "timestamp": datetime.now (UTC).isoformat (), "payload": json.dumps ({
          "event": "page_created", "page": {
              "id": "123456", "title": "Test Page", "space": {"key": "DEV"},
...
def sample_gitlab_event ():
  return {
      "source": "gitlab", "event_type": "push", "doc_id": "42", "timestamp": datetime.now (UTC).isoformat (),
      "payload": json.dumps ({
          "object_kind": "push", "commits": [{"id": "abc123", "message": "Test commit"}],
      }),
  }
...
class TestConsumerInit:
  def test_consumer_creates_group (self, mock_redis_client):
    ...
    consumer = StreamConsumer (redis_client = mock_redis_client, stream_key = "etl:events",
...
class TestStreamProcessing:
  ...
  def test_consume_multiple_messages_no_ack_for_stubs (self, mock_redis_client):
    ...
    event1 = {
        "source": "confluence", "event_type": "page_created", "doc_id": "1",
        "timestamp": datetime.now (UTC).isoformat (), "payload": "{}",
    }
    event2 = {
        "source": "gitlab", "event_type": "push", "doc_id": "2", "timestamp": datetime.now (UTC).isoformat (),
        "payload": "{}",
    }
    mock_redis_client.xreadgroup.return_value = [
        [
            "etl:events", [
            (b"1719000000000-0", event1), (b"1719000000000-1", event2),
        ],
        ]
    ]

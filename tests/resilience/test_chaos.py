...
class TestQdrantUnavailable:
  ...
  def test_qdrant_unavailable_chat (self, client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", side_effect = Exception ("Qdrant unavailable")), patch (
        "proxy.app.main.non_stream_completion", return_value = "Fallback answer without context"), patch (
        "proxy.app.main.cache_manager", None), patch ("proxy.app.main.log_interaction"), ):
      resp = client.post ("/v1/chat/completions", json = {
          "model": "rag-proxy", "messages": [{"role": "user", "content": "Test query"}], "stream": False,
      }, )
...
class TestNeo4jUnavailable:
  ...
  def test_neo4j_unavailable_chat (self, client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search") as mock_search, patch ("proxy.app.main.rerank_chunks",
                                                                    return_value = [0]), patch (
        "proxy.app.main.deduplicate_chunks", return_value = []), patch ("proxy.app.main.build_context",
                                                                        return_value = ""), patch (
        "proxy.app.main.non_stream_completion", return_value = "Graph expansion skipped"), patch (
        "proxy.app.main.cache_manager", None), patch ("proxy.app.main.log_interaction"), ):
      # Mock search results (without graph expansion being called)
      hit = MagicMock ()
      hit.payload = {
          "text": "RAG combines retrieval and generation.", "source_type": "confluence", "title": "RAG Overview",
          "version": "1.0",
      }
      hit.score = 0.9
...
class TestRedisUnavailable:
  ...
  def test_redis_unavailable_chat (self, client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = []), patch ("proxy.app.main.rerank_chunks",
                                                                        return_value = []), patch (
        "proxy.app.main.deduplicate_chunks", return_value = []), patch ("proxy.app.main.build_context",
                                                                        return_value = ""), patch (
        "proxy.app.main.non_stream_completion", return_value = "In-memory cache fallback"), patch (
        "proxy.app.main.cache_manager", None), patch ("proxy.app.main.log_interaction"), ):
      resp = client.post ("/v1/chat/completions", json = {
  ...
  def test_llm_timeout_non_streaming (self, client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = []), patch ("proxy.app.main.rerank_chunks",
        ...
        "proxy.app.main.deduplicate_chunks", return_value = []), patch ("proxy.app.main.build_context",
                                                                        return_value = ""), patch (
        "proxy.app.main.non_stream_completion", side_effect = Exception ("LLM request timeout")), patch (
        "proxy.app.main.cache_manager", None), patch ("proxy.app.main.log_interaction"), ):
      try:
        resp = client.post ("/v1/chat/completions", json = {
            "model": "rag-proxy", "messages": [{"role": "user", "content": "Timeout test"}], "stream": False,
        }, )
  ...
  def test_llm_timeout_streaming (self, client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = []), patch ("proxy.app.main.rerank_chunks",
        ...
        "proxy.app.main.deduplicate_chunks", return_value = []), patch ("proxy.app.main.build_context",
                                                                        return_value = ""), patch (
        "proxy.app.main.stream_completion", side_effect = Exception ("LLM stream timeout")), patch (
        "proxy.app.main.cache_manager", None), patch ("proxy.app.main.log_interaction"), ):
      resp = client.post ("/v1/chat/completions", json = {
          "model": "rag-proxy", "messages": [{"role": "user", "content": "Stream timeout test"}], "stream": True,
      }, )
      assert resp.status_code == 200
      content = resp.text
...
class TestRapidRestart:
  ...
  def test_chat_after_simulated_failure_recovery (self, client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", side_effect = [Exception ("Qdrant down")]), patch (
        "proxy.app.main.non_stream_completion", return_value = "Degraded response"), patch (
        "proxy.app.main.cache_manager", None), patch ("proxy.app.main.log_interaction"), ):
    ...
    hit = MagicMock ()
    hit.payload = {
        "text": "RAG is a technique for augmenting LLMs.", "source_type": "confluence", "title": "RAG",
        "version": "1.0",
    }
    hit.score = 0.95
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = [hit]), patch ("proxy.app.main.rerank_chunks",
        ...
        "proxy.app.main.deduplicate_chunks", return_value = []), patch ("proxy.app.main.build_context",
                                                                        return_value = ""), patch (
        "proxy.app.main.non_stream_completion", return_value = "Recovered response"), patch (
        "proxy.app.main.cache_manager", None), patch ("proxy.app.main.log_interaction"), ):
      resp2 = client.post ("/v1/chat/completions", json = {
          "model": "rag-proxy", "messages": [{"role": "user", "content": "After recovery"}], "stream": False,
      }, )
...
class TestCombinedFailures:
  ...
  def test_multiple_services_down (self, client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", side_effect = Exception ("Qdrant down")), patch (
        "proxy.app.main.non_stream_completion", return_value = "All services down, LLM only"), patch (
        "proxy.app.main.cache_manager", None), patch ("proxy.app.main.log_interaction"), ):
      resp = client.post ("/v1/chat/completions", json = {
          "model": "rag-proxy", "messages": [{"role": "user", "content": "Complete outage test"}], "stream": False,
      }, )

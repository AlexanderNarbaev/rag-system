...
class TestFullExtractChunkIndexPipeline:
  ...
  def make_sample_chunk (self, idx, source_type, source_id, version, text):
    return {
        "hash": f"hash_{source_type}_{idx}", "text": text, "title": f"Section {idx}", "source_type": source_type,
        "source_id": source_id, "version": version, "doc_title": f"Document {source_id}", "keywords": ["test"],
        "entities": [], "summary": "", "position": idx, "semantic_key": "",
    }
  ...
  def mock_chunker (self):
    ...
    with (
      patch ("etl.chunker.semantic_chunker.SemanticChunker"), patch (
        "etl.chunker.semantic_chunker.MetadataEnricher"), ):
      class FakeChunk:
        def __init__ (self, idx, source_metadata):
          self.text = f"Fake chunk text {idx}"
          self.hash = f"hash_{idx}"
          self.__dict__ = {
              "hash": f"hash_{idx}", "text": f"Fake chunk text {idx}", "title": "Section",
              "source_type": source_metadata.get ("source_type", ""),
              "source_id": source_metadata.get ("source_id", ""), "version": source_metadata.get ("version", ""),
              "doc_title": source_metadata.get ("doc_title", ""), "keywords": [], "entities": [], "summary": "",
              "position": idx, "semantic_key": f"key_{idx}",
          }
  ...
  def test_full_extract_chunk_index_flow (self, mock_confluence_extractor, mock_chunker, mock_qdrant_indexer):
    ...
    source_meta = {
        "source_type": "confluence", "source_id": "confluence_123", "version": "1.0", "doc_title": "Test Doc",
    }
  ...
  def test_extract_then_chunk_with_metadata_preservation (self, mock_chunker):
    ...
    source_meta = {
        "source_type": "jira", "source_id": "jira_PROJ-456", "version": "2.1",
  ...
  def test_incremental_sync_adds_new_chunks (self, tmp_path):
    ...
    with patch ("etl.indexer.qdrant_hybrid.QdrantHybridIndexer") as mock_qdrant_class:
      ...
      lake = LiveVectorLake (qdrant_indexer = mock_qdrant, version_store = version_store,
          cold_storage_dir = tmp_path / "cold_lake", use_delta = False, )

      doc_chunks = [{"hash": f"hash_{i}", "text": f"Chunk text {i}", "source_id": "doc_1", "version": "1.0"} for i in
          range (3)]

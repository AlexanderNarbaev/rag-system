...
def _make_scored_points (texts_with_meta):
  ...
  for i, item in enumerate (texts_with_meta):
    ...
    point.payload = {
        "text": item ["text"], "source_type": item.get ("source_type", "confluence"),
        "source_id": item.get ("source_id", f"src_{i}"), "version": item.get ("version", "1.0"),
        "title": item.get ("title", f"Title {i}"), "doc_title": item.get ("doc_title", f"Doc {i}"),
    }
    points.append (point)
  return points
...
class TestChatCompletionRAGPipeline:
  ...
  def test_chat_triggers_retrieval_reranking_and_llm (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = search_results) as mock_search, patch (
        "proxy.app.main.rerank_chunks", return_value = [0, 1]) as mock_rerank, patch (
        "proxy.app.main.non_stream_completion", side_effect = mock_llm) as mock_llm_fn, ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "What is RAG?"}],
      }
  ...
  def test_response_includes_rag_sources (self, app_client):
    ...
    search_results = _make_scored_points ([
        {
            "text": "RAG is retrieval-augmented generation.", "source_type": "confluence", "source_id": "conf_123",
            "title": "RAG Overview", "version": "2.0",
        }, {
            "text": "Hybrid search combines dense and sparse vectors.", "source_type": "gitlab", "source_id": "gl_456",
            "title": "Search Module", "version": "1.0",
        },
    ])
    ...
    with (
      ...
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Explain RAG and hybrid search"}],
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
      data = response.json ()
  ...
  def test_response_includes_feedback_id (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = search_results), patch ("proxy.app.main.rerank_chunks",
        ...
        "proxy.app.main.non_stream_completion", side_effect = mock_llm), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Test query"}],
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
      data = response.json ()
  ...
  def test_empty_search_results_still_returns_response (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = []), patch ("proxy.app.main.non_stream_completion",
                                                                        side_effect = mock_llm), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Unknown topic query"}],
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
  ...
  def test_search_failure_graceful_degradation (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", side_effect = Exception ("Qdrant unavailable")), patch (
        "proxy.app.main.non_stream_completion", side_effect = mock_llm), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Test degraded mode"}],
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
  ...
  def test_multi_turn_conversation_preserves_history (self, app_client):
    ...
    with (
      ...
      assert "Контекст" in captured_messages [0] ["content"]
  ...
  def test_version_filter_passed_to_search (self, app_client):
    ...
    with (
      ...
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Show docs for v2.0"}], "rag_version": "2.0",
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
      assert response.status_code == 200

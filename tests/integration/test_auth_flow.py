...
def _create_test_token (
    user_id = "test-user-001", username = "testuser", roles = None, groups = None,
    secret = "test-secret-key-for-integration", ):
  """Create a valid JWT token for testing."""
  from proxy.app.auth.jwt import create_token

  return create_token (user_id = user_id, username = username, roles = roles or ["user"],
      groups = groups or ["everyone"], access_level = "internal", secret = secret, )


def _create_admin_token (secret = "test-secret-key-for-integration"):
  """Create a JWT token with admin role."""
  return _create_test_token (user_id = "admin-001", username = "admin", roles = ["admin", "user"], groups = ["admins"],
      secret = secret, )
...
def auth_disabled_client ():
  ...
  with (
    patch ("proxy.app.main.cache_manager", None), patch ("proxy.app.main.USE_LANGGRAPH", False), patch (
...
def auth_enabled_client ():
  ...
  with (
    patch ("proxy.app.main.cache_manager", None), patch ("proxy.app.main.USE_LANGGRAPH", False), patch (
      "proxy.app.main.LOG_REQUESTS", False), patch ("proxy.app.main.LLM_MODEL_NAME", "test-model"), patch (
      "proxy.app.shared.config.AUTH_ENABLED", True), patch ("proxy.app.shared.config.JWT_SECRET",
                                                            "test-secret-key-for-integration"), patch (
      "proxy.app.auth.jwt.AUTH_ENABLED", True), patch ("proxy.app.auth.jwt.JWT_SECRET",
...
class TestProtectedEndpointsWithAuth:
  ...
  def test_chat_endpoint_accepts_valid_token (self, auth_enabled_client):
    ...
    search_results [0].payload = {
        "text": "Context", "source_type": "test", "source_id": "1", "version": "1.0", "title": "T", "doc_title": "D",
    }
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = search_results), patch ("proxy.app.main.rerank_chunks",
                                                                                    return_value = [0]), patch (
        "proxy.app.main.non_stream_completion", side_effect = mock_llm), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Hello"}],
      }
      response = auth_enabled_client.post ("/v1/chat/completions", json = payload,
  ...
  def test_feedback_endpoint_rejects_no_token (self, auth_enabled_client):
    ...
    response = auth_enabled_client.post ("/v1/feedback", json = {"feedback_id": "fb_test", "type": "positive"}, )
...
class TestLoginFlow:
  ...
  def test_login_returns_token_pair (self, auth_disabled_client, mock_user_db):
    ...
    mock_user = {
        "id": "user-001", "username": "testuser", "password_hash": "$2b$12$hash", "roles": ["user"],
        "groups": ["everyone"], "access_level": "user", "namespace": "",
    }
    mock_user_db.verify_password.return_value = mock_user

    with (
      patch ("proxy.app.api.auth_endpoints.get_user_db", return_value = mock_user_db), patch (
        "proxy.app.auth.user_db.get_user_db", return_value = mock_user_db), patch ("proxy.app.auth.jwt.JWT_SECRET",
                                                                                   "test-secret-key-for-integration"), ):
      response = auth_disabled_client.post ("/v1/auth/login",
  ...
  def test_refresh_token_returns_new_pair (self, auth_enabled_client, mock_user_db):
    ...
    mock_user = {
        "id": "user-001", "username": "testuser", "roles": ["user"], "groups": ["everyone"], "access_level": "user",
        "namespace": "",
    }
    ...
    with (
      patch ("proxy.app.api.auth_endpoints.get_user_db", return_value = mock_user_db), patch (
        "proxy.app.auth.user_db.get_user_db", return_value = mock_user_db), ):
      response = auth_enabled_client.post ("/v1/auth/refresh", json = {"token": "valid_refresh_token_abc123"}, )
  ...
  def test_refresh_invalid_token_returns_401 (self, auth_enabled_client, mock_user_db):
    ...
    with (
      patch ("proxy.app.api.auth_endpoints.get_user_db", return_value = mock_user_db), patch (
        "proxy.app.auth.user_db.get_user_db", return_value = mock_user_db), ):
      response = auth_enabled_client.post ("/v1/auth/refresh", json = {"token": "invalid_refresh_token"}, )
  ...
  def test_login_then_use_access_token (self, auth_enabled_client, mock_user_db):
    ...
    with (
      patch ("proxy.app.api.auth_endpoints.get_user_db", return_value = mock_user_db), patch (
        "proxy.app.auth.user_db.get_user_db", return_value = mock_user_db), ):
      # Step 1: Login
      login_response = auth_enabled_client.post ("/v1/auth/login",
          json = {"username": "testuser", "password": "password123"}, )
      ...
      search_results [0].score = 0.95
      search_results [0].payload = {
          "text": "Ctx", "source_type": "t", "source_id": "1", "version": "1.0", "title": "T", "doc_title": "D",
      }
      ...
      with (
        patch ("proxy.app.main.hybrid_search", return_value = search_results), patch ("proxy.app.main.rerank_chunks",
                                                                                      return_value = [0]), patch (
          "proxy.app.main.non_stream_completion", side_effect = mock_llm), ):
        chat_response = auth_enabled_client.post ("/v1/chat/completions",
...
class TestLogoutFlow:
  ...
  def test_logout_returns_ok (self, auth_disabled_client, mock_user_db):
    ...
    with (
      patch ("proxy.app.api.auth_endpoints.get_user_db", return_value = mock_user_db), ):
      response = auth_disabled_client.post ("/v1/auth/logout", json = {"refresh_token": "some_token"}, )
...
class TestTokenValidation:
  ...
  def test_create_and_verify_token (self):
    ...
    token = create_token (user_id = "u1", username = "alice", roles = ["admin", "user"], groups = ["engineering"],
        access_level = "confidential", namespace = "eng", secret = secret, )
  ...
  def test_verify_token_rejects_wrong_secret (self):
    ...
    with (
      patch ("proxy.app.auth.jwt._get_verify_key", return_value = "wrong-secret"), pytest.raises (Exception,
                                                                                                  match = "Invalid "
                                                                                                          "token"), ):
      verify_token (token)

...
class _MockStateGraph:
  ...
  def compile (self, checkpointer = None):
    return _MockCompiledGraph (self._nodes, self._edges, self._conditional_edges, self._entry, )
...
class TestVersionExtractionInQueryProcessing:
  ...
  def test_version_extracted_before_search (self):
    ...
    queries_and_versions = [
        ("Покажи документацию v2.0 по архитектуре", "2.0"), ("Use version 3.1 API docs", "3.1"),
        ("What changed in v1.2.3", "1.2.3"), ("Документы от 2025-06-01", "2025-06-01"),
...
class TestHybridSearchResultsThroughReranking:
  ...
  def test_rerank_filters_top_k (self):
    ...
    with (
      patch ("proxy.app.core.rerank.reranker") as mock_reranker, patch ("proxy.app.core.rerank.cache_manager", None), ):
      ...
      chunks = [
          "Docker containers are used for isolation.", "CI/CD pipeline is configured via .gitlab-ci.yml.",
          "Kubernetes manages container orchestration.", "PostgreSQL is a relational database.",
          "Use GitLab CI for automated deployments.", "Node.js is a JavaScript runtime.",
      ]
...
class TestSLMRoutingDecisions:
  ...
  def test_query_decomposition_splits_complex_queries (self):
    ...
    with patch ("proxy.app.llm.slm._call_slm_sync",
...
class TestOrchestratorFlow:
  ...
  def test_orchestrator_state_flows_through_nodes (self):
    ...
    with (
      patch ("proxy.app.shared.config.USE_LANGGRAPH", True), patch.object (_graph_mod, "StateGraph",
                                                                           _MockStateGraph), patch.object (_graph_mod,
                                                                                                           "END",
                                                                                                           _END_SENTINEL), patch.object (
        _graph_mod, "MemorySaver", _MockMemorySaver), patch.object (_orch_mod,
                                                                    "hybrid_search") as mock_search, patch.object (
        _orch_mod, "rerank_chunks", return_value = [0, 1]), patch.object (_orch_mod, "non_stream_completion",
                                                                          new_callable = MagicMock) as mock_llm,
      patch.object (
        _orch_mod, "non_stream_completion_sync", new_callable = MagicMock) as mock_llm_sync, ):
      ...
      result = orchestrator.invoke ({
          "query": "Что такое RAG?", "version": None, "temperature": 0.2, "max_tokens": 4096, "stream": False,
          "rewritten_query": None, "rewrite_count": 0, "retrieved_chunks": [], "reranked_chunks": [],
          "graph_context": "", "context": "", "answer": "", "sufficient": False,
      })
  ...
  def test_orchestrator_rewrite_loop_limit (self):
    ...
    with (
      patch ("proxy.app.shared.config.USE_LANGGRAPH", True), patch ("proxy.app.shared.config.MAX_RETRIEVAL_LOOPS",
                                                                    1), patch.object (_graph_mod, "StateGraph",
                                                                                      _MockStateGraph), patch.object (
        _graph_mod, "END", _END_SENTINEL), patch.object (_graph_mod, "MemorySaver", _MockMemorySaver), patch.object (
        _orch_mod, "hybrid_search") as mock_search, patch.object (_orch_mod, "rerank_chunks",
                                                                  return_value = [0]), patch.object (_orch_mod,
                                                                                                     "non_stream_completion",
                                                                                                     new_callable = MagicMock) as mock_llm, patch.object (
        _orch_mod, "non_stream_completion_sync", new_callable = MagicMock) as mock_llm_sync, ):
      ...
      orchestrator = RAGOrchestrator ()
      result = orchestrator.invoke ({
          "query": "Сложный запрос с низкой релевантностью", "version": None, "temperature": 0.2, "max_tokens": 4096,
          "stream": False, "rewritten_query": None, "rewrite_count": 2, "retrieved_chunks": [], "reranked_chunks": [],
          "graph_context": "", "context": "", "answer": "", "sufficient": False,
      })
      assert "answer" in result

...
class TestChatCompletionsNonStreaming:
  ...
  def test_chat_completion_returns_openai_format (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = self.search_results), patch ("proxy.app.main.rerank_chunks",
                                                                                         return_value = [
                                                                                             0, 1, 3
                                                                                         ]), patch (
        "proxy.app.main.non_stream_completion", self.mock_llm), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Что такое RAG?"}],
      }
  ...
  def test_chat_completion_builds_context_from_search_results (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search") as mock_search, patch ("proxy.app.main.rerank_chunks",
                                                                    return_value = [0, 2]), patch (
        "proxy.app.main.non_stream_completion", self.mock_llm), ):
      mock_search.return_value = self.search_results

      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Расскажи про CI/CD pipeline"}],
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
      assert response.status_code == 200
      data = response.json ()
  ...
  def test_chat_completion_with_version_filter (self, app_client):
    ...
    with (
      ...
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Расскажи про RAG"}], "rag_version": "2.0",
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
      assert response.status_code == 200
  ...
  def test_chat_completion_handles_empty_search_results (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = []), patch ("proxy.app.main.non_stream_completion",
                                                                        self.mock_llm), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Запрос без результатов"}],
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
      assert response.status_code == 200
      data = response.json ()
  ...
  def test_chat_completion_uses_cache_on_second_request (self, app_client):
    ...
    with (
      ...
      assert (
          response1.json () ["choices"] [0] ["message"] ["content"] == response2.json () ["choices"] [0] ["message"] [
        "content"])
  ...
  def test_chat_completion_force_refresh_skips_cache (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = self.search_results), patch ("proxy.app.main.rerank_chunks",
                                                                                         return_value = [0]), patch (
        "proxy.app.main.non_stream_completion", side_effect = tracking_llm), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Cache test query"}],
      }
      response1 = app_client.post ("/v1/chat/completions", json = payload)
      assert response1.status_code == 200
  ...
  def test_chat_completion_extracts_version_from_query_text (self, app_client):
    ...
    with (
      ...
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Покажи документацию v2.0 про RAG"}],
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
      assert response.status_code == 200
      call_kwargs = mock_search.call_args.kwargs
...
class TestChatCompletionsStreaming:
  ...
  def test_streaming_response_returns_sse_format (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search") as mock_search, patch ("proxy.app.main.rerank_chunks",
                                                                    return_value = [0]), patch (
        "proxy.app.main.stream_completion", side_effect = mock_stream_llm), ):
      ...
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Что такое RAG?"}], "stream": True,
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
      assert response.status_code == 200
...
class TestErrorHandling:
  ...
  def test_chat_completion_handles_rerank_exception (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = [mock_hit]), patch ("proxy.app.main.rerank_chunks",

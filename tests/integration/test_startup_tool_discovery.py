...
def tools_client ():
  ...
  saved = {}
  for attr in (
        "cache_manager", "USE_LANGGRAPH", "LOG_REQUESTS", "LLM_MODEL_NAME", "WARMUP_ENABLED", "OTEL_ENABLED",
        "GRACEFUL_SHUTDOWN_ENABLED", "USE_REDIS",
  ):
...
class TestStartupDiscoveryIntegration:
  ...
  def test_tools_endpoint_returns_discovered_tools (self, tools_client):
    ...
    registry.register (ToolDefinition (name = "search_documents", description = "Search indexed documents",
        parameters = [ToolParam (name = "query", type = str, description = "Query")], category = "search",
        tags = ["hybrid"], visibility = ToolVisibility.PUBLIC, provider = "sdk", ))
    registry.register (
        ToolDefinition (name = "list_pets", description = "List pets from the store", parameters = [], category = "api",
            tags = ["openapi", "pets"], visibility = ToolVisibility.PUBLIC, provider = "openapi", ))
    registry.register (ToolDefinition (name = "weekly_report", description = "Generate weekly report",
        parameters = [ToolParam (name = "team", type = str)], category = "reporting",
        tags = ["declarative", "scheduled"], visibility = ToolVisibility.PUBLIC, provider = "declarative", ))
  ...
  def test_tools_endpoint_filter_by_provider (self, tools_client):
    ...
    registry.register (ToolDefinition (name = "sdk_search", description = "SDK search tool",
        parameters = [ToolParam (name = "q", type = str)], category = "search", tags = [],
        visibility = ToolVisibility.PUBLIC, provider = "sdk", ))
    registry.register (
        ToolDefinition (name = "openapi_get", description = "OpenAPI GET tool", parameters = [], category = "api",
            tags = [], visibility = ToolVisibility.PUBLIC, provider = "openapi", ))
  ...
  def test_tools_endpoint_returns_correct_fields (self, tools_client):
    ...
    registry.register (ToolDefinition (name = "test_tool", description = "Test description",
        parameters = [ToolParam (name = "input", type = str, description = "Input value")], category = "testing",
        tags = ["integration"], visibility = ToolVisibility.PUBLIC, version = "1.0.0", provider = "sdk", ))

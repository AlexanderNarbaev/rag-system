...
class TestReadinessProbe:
  ...
  def test_ready_returns_200_when_all_components_ok (self, app_client):
    ...
    with (
      patch ("proxy.app.core.retrieval.qdrant_client", _mock_qdrant_ok ()), patch ("requests.get",
  ...
  def test_ready_returns_503_when_qdrant_unavailable (self, app_client):
    ...
    with (
      patch ("proxy.app.core.retrieval.qdrant_client", _mock_qdrant_fail ()), patch ("requests.get",
                                                                                     return_value = _mock_llm_response (
                                                                                         200)), ):
      response = app_client.get ("/v1/health/ready")
...
class TestHealthEndpoint:
  ...
  def test_health_degraded_when_qdrant_unavailable (self, app_client):
    ...
    with (
      patch ("proxy.app.core.retrieval.qdrant_client", _mock_qdrant_fail ("Timeout")), patch ("requests.get",
                                                                                              return_value =
                                                                                              _mock_llm_response (
                                                                                                  200)), ):
      response = app_client.get ("/v1/health")

...
def _make_scored_point (text, score = 0.95, source_type = "confluence", version = "1.0"):
  ...
  point.score = score
  point.payload = {
      "text": text, "source_type": source_type, "source_id": "src_1", "version": version, "title": "Test Title",
      "doc_title": "Test Doc",
  }
  return point
...
class TestStreamingChatCompletion:
  ...
  def test_streaming_returns_sse_content_type (self, app_client):
    ...
    async def mock_stream (*args, **kwargs):
      ...
      yield {
          "id": "1", "object": "chat.completion.chunk", "choices": [{"delta": {"content": " world"}, "index": 0}],
      }
    
    with (
      patch ("proxy.app.main.hybrid_search", return_value = search_results), patch ("proxy.app.main.rerank_chunks",
                                                                                    return_value = [0]), patch (
        "proxy.app.main.stream_completion", side_effect = mock_stream), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Test streaming"}], "stream": True,
      }
  ...
  def test_streaming_sends_done_sentinel (self, app_client):
    ...
    async def mock_stream (*args, **kwargs):
      yield {
          "id": "1", "object": "chat.completion.chunk", "choices": [{"delta": {"content": "Answer"}, "index": 0}],
      }
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = search_results), patch ("proxy.app.main.rerank_chunks",
        ...
        "proxy.app.main.stream_completion", side_effect = mock_stream), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Test"}], "stream": True,
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
      body = response.text
  ...
  def test_streaming_chunks_are_valid_json (self, app_client):
    ...
    async def mock_stream (*args, **kwargs):
      yield {
          "id": "1", "object": "chat.completion.chunk", "choices": [{"delta": {"content": "Part1"}, "index": 0}],
      }
      yield {
          "id": "1", "object": "chat.completion.chunk", "choices": [{"delta": {"content": "Part2"}, "index": 0}],
      }
      yield {
          "id": "1", "object": "chat.completion.chunk", "choices": [{"delta": {"content": "Part3"}, "index": 0}],
      }
    ...
    with (
      ...
      data_events = [e for e in events if
          "_done" not in e and "_raw" not in e and "role" not in e and "rag_feedback_id" not in e]
  ...
  def test_streaming_handles_empty_choices (self, app_client):
    ...
    async def mock_stream (*args, **kwargs):
      yield {
          "id": "1", "object": "chat.completion.chunk", "choices": [{"delta": {"content": "Good"}, "index": 0}],
      }
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = search_results), patch ("proxy.app.main.rerank_chunks",
        ...
        "proxy.app.main.stream_completion", side_effect = mock_stream), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Test empty choices"}], "stream": True,
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
  ...
  def test_streaming_handles_choices_with_no_content (self, app_client):
    ...
    async def mock_stream (*args, **kwargs):
      yield {
          "id": "1", "object": "chat.completion.chunk", "choices": [{"delta": {"role": "assistant"}, "index": 0}],
      }
      yield {
  ...
  def test_streaming_feedback_before_done (self, app_client):
    ...
    async def mock_stream (*args, **kwargs):
      yield {
          "id": "1", "object": "chat.completion.chunk", "choices": [{"delta": {"content": "Text"}, "index": 0}],
      }
  ...
  def test_streaming_with_empty_search_results (self, app_client):
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = []), patch ("proxy.app.main.stream_completion",
                                                                        side_effect = mock_stream), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Unknown topic"}], "stream": True,
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
  ...
  def test_streaming_error_yields_error_event (self, app_client):
    ...
    async def mock_stream (*args, **kwargs):
      yield {
          "id": "1", "object": "chat.completion.chunk", "choices": [{"delta": {"content": "Start"}, "index": 0}],
      }
    ...
    with (
      patch ("proxy.app.main.hybrid_search", return_value = search_results), patch ("proxy.app.main.rerank_chunks",
        ...
        "proxy.app.main.stream_completion", side_effect = mock_stream), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Test error"}], "stream": True,
      }
      response = app_client.post ("/v1/chat/completions", json = payload)
      body = response.text
  ...
  def test_streaming_search_failure_graceful_degradation (self, app_client):
    ...
    async def mock_stream (*args, **kwargs):
      yield {
          "id": "1", "object": "chat.completion.chunk", "choices": [{"delta": {"content": "Fallback"}, "index": 0}],
      }
    
    with (
      patch ("proxy.app.main.hybrid_search", side_effect = Exception ("Qdrant timeout")), patch (
        "proxy.app.main.stream_completion", side_effect = mock_stream), ):
      payload = {
          "model": "test-model", "messages": [{"role": "user", "content": "Test degraded streaming"}], "stream": True,
      }
      response = app_client.post ("/v1/chat/completions", json = payload)

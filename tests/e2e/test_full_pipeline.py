...
def mock_rag_pipeline_with_context ():
  ...
  chunk_a = {
      "text": "RAG (Retrieval-Augmented Generation) combines retrieval with generation.", "source_type": "confluence",
      "title": "RAG Overview", "doc_title": "Architecture Guide", "version": "2.0",
  }
  chunk_b = {
  ...
  with (
    ...
    yield {
        "hybrid_search": mock_hybrid, "rerank_chunks": mock_rerank, "deduplicate_chunks": mock_dedup,
        "build_context": mock_build, "non_stream_completion": mock_nonstream, "stream_completion": mock_stream,
        "log_interaction": mock_log, "chunks": [chunk_a, chunk_b],
    }
...
class TestChatCompletionWithRAGContext:
  ...
  def test_chat_response_includes_rag_extensions (self, client, mock_rag_pipeline):
    ...
    response = client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "Explain RAG architecture"}], "stream": False,
    }, )
    assert response.status_code == 200
    data = response.json ()
  ...
  def test_chat_with_version_parameter (self, client, mock_rag_pipeline):
    ...
    response = client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "What changed?"}], "rag_version": "2.0",
        "stream": False,
    }, )
    assert response.status_code == 200
  ...
  def test_chat_with_force_refresh_bypasses_cache (self, client, mock_rag_pipeline):
    ...
    response = client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "Latest info"}], "rag_force_refresh": True,
        "stream": False,
    }, )
    assert response.status_code == 200
  ...
  def test_chat_with_custom_temperature_and_max_tokens (self, client, mock_rag_pipeline):
    ...
    response = client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "test"}], "temperature": 0.7, "max_tokens": 2048,
        "stream": False,
    }, )
    assert response.status_code == 200
  ...
  def test_chat_with_multi_turn_conversation (self, client, mock_rag_pipeline_with_context):
    ...
    response = client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [
            {"role": "system", "content": "You are a helpful assistant."}, {"role": "user", "content": "What is RAG?"},
  ...
  def test_chat_missing_user_message_returns_400 (self, client, mock_rag_pipeline):
    ...
    response = client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "system", "content": "You are helpful."}], "stream": False,
    }, )
  ...
  def test_chat_skip_generation_returns_chunks_only (self, client, mock_rag_pipeline_with_context):
    ...
    response = client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "What is RAG?"}], "rag_skip_generation": True,
        "stream": False,
    }, )
    assert response.status_code == 200
    data = response.json ()
...
class TestStreamingChatCompletion:
  ...
  def test_streaming_ends_with_done_sentinel (self, client, mock_rag_pipeline):
    ...
    response = client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "Hello"}], "stream": True,
    }, )
    assert response.status_code == 200
    body = response.text
    assert "[DONE]" in body
...
class TestFeedbackSubmission:
  ...
  def test_positive_feedback_accepted (self, client, mock_rag_pipeline, mock_feedback_logger):
    ...
    with patch ("proxy.app.shared.config.ENRICHMENT_ENABLED", False):
      response = client.post ("/v1/feedback", json = {
          "feedback_id": "fb_test123", "rating": "positive", "comment": "Great answer!",
      }, )
      assert response.status_code == 200
      data = response.json ()
  ...
  def test_negative_feedback_with_correction_accepted (self, client, mock_rag_pipeline, mock_feedback_logger):
    ...
    with patch ("proxy.app.shared.config.ENRICHMENT_ENABLED", False):
      response = client.post ("/v1/feedback", json = {
          "feedback_id": "fb_test456", "rating": "negative", "correction": "The correct answer is...",
  ...
  def test_feedback_invalid_rating_returns_422 (self, client, mock_rag_pipeline):
    ...
    response = client.post ("/v1/feedback", json = {
        "feedback_id": "fb_test", "rating": "neutral",  # invalid — must be positive/negative
    }, )
    assert response.status_code == 422
...
class TestHealthChecks:
  ...
  def test_health_ready_with_all_services_up (self, client):
    ...
    with (
      patch ("proxy.app.core.retrieval.qdrant_client") as mock_qdrant, patch ("requests.get") as mock_get, ):
  ...
  def test_health_ready_with_llm_down (self, client):
    ...
    with (
      patch ("proxy.app.core.retrieval.qdrant_client") as mock_qdrant, patch ("requests.get", side_effect = Exception (
        "LLM unreachable")), ):
      mock_qdrant.get_collections.return_value = {}
...
class TestFullPipelineIntegration:
  ...
  def test_chat_then_feedback_flow (self, client, mock_rag_pipeline, mock_feedback_logger):
    ...
    with patch ("proxy.app.shared.config.ENRICHMENT_ENABLED", False):
      feedback_response = client.post ("/v1/feedback", json = {
          "feedback_id": feedback_id, "rating": "positive", "comment": "Accurate answer",
      }, )
  ...
  def test_chat_with_retrieval_context_then_feedback_with_correction (
    ...
    chat_response = client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "Explain hybrid search"}], "stream": False,
    }, )
    assert chat_response.status_code == 200
    chat_data = chat_response.json ()
    ...
    with patch ("proxy.app.shared.config.ENRICHMENT_ENABLED", False):
      fb_response = client.post ("/v1/feedback", json = {
          "feedback_id": chat_data ["rag_feedback_id"], "rating": "negative",
  ...
  def test_health_then_chat_then_health (self, client, mock_rag_pipeline):
    ...
    chat = client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "test"}], "stream": False,
    }, )

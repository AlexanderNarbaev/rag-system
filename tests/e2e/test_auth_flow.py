...
class TestLoginFlow:
  ...
  def test_login_flow (self, service_url: str):
    ...
    login_resp = requests.post (f"{service_url}/v1/auth/login", json = {"username": "testuser", "password": "testpass"},
        timeout = 10, )
    ...
    chat_resp = requests.post (f"{service_url}/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "Hello"}], "stream": False,
  ...
  def test_auth_me_endpoint (self, service_url: str, auth_headers: dict):
    ...
    resp = requests.get (f"{service_url}/v1/auth/me", headers = auth_headers, timeout = 10, )
  ...
  def test_token_refresh (self, service_url: str, auth_headers: dict):
    ...
    resp = requests.post (f"{service_url}/v1/auth/refresh", json = {"token": token}, timeout = 10, )
...
class TestUnauthorizedRequests:
  ...
  def test_login_invalid_credentials (self, service_url: str):
    ...
    resp = requests.post (f"{service_url}/v1/auth/login", json = {"username": "invalid", "password": "wrong"},
        timeout = 10, )
    if resp.status_code != 200:

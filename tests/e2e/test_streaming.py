...
class TestStreamingRAG:
  ...
  def test_streaming_rag (self, service_url: str, auth_headers: dict):
    ...
    payload = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "What is RAG?"}], "temperature": 0.2,
        "stream": True,
    }
    resp = requests.post (f"{service_url}/v1/chat/completions", json = payload,
  ...
  def test_streaming_empty_query (self, service_url: str, auth_headers: dict):
    ...
    resp = requests.post (f"{service_url}/v1/chat/completions", json = payload,
        headers = {**auth_headers, "Accept": "text/event-stream"}, timeout = 30, stream = True, )
    assert resp.status_code == 200
    chunk_count = 0
    for line in resp.iter_lines (decode_unicode = True):

...
def _make_mock_chunks (count: int = 3):
  ...
  for i in range (count):
    ...
    chunk.payload = {
        "text": f"This is relevant context chunk {i} about RAG systems.", "source_type": "confluence",
        "source_id": f"page_{i}", "title": f"RAG Documentation Part {i}", "doc_title": f"RAG Guide {i}",
        "version": "1.0",
    }
    chunks.append (chunk)
  return chunks
...
class TestFullRAGPipeline:
  ...
  def test_feedback_submission (self, app_client, mock_rag_pipeline, mock_feedback_logger):
    ...
    with patch ("proxy.app.shared.config.ENRICHMENT_ENABLED", False):
      response = app_client.post ("/v1/feedback", json = {
          "feedback_id": "fb_test_123", "rating": "positive", "comment": "Good answer",
      }, )
    assert response.status_code == 200
    data = response.json ()
  ...
  def test_chat_then_feedback_flow (self, app_client, mock_rag_pipeline, mock_feedback_logger):
    ...
    # Step 1: Chat request
    chat_response = app_client.post ("/v1/chat/completions", json = {
        "model": "rag-proxy", "messages": [{"role": "user", "content": "What is RAG?"}], "stream": False,
    }, )

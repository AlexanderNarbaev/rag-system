...
class TestEnhancedToolRegistry:
    ...
    def test_get_dependency_graph(self):
        self.registry.register(ToolDefinition(name="dep1", description="d", handler=lambda: "", depends_on=["dep2"]))
        graph = self.registry.get_dependency_graph()
        assert "dep1" in graph

    def test_list_by_category(self):
        self.registry.register(ToolDefinition(name="cat1", description="d", handler=lambda: "", category="test_cat"))

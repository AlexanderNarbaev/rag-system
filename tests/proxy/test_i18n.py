...
from proxy.app.shared.i18n import (
  FALLBACK_MESSAGES, SUPPORTED_LANGUAGES, SYSTEM_PROMPTS, detect_language, get_fallback_message, get_system_prompt,
)
...
class TestDetectLanguage:
  ...
  def test_long_multilingual_text_returns_majority (self):
    long_de = ("Dies ist ein sehr langer deutscher Text mit vielen Wörtern über CI/CD-Pipelines und deren "
               "Konfiguration in verschiedenen Umgebungen.")
    assert detect_language (long_de) == "de"
  
  def test_long_french_text (self):
    long_fr = ("Ceci est un très long texte français avec de nombreux mots sur les pipelines CI/CD et leur "

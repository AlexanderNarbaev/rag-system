...
class TestTracingSetup:
  ...
  def test_setup_tracing_initializes_provider (self):
    ...
    try:
      with (
        patch ("proxy.app.shared.tracing.OTEL_ENABLED", True), patch ("proxy.app.shared.tracing.OTEL_EXPORTER_ENDPOINT",
                                                                      "http://localhost:4318/v1/traces"), patch (
          "proxy.app.shared.tracing.OTEL_BATCH_TIMEOUT", 5), patch (
          "proxy.app.shared.tracing.OTLPSpanExporter") as mock_exporter_cls, patch (
          "proxy.app.shared.tracing.BatchSpanProcessor") as mock_bsp_cls, patch (
          "proxy.app.shared.tracing.TracerProvider") as mock_provider_cls, patch ("proxy.app.shared.tracing.trace"), ):
  ...
  def test_setup_tracing_sets_global_provider (self):
    ...
    with (
      patch ("proxy.app.shared.tracing.OTEL_ENABLED", True), patch (
        "proxy.app.shared.tracing.trace") as mock_trace, patch ("proxy.app.shared.tracing.OTLPSpanExporter"), patch (
        "proxy.app.shared.tracing.BatchSpanProcessor"), patch (
        "proxy.app.shared.tracing.TracerProvider") as mock_provider_cls, ):
      mock_provider = MagicMock ()
      mock_provider_cls.return_value = mock_provider
  ...
  def test_setup_tracing_handles_exporter_error (self):
    ...
    with (
      patch ("proxy.app.shared.tracing.OTEL_ENABLED", True), patch ("proxy.app.shared.tracing.OTLPSpanExporter",
                                                                    side_effect = ConnectionError (
                                                                        "Cannot connect")), patch (
        "proxy.app.shared.tracing.TracerProvider") as mock_provider_cls, ):
      mock_provider = MagicMock ()
      mock_provider_cls.return_value = mock_provider

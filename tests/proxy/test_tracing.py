...
class TestTracingNoOpWhenDisabled:
    ...
    def test_setup_tracing_noop_when_disabled(self):
        ...
        from proxy.app.tracing import setup_tracing

        with patch("proxy.app.tracing.OTEL_ENABLED", False):
            result = setup_tracing()
            assert result is None

    def test_tracer_is_proxy_when_disabled(self):
        """Module-level tracer should be a no-op proxy when tracing is disabled."""
        from proxy.app.tracing import tracer
    ...
    def test_get_current_span_noop(self):
        ...
        from proxy.app.tracing import get_current_span
    ...
    def test_add_event_noop(self):
        ...
        from proxy.app.tracing import add_event

        with patch("proxy.app.tracing.trace.get_current_span") as mock_span_fn:
            mock_span = MagicMock()
    ...
    def test_set_span_error_noop(self):
        ...
        from proxy.app.tracing import set_span_error
...
class TestTracingSetup:
    ...
    def test_setup_tracing_initializes_provider(self):
        ...
        from proxy.app.tracing import setup_tracing, _tracing_initialized, _tracer_provider
        ...
        try:
            with (
                patch("proxy.app.tracing.OTEL_ENABLED", True),
                patch("proxy.app.tracing.OTEL_EXPORTER_ENDPOINT", "http://localhost:4318/v1/traces"),
                patch("proxy.app.tracing.OTEL_BATCH_TIMEOUT", 5),
                patch("proxy.app.tracing.OTLPSpanExporter") as mock_exporter_cls,
                patch("proxy.app.tracing.BatchSpanProcessor") as mock_bsp_cls,
                patch("proxy.app.tracing.TracerProvider") as mock_provider_cls,
                patch("proxy.app.tracing.trace"),
            ):
            ...
            import proxy.app.tracing as tracing_mod
    ...
    def test_setup_tracing_sets_global_provider(self):
        ...
        with (
            patch("proxy.app.tracing.OTEL_ENABLED", True),
            patch("proxy.app.tracing.trace") as mock_trace,
            patch("proxy.app.tracing.OTLPSpanExporter"),
            patch("proxy.app.tracing.BatchSpanProcessor"),
            patch("proxy.app.tracing.TracerProvider") as mock_provider_cls,
        ):
            mock_provider = MagicMock()
            mock_provider_cls.return_value = mock_provider
    ...
    def test_setup_tracing_handles_exporter_error(self):
        ...
        with (
            patch("proxy.app.tracing.OTEL_ENABLED", True),
            patch("proxy.app.tracing.OTLPSpanExporter", side_effect=ConnectionError("Cannot connect")),
            patch("proxy.app.tracing.TracerProvider") as mock_provider_cls,
        ):
            mock_provider = MagicMock()
            mock_provider_cls.return_value = mock_provider
...
class TestTracingConfig:
    ...
    def test_otel_config_defaults(self):
        ...
        import proxy.app.config as cfg

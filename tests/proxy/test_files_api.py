...
def client ():
  ...
  with (
    patch ("proxy.app.api.files.require_role") as mock_require, patch (
      "proxy.app.api.files.MinioClient") as mock_minio_cls, ):
...
class TestUploadFile:
  def test_upload_success (self, client):
    ...
    response = c.post ("/v1/files", files = {"file": ("test.txt", io.BytesIO (b"hello"), "text/plain")}, )
  ...
  def test_upload_rejects_disallowed_content_type (self, client):
    ...
    c, mock_minio = client
    response = c.post ("/v1/files", files = {"file": ("test.exe", io.BytesIO (b"MZ"), "application/x-msdownload")}, )
  ...
  def test_upload_rejects_oversized_file (self, client):
    ...
    response = c.post ("/v1/files", files = {"file": ("big.txt", io.BytesIO (large_content), "text/plain")}, )
  ...
  def test_upload_handles_storage_error (self, client):
    ...
    response = c.post ("/v1/files", files = {"file": ("test.txt", io.BytesIO (b"data"), "text/plain")}, )
...
class TestListFiles:
  ...
  def test_list_files_with_results (self, client):
    ...
    mock_minio.list_files.return_value = [
        {
            "key": "uploads/abc/test.txt", "size": 100, "last_modified": "2025-01-01T00:00:00", "etag": "abc123",
        }
    ]
...
class TestGetFileMetadata:
  def test_metadata_success (self, client):
    ...
    mock_minio.get_file_metadata.return_value = {
        "key": "uploads/abc/test.txt", "size": 42, "last_modified": "2025-01-01T00:00:00", "content_type": "text/plain",
        "metadata": {"original_filename": "test.txt"}, "etag": "abc",
    }
...
class TestDownloadFile:
  def test_download_success (self, client):
    ...
    c, mock_minio = client
    mock_minio.get_file_metadata.return_value = {
        "key": "test.txt", "content_type": "text/plain", "metadata": {"original_filename": "test.txt"},
    }

...
class TestRerankChunks:
    ...
    def test_rerank_with_cache_hit(self):
        ...
        with (
            patch("proxy.app.core.rerank.reranker", mock_reranker),
            patch("proxy.app.core.rerank.cache_manager", mock_cache),
        ):
    ...
    def test_truncate_applied(self):
        ...
        with (
            ...
            patch("proxy.app.core.rerank._truncate_text") as mock_trunc,
        ):
    ...
    def test_auto_initialize(self):
        with (
            patch("proxy.app.core.rerank.reranker", None),
            patch("proxy.app.core.rerank.initialize_reranker") as mock_init,
            patch("proxy.app.core.rerank.cache_manager", None),
        ):
...
class TestRerankChunksWithScores:
    ...
    def test_returns_index_score_pairs(self):
        with (
            patch("proxy.app.core.rerank.rerank_chunks", return_value=[2, 0]),
...
class TestInitializeReranker:
    ...
    def test_initializes_with_in_memory_cache(self):
        ...
        with (
            patch("proxy.app.llm.remote_services.create_reranker", return_value=mock_reranker),
            patch("proxy.app.core.rerank.USE_REDIS", False),
        ):
            initialize_reranker()
    ...
    def test_initializes_with_redis(self):
        ...
        with (
            patch("proxy.app.llm.remote_services.create_reranker", return_value=mock_reranker),
            patch("proxy.app.core.rerank.USE_REDIS", True),
            patch("proxy.app.core.rerank.REDIS_URL", "redis://test:6379"),
        ):
            initialize_reranker()
            from proxy.app.core.rerank import cache_manager

...
class TestWarmupEmbedder:
    ...
    async def test_returns_true_on_success(self):
        with patch("app.retrieval.hybrid_search", return_value=[]) as mock_search:
    ...
    async def test_returns_false_on_failure(self):
        with patch("app.retrieval.hybrid_search", side_effect=RuntimeError("No GPU")) as mock_search:
            result = await warmup_embedder()
    ...
    async def test_import_failure_graceful(self):
        with patch("app.retrieval.hybrid_search", side_effect=ImportError()):
            result = await warmup_embedder()
            assert result is False
...
class TestWarmupReranker:
    ...
    @pytest.mark.asyncio
    async def test_returns_true_on_success(self):
        with patch("app.rerank.rerank_chunks", return_value=[0]) as mock_rerank:
    ...
    @pytest.mark.asyncio
    async def test_returns_false_on_failure(self):
        with patch("app.rerank.rerank_chunks", side_effect=RuntimeError("OOM")):
            result = await warmup_reranker()
            assert result is False

    @pytest.mark.asyncio
    async def test_passes_warmup_query_and_doc(self):
        with patch("app.rerank.rerank_chunks") as mock_rerank:
...
class TestWarmupLLM:
    ...
    async def test_returns_true_on_success(self):
        ...
        with patch("proxy.app.warmup.LLM_ENDPOINT", "http://localhost:8000/v1"):
            with patch("proxy.app.warmup.LLM_MODEL_NAME", "test-model"):
                with patch("app.provider_adapter.non_stream_completion", mock_completion):
    ...
    async def test_sends_minimal_tokens(self):
        ...
        with patch("proxy.app.warmup.LLM_ENDPOINT", "http://localhost:8000/v1"):
            with patch("proxy.app.warmup.LLM_MODEL_NAME", "test-model"):
                with patch("app.provider_adapter.non_stream_completion", mock_completion) as mock:
                    await warmup_llm()
    ...
    async def test_skips_when_no_llm_configured(self):
        ...
        with patch("proxy.app.warmup.LLM_ENDPOINT", ""):
            with patch("proxy.app.warmup.LLM_MODEL_NAME", ""):
                result = await warmup_llm()
                assert result is False
...
class TestWarmupAll:
    ...
    async def test_returns_disabled_when_warmup_disabled(self):
        with patch("proxy.app.warmup.WARMUP_ENABLED", False):
    ...
    async def test_returns_ok_when_all_succeed(self):
        with patch("proxy.app.warmup.WARMUP_ENABLED", True):
            with patch("proxy.app.warmup.warmup_embedder", AsyncMock(return_value=True)):
                with patch("proxy.app.warmup.warmup_reranker", AsyncMock(return_value=True)):
                    with patch("proxy.app.warmup.warmup_llm", AsyncMock(return_value=True)):
                        result = await warmup_all()
    ...
    async def test_returns_partial_when_one_fails(self):
        with patch("proxy.app.warmup.WARMUP_ENABLED", True):
            with patch("proxy.app.warmup.warmup_embedder", AsyncMock(return_value=True)):
                with patch("proxy.app.warmup.warmup_reranker", AsyncMock(return_value=False)):
                    with patch("proxy.app.warmup.warmup_llm", AsyncMock(return_value=True)):
                        result = await warmup_all()
    ...
    async def test_handles_exceptions_as_false(self):
        with patch("proxy.app.warmup.WARMUP_ENABLED", True):
            with patch("proxy.app.warmup.warmup_embedder", AsyncMock(return_value=True)):
                with patch("proxy.app.warmup.warmup_reranker", AsyncMock(side_effect=Exception("crash"))):
                    with patch("proxy.app.warmup.warmup_llm", AsyncMock(return_value=True)):
                        result = await warmup_all()
                        assert result["reranker"] is False
                        assert result["status"] == "partial"
...
class TestWarmupConfig:
    ...
    def test_config_defaults(self):
        from proxy.app.config import WARMUP_ENABLED, WARMUP_ON_STARTUP
    ...
    def test_config_env_overrides(self):
        ...
        assert proxy.app.config.WARMUP_ENABLED is False
        assert proxy.app.config.WARMUP_ON_STARTUP is False

...
class TestGenerateHypotheticalAnswer:
    ...
    def test_generates_answer_when_slm_available(self):
        with patch("proxy.app.hyde._call_slm_sync", return_value="A test answer."):
    ...
    def test_returns_original_query_when_slm_empty(self):
        with patch("proxy.app.hyde._call_slm_sync", return_value=""):
            result = generate_hypothetical_answer("What is testing?")
            assert result == "What is testing?"

    def test_returns_original_query_when_slm_fails(self):
        with patch("proxy.app.hyde._call_slm_sync", side_effect=Exception("SLM down")):
            result = generate_hypothetical_answer("What is testing?")
            assert result == "What is testing?"

    def test_strips_whitespace_from_result(self):
        with patch("proxy.app.hyde._call_slm_sync", return_value="   A padded answer.   "):
            result = generate_hypothetical_answer("Query")
            assert result == "A padded answer."

    def test_empty_query(self):
        with patch("proxy.app.hyde._call_slm_sync", return_value="Answer"):
...
class TestHydeSearch:
    ...
    @patch("proxy.app.hyde.hybrid_search")
    @patch("proxy.app.hyde.embedder")
    def test_search_returns_chunks(self, mock_embedder, mock_search):
        ...
        with patch("proxy.app.hyde._call_slm_sync", return_value="A hypothetical answer about documents."):
            with patch("proxy.app.hyde.HYDE_ENABLED", True):
    ...
    def test_search_fallback_when_slm_fails(self, mock_embedder, mock_search):
        ...
        with patch("proxy.app.hyde._call_slm_sync", side_effect=Exception("SLM error")):
            with patch("proxy.app.hyde.HYDE_ENABLED", True):
    ...
    def test_search_returns_empty_on_search_failure(self, mock_embedder, mock_search):
        ...
        with patch("proxy.app.hyde._call_slm_sync", return_value="Hypothesis"):
            with patch("proxy.app.hyde.HYDE_ENABLED", True):
    ...
    def test_search_skips_when_hyde_disabled(self, mock_embedder, mock_search):
        ...
        with patch("proxy.app.hyde.HYDE_ENABLED", False):
            result = hyde_search("Query")
    ...
    def test_search_with_version(self, mock_embedder, mock_search):
        ...
        with patch("proxy.app.hyde._call_slm_sync", return_value="Versioned answer"):
            with patch("proxy.app.hyde.HYDE_ENABLED", True):

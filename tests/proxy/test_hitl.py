...
from proxy.app.core.hitl import (
  FeedbackType, InteractionLogger, export_intent_dataset, export_training_dataset, get_logger, log_feedback_sync,
  log_interaction,
)
...
class TestInteractionLogger:
  ...
  def test_log_interaction_writes_file (self, logger, log_dir):
    logger.log_interaction (request_id = "req-1", user_query = "How to set up CI?", context = "Some context",
        response = "Use .gitlab-ci.yml", metadata = {"model": "test-model"}, )
  ...
  def test_log_interaction_with_feedback (self, logger):
    logger.log_interaction (request_id = "req-2", user_query = "test", context = "ctx", response = "resp",
        user_feedback = FeedbackType.POSITIVE, corrected_response = "better resp", )
    with open (logger.interactions_file) as f:
  ...
  def test_log_feedback_writes_file (self, logger):
    logger.log_feedback (request_id = "req-1", feedback_type = FeedbackType.POSITIVE, comment = "Great answer!",
        expert_id = "expert-42")
  ...
  def test_log_feedback_with_correction (self, logger):
    logger.log_feedback (request_id = "req-x", feedback_type = FeedbackType.CORRECTION,
  ...
  def test_get_interactions_returns_reverse_order (self, logger):
    for i in range (5):
      logger.log_interaction (request_id = f"req-{i}", user_query = f"query {i}", context = "ctx",
                              response = f"resp {i}")
...
class TestLogInteractionAsync:
  ...
  async def test_calls_logger_when_enabled (self):
    ...
    with (
      patch ("proxy.app.core.hitl.LOG_REQUESTS", True), patch ("proxy.app.core.hitl.get_logger",
                                                               return_value = mock_logger), patch ("asyncio.to_thread",
                                                                                                   new_callable =
                                                                                                   lambda: AsyncMock
                                                                                                       ()) as
    mock_thread, ):
...
class TestExportTrainingDataset:
  ...
  def test_exports_corrected_responses (self, tmp_path):
    ...
    logger.log_interaction (request_id = "r1", user_query = "How to X?", context = "ctx", response = "bad answer",
        corrected_response = "good answer", )
    logger.log_interaction (request_id = "r2", user_query = "What is Y?", context = "ctx", response = "just ok",
        user_feedback = FeedbackType.POSITIVE, )
    logger.log_interaction (request_id = "r3", user_query = "What is Z?", context = "ctx", response = "bad",
...
class TestExportIntentDataset:
  ...
  def test_exports_query_intent_pairs (self, tmp_path):
    logger = InteractionLogger (log_dir = tmp_path)
    logger.log_interaction (request_id = "r1", user_query = "How to set up CI/CD?", context = "ctx",
        response = "Use .gitlab-ci.yml", )
    logger.log_interaction (request_id = "r2", user_query = "What is Docker?", context = "ctx",
        response = "A container platform", )
    logger.log_interaction (request_id = "r3", user_query = "Compare Kubernetes vs Nomad", context = "ctx",
    ...
    with (
      patch ("proxy.app.core.hitl.get_logger", return_value = logger), patch ("proxy.app.llm.slm.classify_intent",
  ...
  def test_writes_jsonl_format (self, tmp_path):
    logger = InteractionLogger (log_dir = tmp_path)
    logger.log_interaction (request_id = "r1", user_query = "Hello", context = "ctx", response = "Hi there!", )
    ...
    assert record ["intent"] in (
        "greeting", "simple_fact", "factual", "procedural", "comparison", "summarize", "complex", "unknown",
    )
  
  def test_skips_interactions_with_empty_query (self, tmp_path):
    logger = InteractionLogger (log_dir = tmp_path)
    logger.log_interaction (request_id = "r1", user_query = "", context = "ctx", response = "resp", )
    logger.log_interaction (request_id = "r2", user_query = "Valid query", context = "ctx", response = "resp", )
  ...
  def test_uses_multilingual_classification_when_available (self, tmp_path):
    logger = InteractionLogger (log_dir = tmp_path)
    logger.log_interaction (request_id = "r1", user_query = "Bonjour", context = "ctx", response = "Bonjour!", )
    ...
    with (
      patch ("proxy.app.core.hitl.get_logger", return_value = logger), patch (
        "proxy.app.llm.slm.classify_intent_multilingual", side_effect = mock_multilingual), ):

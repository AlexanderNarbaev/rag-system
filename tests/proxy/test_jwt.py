...
from proxy.app.auth.jwt import (
  UserContext, _get_verify_key, blacklist_access_token, create_mock_token, create_token, create_token_pair,
...
class TestCreateTokenPair:
  ...
  async def test_access_token_contains_claims (self):
    ...
    with patch ("proxy.app.auth.user_db.get_user_db", return_value = mock_db):
      ...
      decoded = jwt.decode (result ["access_token"], key = "test-secret-key-for-unit-tests", algorithms = ["HS256"],
...
class TestBlacklistAccessToken:
  ...
  async def test_blacklists_valid_token_with_jti (self):
    ...
    now = int (time.time ())
    payload = {
        "sub": "u1", "preferred_username": "alice", "roles": ["user"], "groups": [], "access_level": "internal",
        "namespace": "", "iat": now, "exp": now + 3600, "jti": "unique-jti-123", "type": "access",
    }
  ...
  async def test_blacklists_expired_token_with_jti (self):
    ...
    now = int (time.time ())
    payload = {
        "sub": "u1", "preferred_username": "alice", "roles": [], "groups": [], "access_level": "internal",
        "iat": now - 7200, "exp": now - 3600,  # Expired
        "jti": "expired-jti-456", "type": "access",
    }
    token = jwt.encode (payload, key = "test-secret-key-for-unit-tests", algorithm = "HS256")
...
class TestCreateMockToken:
  ...
  def test_custom_secret (self):
    ...
    decoded = jwt.decode (token, key = "custom-key", algorithms = ["HS256"], options = {"verify_exp": False}, )
    assert decoded ["sub"] == "test-user"

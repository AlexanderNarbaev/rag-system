...
class TestToolAuditRecord:
  ...
  def test_create_record_with_all_fields (self):
    ...
    record = ToolAuditRecord (timestamp = "2025-01-01T00:00:00Z", tool_name = "search_documents",
        tool_call_id = "call_1", user_id = "user_1", request_id = "req_1", params = {"query": "RAG", "top_k": 5},
  ...
  def test_create_record_with_error (self):
    ...
    record = ToolAuditRecord (timestamp = "2025-01-01T00:00:00Z", tool_name = "search_documents",
        tool_call_id = "call_2", user_id = None, request_id = "req_2", params = {}, result_status = "error",
        duration_ms = 0.0, error = "Tool not found", )
  ...
  def test_to_json_produces_serializable_dict (self):
    ...
    record = ToolAuditRecord (timestamp = "2025-01-01T00:00:00Z", tool_name = "search_documents",
        tool_call_id = "call_3", user_id = "user_1", request_id = "req_3", params = {"query": "test"},
        result_status = "success", duration_ms = 5.0, )
...
class TestToolAuditLoggerStdout:
  ...
  def test_log_invocation_writes_to_stdout (self, audit_logger):
    ...
    sys.stdout = captured
    try:
      audit_logger.log_invocation (tool_name = "search_documents", tool_call_id = "call_1", user_id = "user_1",
          request_id = "req_1", params = {"query": "RAG"}, result_status = "success", duration_ms = 12.5, )
    finally:
  ...
  def test_log_invocation_with_error (self, audit_logger):
    ...
    sys.stdout = captured
    try:
      audit_logger.log_invocation (tool_name = "bad_tool", tool_call_id = "call_err", result_status = "error",
  ...
  def test_log_invocation_sanitizes_params (self, audit_logger):
    ...
    sys.stdout = captured
    try:
      audit_logger.log_invocation (tool_name = "auth_tool", tool_call_id = "call_3",
  ...
  def test_log_invocation_rounds_duration (self, audit_logger):
    ...
    sys.stdout = captured
    try:
      audit_logger.log_invocation (tool_name = "t", tool_call_id = "c", duration_ms = 12.56789, )
    finally:
      sys.stdout = sys.__stdout__
...
class TestToolAuditLoggerFile:
  ...
  def test_multiple_events_appended (self, audit_logger):
    for i in range (5):
      audit_logger.log_invocation (tool_name = f"tool_{i}", tool_call_id = f"call_{i}", params = {"index": i}, )
    with open (audit_logger._audit_file) as f:
      lines = f.readlines ()
    assert len (lines) == 5
...
class TestToolAuditLoggerBoth:
  ...
  def test_log_invocation_writes_to_both (self, audit_logger):
    ...
    sys.stdout = captured
    try:
      audit_logger.log_invocation (tool_name = "search_documents", tool_call_id = "call_both", params = {"q": "test"}, )
    finally:
      sys.stdout = sys.__stdout__
...
class TestLogFromResult:
  ...
  def test_log_from_result_success (self):
    ...
    try:
      al = ToolAuditLogger (destination = AuditDestination.STDOUT)
      result = ToolResult (tool_name = "search_documents", tool_call_id = "call_1", content = "Found 3 results",
          duration_ms = 15.0, )
  ...
  def test_log_from_result_error (self):
    ...
    try:
      al = ToolAuditLogger (destination = AuditDestination.STDOUT)
      result = ToolResult (tool_name = "unknown_tool", tool_call_id = "call_err", error = "Tool not found", )

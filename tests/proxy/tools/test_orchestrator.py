...
from tools.definition import (
  RetryPolicy, ToolCall, ToolDefinition, ToolParam, ToolResult, ToolVisibility,
)
...
def _make_tool (
  ...
  return ToolDefinition (name = name, description = description, parameters = parameters, handler = handler,
      ...
      provider = provider, depends_on = depends_on or [], timeout_seconds = timeout_seconds,
...
class TestParallelExecutorExecuteSingle:
  ...
  async def test_execute_single_success (self):
    ...
    tool = _make_tool ("echo", handler = lambda **kw: f"Echo: {kw.get ('message')}", parameters = [], )
  ...
  async def test_execute_single_with_retry (self):
    ...
    tool = _make_tool ("retry_me", handler = retry_handler, parameters = [],
        retry_policy = RetryPolicy (max_retries = 3, backoff = "constant", initial_delay_seconds = 0.0,
                                    jitter = False), )
    registry = _make_registry ([tool])
    executor = ParallelExecutor (max_concurrency = 2)
  ...
  async def test_execute_single_with_retry_exhausted (self):
    ...
    tool = _make_tool ("always_fail", handler = always_fails, parameters = [],
        retry_policy = RetryPolicy (max_retries = 2, backoff = "constant", initial_delay_seconds = 0.0,
                                    jitter = False), )
    registry = _make_registry ([tool])
    executor = ParallelExecutor (max_concurrency = 2)
...
class TestParallelExecutorExecuteAll:
  ...
  async def test_execute_all_preserves_order (self):
    ...
    tools = [_make_tool (f"tool_{i}", handler = (lambda idx: lambda **kw: f"result_{idx}") (i), parameters = []) for i
        in range (5)]

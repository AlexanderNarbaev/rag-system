...
def tmp_yaml_dir (yaml_http_shell_content, yaml_malformed_tool, yaml_shell_no_whitelist):
  ...
  with tempfile.TemporaryDirectory () as tmpdir:
    ...
    (base / "tools.json").write_text (json.dumps ({
        "tools": [
            {
                "name": "json_tool", "type": "http", "description": "A JSON-defined tool",
...
class TestInterpolateVariables:
  def test_simple_env_var (self):
    ...
    result = _interpolate_variables ("Hello {{NAME}}", params = {}, env_vars = {"NAME": "World"}, context = {}, )
    assert result == "Hello World"

  def test_param_substitution (self):
    from tools.declarative import _interpolate_variables

    result = _interpolate_variables ("https://{{HOST}}/search?q={{query}}",
        params = {"query": "test", "max_results": 5}, env_vars = {"HOST": "example.com"}, context = {}, )
    assert result == "https://example.com/search?q=test"

  def test_context_substitution (self):
    from tools.declarative import _interpolate_variables

    result = _interpolate_variables ("User: {{CONTEXT.user_id}}", params = {}, env_vars = {},
  ...
  def test_missing_var_keeps_placeholder (self):
    ...
    result = _interpolate_variables ("Hello {{MISSING}}", params = {}, env_vars = {}, context = {}, )
...
class TestDeclarativeToolLoader:
  ...
  def test_shell_blocks_metacharacters (self):
    ...
    handler = _make_shell_handler (command = "df -h {{path}}", allowed_commands = ["df"], allowed_paths = ["/"], )
  ...
  async def test_http_handler_mock (self):
    ...
    handler = _make_http_handler (method = "GET", url_template = "https://{{HOST}}/search?q={{query}}",
        headers = {"Accept": "application/json"}, allowed_hosts = ["api.example.com"], )
...
class TestDeclarativeToolSchema:
  def test_validate_valid_http (self):
    ...
    valid = {
        "name": "test_api", "type": "http", "description": "A test API",

...
from tools.definition import (
  ToolDefinition, ToolParam, ToolResult, ToolVisibility,
)
...
def _make_tool (
    name, description = "Test tool", category = "general", tags = None, visibility = ToolVisibility.PUBLIC,
    handler = None, provider = "sdk", depends_on = None, parameters = None, async_handler = None, ):
  ...
  return ToolDefinition (name = name, description = description, parameters = parameters, handler = handler,
      async_handler = async_handler, category = category, tags = tags or [], visibility = visibility,
      provider = provider, depends_on = depends_on or [], )
...
class TestRegistryListTools:
  ...
  def setup_registry (self):
    ...
    self.registry.register (
        _make_tool (name = "public_tool", visibility = ToolVisibility.PUBLIC, category = "search", tags = ["fast"],
            provider = "sdk", ))
    self.registry.register (
        _make_tool (name = "admin_tool", visibility = ToolVisibility.ADMIN, category = "admin", tags = ["slow"],
            provider = "declarative", ))
    self.registry.register (
        _make_tool (name = "user_tool", visibility = ToolVisibility.USER, category = "search", tags = ["fast", "beta"],
            provider = "openapi", ))
  ...
  def test_list_tools_combined_filters (self):
    result = self.registry.list_tools (category = "search", tags = ["fast"], provider = "sdk", )
    assert len (result) == 1
    assert result [0].name == "public_tool"
...
class TestRegistryExecute:
  ...
  def test_execute_handler_exception (self):
    ...
    registry.register (_make_tool (name = "bad", parameters = [],  # no required params
        handler = bad_handler, ))
...
class TestRegistryExecuteAsync:
  ...
  async def test_execute_async_uses_async_handler (self):
    ...
    registry = EnhancedToolRegistry ()
    tool = _make_tool (name = "greet_async", parameters = [
        ToolParam (name = "name", type = str, description = "Name"),
    ], handler = lambda name: "sync fallback", async_handler = async_handler, )
    registry.register (tool)
...
class TestValidateTool:
  ...
  def test_duplicate_param_names (self):
    ...
    registry = EnhancedToolRegistry ()
    tool = ToolDefinition (name = "t", description = "desc", handler = lambda: None, parameters = [
        ToolParam (name = "q", type = str), ToolParam (name = "q", type = int),
    ], )
    issues = registry.validate_tool (tool)
...
class TestDiscoverFromProvider:
  def test_discover_registers_tools_from_provider (self):
    from tools.registry import (
      EnhancedToolRegistry, SDKProvider,
    )

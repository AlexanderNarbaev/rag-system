...
class TestLegacyMigration:
  ...
  async def test_migrate_from_env (self, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr ("proxy.app.auth.user_db.AUTH_VALID_USERS", json.dumps ({
...
class TestRowToDict:
  def test_parses_json_fields (self):
    row = (
        "id1", "alice", "hash", "alice@test.com", '["admin", "user"]', '["eng"]', "internal", "ns", 1, "2024-01-01",
        "2024-01-01",
    )
  ...
  def test_handles_invalid_json (self):
    row = (
        "id1", "alice", "hash", "", "not-json", "also-not-json", "internal", "", 1, "2024-01-01", "2024-01-01",
    )
    result = UserDatabase._row_to_dict (row)

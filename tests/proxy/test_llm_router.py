...
from proxy.app.llm.router import (
  LLMError, _send_completion_request, non_stream_completion, non_stream_completion_sync, stream_completion,
)
...
class TestSendCompletionRequest:
  ...
  async def test_non_stream_success (self):
    ...
    with patch ("aiohttp.ClientSession", return_value = mock_session):
      result = await _send_completion_request ([{"role": "user", "content": "hi"}], temperature = 0.2, max_tokens = 100,
  ...
  async def test_retry_eventually_succeeds (self):
    ...
    with patch ("aiohttp.ClientSession", return_value = mock_session), patch ("asyncio.sleep", AsyncMock ()):
      result = await _send_completion_request ([{"role": "user", "content": "hi"}], 0.2, 100, stream = False, retry = 1)

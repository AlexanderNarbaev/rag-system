...
from proxy.app import orchestrator
from proxy.app.orchestrator import _dynamic_top_k


class TestDynamicTopK:
    """Tests for the _dynamic_top_k function.

    Uses orchestrator.IntentType because orchestrator imports from app.slm_router
    which is a separate module from proxy.app.slm_router, so IntentType values
    from proxy.app.slm_router are different objects than those used in the

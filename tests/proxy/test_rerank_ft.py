...
from proxy.app.core.rerank import (
  RERANKER_FT_ENABLED, collect_training_pairs, fine_tune_reranker,
)
...
class TestCollectTrainingPairs:
  ...
  def test_returns_list_of_tuples (self, tmp_path):
    ...
    feedback_dir.mkdir ()
    feedback_entry = {
        "query": "How to configure nginx?", "chunks": [
            {"text": "Nginx config guide", "id": "c1"}, {"text": "Docker setup", "id": "c2"},
        ], "positive_chunk_ids": ["c1"], "negative_chunk_ids": ["c2"],
    }
  ...
  def test_requires_minimum_positives (self, tmp_path):
    ...
    for i in range (3):
      entry = {
          "query": f"test query {i}", "chunks": [{"text": f"chunk {i}", "id": f"c{i}"}],
          "positive_chunk_ids": [f"c{i}"], "negative_chunk_ids": [],
      }

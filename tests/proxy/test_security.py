...
from proxy.app.shared.security import (
  DependencyScanner, InputValidator, SecretsManager, SecurityHeaders,
)
...
class TestInputValidator:
  ...
  def test_sanitize_headers_redacts_sensitive (self):
    headers = {
        "content-type": "application/json", "authorization": "Bearer secret-token-12345", "x-api-key": "sk-abcdef",
    }
...
class TestSecretsManager:
  ...
  def test_mask_in_response_nested (self):
    data = {
        "user": {"name": "Alice", "password": "hunter2"}, "auth": {"token": "abc", "public": "visible"},
    }
    result = SecretsManager.mask_in_response (data)

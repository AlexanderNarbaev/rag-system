...
class TestRemoteEmbeddingClient:
  ...
  def test_encode_multiple_texts (self):
    ...
    mock_resp.read.return_value = json.dumps ({
        "data": [
            {"embedding": [0.1, 0.2, 0.3], "index": 0}, {"embedding": [0.4, 0.5, 0.6], "index": 1},
        ]
    }).encode ("utf-8")
    mock_resp.__enter__ = MagicMock (return_value = mock_resp)
    mock_resp.__exit__ = MagicMock (return_value = False)
  ...
  def test_check_health_includes_auth_header (self):
    ...
    with patch.object (urllib.request, "urlopen", side_effect = mock_urlopen):
      client = RemoteEmbeddingClient (endpoint = "http://localhost:8080", api_key = "my-secret-api-key", )
      result = client._check_health ()
      assert result is True
...
class TestRemoteRerankerClient:
  ...
  def test_predict_success (self):
    ...
    mock_resp.read.return_value = json.dumps ({
        "results": [
            {"index": 0, "relevance_score": 0.9}, {"index": 1, "relevance_score": 0.3},
        ]
    }).encode ("utf-8")
    mock_resp.__enter__ = MagicMock (return_value = mock_resp)
    mock_resp.__exit__ = MagicMock (return_value = False)
  ...
  def test_check_health_includes_auth_header (self):
    ...
    with patch.object (urllib.request, "urlopen", side_effect = mock_urlopen):
      client = RemoteRerankerClient (endpoint = "http://localhost:8080", api_key = "reranker-secret", )
      result = client._check_health ()
      assert result is True
      assert len (captured_requests) >= 1
      auth_header = captured_requests [0].get_header ("Authorization")
  ...
  def test_predict_multiple_queries (self):
    ...
    def mock_urlopen (req, timeout = 60):
      mock_resp = MagicMock ()
      mock_resp.read.return_value = json.dumps ({"results": [{"index": 0, "relevance_score": 0.8}]}).encode ("utf-8")
      mock_resp.__enter__ = MagicMock (return_value = mock_resp)
      mock_resp.__exit__ = MagicMock (return_value = False)
      call_count [0] += 1
      return mock_resp

...
class TestAuditEvent:
  ...
  def test_create_basic_event (self):
    event = AuditEvent (event_id = "evt_123", timestamp = "2025-01-01T00:00:00Z", event_type = "query",
        user_id = "user1", client_ip = "127.0.0.1", endpoint = "/v1/chat/completions", request_hash = "abc123",
  ...
  def test_to_dict_excludes_none (self):
    event = AuditEvent (event_id = "evt_1", timestamp = "2025-01-01T00:00:00Z", event_type = "query", user_id = None,
        client_ip = "10.0.0.1", endpoint = "/v1/test", request_hash = "hash1", details = {"key": "val"}, )
    d = event.to_dict ()
  ...
  def test_to_json_serializable (self):
    event = AuditEvent (event_id = "evt_json", timestamp = "2025-01-01T00:00:00Z", event_type = "access_denied",
        user_id = "user2", client_ip = "192.168.1.1", endpoint = "/admin", request_hash = "n/a",
  ...
  def test_fields_have_correct_types (self):
    event = AuditEvent (event_id = "evt_types", timestamp = "2025-06-22T10:00:00Z", event_type = "error",
        user_id = "u1", client_ip = "1.2.3.4", endpoint = "/error", request_hash = "hash_err", duration_ms = 42.5,
        tokens_used = 100, result_status = "error", )
...
class TestAuditLogger:
  ...
  def test_log_query_writes_event (self, audit_logger):
    audit_logger.log_query (user_id = "user1", query = "What is RAG?", response_preview = "RAG is a technique.",
        chunks = 5, duration_ms = 100.0, tokens = 50, client_ip = "10.0.0.1", )
  ...
  def test_log_access_denied (self, audit_logger):
    audit_logger.log_access_denied (user_id = "user2", resource = "/admin/config", reason = "insufficient_permissions",
        client_ip = "10.0.0.2", )
    with open (audit_logger._audit_file) as f:
      lines = f.readlines ()
    assert len (lines) == 1
  ...
  def test_log_config_change (self, audit_logger):
    audit_logger.log_config_change (user_id = "admin", key = "MAX_CHUNKS_RETRIEVAL", old_value = "50",
        new_value = "100", client_ip = "10.0.0.1", )
    with open (audit_logger._audit_file) as f:
      lines = f.readlines ()
    assert len (lines) == 1
    event = json.loads (lines [0])
  ...
  def test_log_error (self, audit_logger):
    audit_logger.log_error (error_type = "ValueError", error_msg = "Invalid input parameter",
        stack_trace = "Traceback line 1\nTraceback line 2", context = {"user_id": "user3", "endpoint": "/api"},
        client_ip = "10.0.0.3", endpoint = "/v1/chat/completions", )
    with open (audit_logger._audit_file) as f:
      lines = f.readlines ()
    assert len (lines) == 1
    event = json.loads (lines [0])
  ...
  def test_log_auth (self, audit_logger):
    audit_logger.log_auth (user_id = "user4", action = "login", success = True, details = {"method": "api_key"},
        client_ip = "10.0.0.4", )
    with open (audit_logger._audit_file) as f:
      lines = f.readlines ()
    assert len (lines) == 1
  ...
  def test_log_auth_failure (self, audit_logger):
    audit_logger.log_auth (user_id = None, action = "login", success = False,
        details = {"reason": "invalid_credentials"}, client_ip = "10.0.0.5", )
    with open (audit_logger._audit_file) as f:
      lines = f.readlines ()
    assert len (lines) == 1
    event = json.loads (lines [0])
  ...
  def test_multiple_events_appended (self, audit_logger):
    for i in range (5):
      audit_logger.log_query (user_id = f"user_{i}", query = f"query {i}", response_preview = f"response {i}",
          chunks = i, duration_ms = i * 10.0, tokens = i * 5, )
    with open (audit_logger._audit_file) as f:
      lines = f.readlines ()
    assert len (lines) == 5
  
  def test_query_history_filter_by_user (self, audit_logger):
    audit_logger.log_query (user_id = "alice", query = "q1", response_preview = "r1", chunks = 1, duration_ms = 10,
                            tokens = 5)
    audit_logger.log_query (user_id = "bob", query = "q2", response_preview = "r2", chunks = 2, duration_ms = 20,
                            tokens = 10)
    audit_logger.log_query (user_id = "alice", query = "q3", response_preview = "r3", chunks = 3, duration_ms = 30,
                            tokens = 15)
  ...
  def test_query_history_limit (self, audit_logger):
    for i in range (10):
      audit_logger.log_query (user_id = "u1", query = f"q{i}", response_preview = "r", chunks = 1, duration_ms = 10,
          tokens = 1)
  ...
  def test_query_history_start_time_filter (self, audit_logger):
    ...
    time.sleep (0.01)
    audit_logger.log_query (user_id = "u1", query = "after", response_preview = "r", chunks = 1, duration_ms = 10,
                            tokens = 1)
    time.sleep (0.01)
    t2 = datetime.datetime.now (datetime.UTC).isoformat ()
    audit_logger.log_query (user_id = "u1", query = "even_later", response_preview = "r", chunks = 1, duration_ms = 10,
        tokens = 1)
  ...
  def test_export_report (self, audit_logger):
    ...
    audit_logger.log_query (user_id = "u1", query = "q1", response_preview = "r1", chunks = 2, duration_ms = 50,
                            tokens = 100)

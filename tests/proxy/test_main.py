...
_modules_to_mock = [
    ...
    "langgraph.graph", "langgraph.checkpoint", "neo4j", "redis", "redis.asyncio", "tiktoken", "bcrypt",
]
...
def mock_rag_pipeline ():
  ...
  with (
    patch ("proxy.app.main.hybrid_search") as mock_hybrid, patch (
      ...
      "proxy.app.main.build_context") as mock_build, patch (
      "proxy.app.main.non_stream_completion") as mock_nonstream, patch (
      "proxy.app.main.stream_completion") as mock_stream, patch ("proxy.app.main.extract_version_from_query",
                                                                 return_value = None), patch (
      "proxy.app.main.cache_manager", None), patch ("proxy.app.main.log_interaction") as mock_log, ):
    ...
    yield {
        "hybrid_search": mock_hybrid, "rerank_chunks": mock_rerank, "deduplicate_chunks": mock_dedup,
        "build_context": mock_build, "non_stream_completion": mock_nonstream, "stream_completion": mock_stream,
...
class TestChatCompletionsNonStreaming:
  ...
  def test_rag_skip_generation_returns_chunks_only (self, client, mock_rag_pipeline):
    ...
    mock_rag_pipeline ["hybrid_search"].return_value = [
        MagicMock (payload = chunk_a, score = 0.95), MagicMock (payload = chunk_b, score = 0.85),
    ]
...
class TestProcessRagQuery:
  ...
  async def test_no_search_results (self):
    ...
    with (
      patch ("proxy.app.main.cache_manager", None), patch ("proxy.app.main.hybrid_search", return_value = []), patch (
        "proxy.app.main.non_stream_completion", return_value = "Answer from LLM"), ):
      result, context, from_cache, sources, ragas_scores = await process_rag_query (user_query = "test",
          stream = False, )
  ...
  async def test_streaming_returns_context_and_messages (self):
    ...
    with (
      patch ("proxy.app.main.cache_manager", None), patch ("proxy.app.main.hybrid_search", mock_search), patch (
        "proxy.app.main.rerank_chunks", return_value = [0]), patch (
        "proxy.app.main.deduplicate_chunks") as mock_dedup, patch ("proxy.app.main.build_context",
                                                                   return_value = "Built context"), ):
      mock_dedup.return_value = [(mock_chunk, 0.95), (mock_chunk, 0.90)]
      context, messages, _, sources, _ = await process_rag_query (user_query = "test", stream = True, )
...
class TestToolsEndpoint:
  ...
  def sample_tools (self):
    ...
    search_tool = ToolDefinition (name = "search_documents",
        ...
        ], category = "search", tags = ["hybrid", "live"], version = "1.0.0", visibility = ToolVisibility.PUBLIC,
        timeout_seconds = 30.0, provider = "sdk", depends_on = [], )
    admin_tool = ToolDefinition (name = "admin_reindex", description = "Trigger full reindex of knowledge base",
        parameters = [
            ToolParam (name = "collection", type = str, description = "Collection name"),
        ], category = "admin", tags = ["maintenance"], version = "1.0.0", visibility = ToolVisibility.ADMIN,
        timeout_seconds = 300.0, provider = "sdk", depends_on = ["search_documents"], )
    expert_tool = ToolDefinition (name = "review_feedback", description = "Review and approve expert feedback",
        parameters = [
            ToolParam (name = "feedback_id", type = str, description = "Feedback ID"),
        ], category = "review", tags = ["expert", "live"], version = "1.0.0", visibility = ToolVisibility.EXPERT,
        timeout_seconds = 60.0, provider = "declarative", depends_on = [], )
    user_tool = ToolDefinition (name = "save_bookmark", description = "Save a document bookmark for later",
        parameters = [
  ...
  def test_list_tools_filter_by_tag (self, client, mock_registry):
    ...
    with (
      patch ("proxy.app.main.get_enhanced_registry", return_value = mock_registry), patch (
        "proxy.app.main._highest_role_from_user", return_value = "admin"), ):
  ...
  def test_tool_list_filtered_by_role (self, client, mock_registry):
    ...
    search_tool = ToolDefinition (name = "search_documents", description = "Search",
        ...
        visibility = ToolVisibility.PUBLIC, provider = "sdk", )
    expert_tool = ToolDefinition (name = "review_feedback", description = "Review",
        parameters = [ToolParam (name = "feedback_id", type = str)], category = "review", tags = ["expert"],
        visibility = ToolVisibility.EXPERT, provider = "sdk", )
    user_tool = ToolDefinition (name = "save_bookmark", description = "Bookmark",
        parameters = [ToolParam (name = "doc_id", type = str)], category = "bookmarks", tags = ["user"],
        visibility = ToolVisibility.USER, provider = "sdk", )
    admin_tool = ToolDefinition (name = "admin_reindex", description = "Reindex",
        parameters = [ToolParam (name = "collection", type = str)], category = "admin", tags = ["maintenance"],

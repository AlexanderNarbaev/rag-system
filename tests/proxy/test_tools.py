...
class TestToolDefinition:
  def test_creates_with_required_fields (self):
    ...
    td = ToolDefinition (name = "search_docs", description = "Search documents", parameters_schema = {
  ...
  def test_creates_with_optional_fields (self):
    ...
    td = ToolDefinition (name = "test", description = "Test tool", parameters_schema = {}, handler = lambda: None,
        category = "search", is_async = False, )
...
class TestToolRegistry:
  ...
  def sample_tool (self):
    ...
    return ToolDefinition (name = "greet", description = "Greet someone",
  ...
  def test_register_duplicate_overwrites (self, registry, sample_tool):
    ...
    registry.register (sample_tool)
    new_tool = ToolDefinition (name = "greet", description = "Updated", parameters_schema = {},
        handler = lambda: "new", )
...
class TestExecuteTool:
  ...
  def test_execute_sync_tool (self, registry):
    ...
    registry.register (ToolDefinition (name = "add", description = "Add two numbers", parameters_schema = {
  ...
  def test_execute_tool_with_kwargs (self, registry):
    ...
    registry.register (
        ToolDefinition (name = "search", description = "Search", parameters_schema = {}, handler = search, ))
  ...
  def test_execute_tool_handler_raises (self, registry):
    ...
    registry.register (
        ToolDefinition (name = "failing", description = "Always fails", parameters_schema = {}, handler = failing, ))
...
class TestHandleFunctionCall:
  ...
  def registry (self):
    ...
    reg = ToolRegistry ()
    reg.register (ToolDefinition (name = "echo", description = "Echoes input", parameters_schema = {},
  ...
  def test_handle_valid_function_call (self, registry):
    ...
    call = {
        "id": "call_001", "function": {"name": "echo", "arguments": '{"text": "hello"}'},
    }
  ...
  def test_handle_function_call_tool_not_found (self, registry):
    ...
    call = {
        "id": "call_002", "function": {"name": "nonexistent", "arguments": "{}"},
    }
    result = handle_function_call (call, registry)
    assert result.error is not None
    assert "not found" in result.error.lower ()
  ...
  def test_handle_function_call_bad_json_arguments (self, registry):
    ...
    call = {
        "id": "call_004", "function": {"name": "echo", "arguments": "not json"},
    }
    result = handle_function_call (call, registry)
    assert result.error is not None

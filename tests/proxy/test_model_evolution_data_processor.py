            ...
            entries = [
                {
                    "query": "What is X?",
                    "answer": "Wrong answer",
                    "correction": "Correct answer about X",
                },
            ]
            log_path = self._write_hitl_log(tmpdir, entries)
            output_dir = os.path.join(tmpdir, "output")
    ...
    def test_generates_reranker_data_from_chunks(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            entries = [
                {
                    "query": "How to deploy?",
                    "chunks": [{"text": "Use k8s for deployment."}, "Plain text chunk"],
                    "relevance": 0.95,
                },
            ]
            log_path = self._write_hitl_log(tmpdir, entries)
...
class TestFormatForSLM:
    def test_formats_query_and_intent(self):
        processor = DataProcessor()
        data = [
            {"query": "How to deploy?", "intent": "deployment"},
            {"query": "What is RAG?", "intent": "definition"},
        ]

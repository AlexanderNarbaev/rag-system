...
from proxy.app.core.hallucination import (
  HallucinationReport, check_claim_against_context, compute_hallucination_rate, detect_hallucinations,
  extract_factual_claims, run_hallucination_benchmark,
)
...
class TestExtractFactualClaims:
  ...
  def test_extract_simple_claims (self):
    claims = extract_factual_claims ("Python was created by Guido van Rossum. It is widely used for web development.")
    assert len (claims) >= 1
  ...
  def test_extract_numeric_claims (self):
    claims = extract_factual_claims ("The server has 64 GB of RAM. It runs at 3.5 GHz. The temperature is 72 degrees.")
    assert len (claims) >= 2
...
class TestCheckClaimAgainstContext:
  ...
  def test_direct_match (self):
    result = check_claim_against_context ("Python was created by Guido van Rossum",
        "Python is a programming language created by Guido van Rossum in 1991.", )
    assert result is True
  
  def test_no_match (self):
    result = check_claim_against_context ("The moon landing was faked by NASA in a Hollywood studio",
        "Python is a programming language created by Guido van Rossum in 1991.", )
  ...
  def test_partial_match (self):
    result = check_claim_against_context ("Docker uses OS-level virtualization for containers",
...
class TestDetectHallucinations:
  ...
  def test_no_hallucinations_when_all_supported (self):
    report = detect_hallucinations (answer = "Python was created by Guido van Rossum. It is used for web development.",
  ...
  def test_report_fields_populated (self):
    report = detect_hallucinations (answer = "Docker is a container platform. It was created by Solomon Hykes.",
  ...
  def test_supported_claims_in_evidence_links (self):
    report = detect_hallucinations (answer = "Python is a programming language.",
...
class TestComputeHallucinationRate:
  ...
  def test_mixed_answers (self):
    answers = [
        "Python is a language.", "Java was created by Dennis Ritchie.",
    ]
    contexts = [
        "Python is a programming language.", "Java was created by James Gosling.",
    ]
    rate = compute_hallucination_rate (answers, contexts)
    assert 0.0 <= rate <= 1.0
...
class TestRunHallucinationBenchmark:
  ...
  def test_benchmark_target_below_5_percent (self):
    dataset = [{
        "query": f"Query {i}", "answer": f"Answer {i} is correct and grounded in the provided context.",
...
class TestHallucinationReport:
  ...
  def test_report_fields (self):
    report = HallucinationReport (hallucination_rate = 0.05, hallucinated_claims = ["Fake claim"],
        supported_claims = 19, total_claims = 20, evidence_links = {"claim1": "context sentence 1"}, )

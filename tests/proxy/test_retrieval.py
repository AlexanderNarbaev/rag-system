...
from proxy.app.core.retrieval import (
  _compute_dense_embedding, check_qdrant_health, graph_expand_query, hybrid_search, initialize_retrieval,
...
class TestReciprocalRankFusion:
  ...
  def test_merges_two_lists (self):
    dense = [
        _make_mock_scored_point ("a", 0.9), _make_mock_scored_point ("b", 0.8),
    ]
    sparse = [
        _make_mock_scored_point ("c", 0.7), _make_mock_scored_point ("a", 0.5),
    ]
...
class TestHybridSearch:
  ...
  def test_without_sparse (self):
    ...
    with (
      patch ("proxy.app.core.retrieval.qdrant_client") as mock_qdrant, patch (
        "proxy.app.core.retrieval.embedder"), patch ("proxy.app.core.retrieval._compute_dense_embedding",
                                                     return_value = [0.1, 0.2]), patch (
        "proxy.app.core.retrieval._compute_sparse_embedding", return_value = None), ):
  ...
  def test_with_sparse (self):
    ...
    with (
      patch ("proxy.app.core.retrieval.qdrant_client") as mock_qdrant, patch (
        "proxy.app.core.retrieval.embedder"), patch ("proxy.app.core.retrieval._compute_dense_embedding",
                                                     return_value = [0.1, 0.2]), patch (
        "proxy.app.core.retrieval._compute_sparse_embedding", return_value = MagicMock ()), ):
  ...
  def test_auto_init_when_none (self):
    with (
      patch ("proxy.app.core.retrieval.qdrant_client", None), patch ("proxy.app.core.retrieval.embedder", None), patch (
        "proxy.app.core.retrieval.initialize_retrieval") as mock_init, patch (
        "proxy.app.core.retrieval._compute_dense_embedding", return_value = [0.1]), patch (
        "proxy.app.core.retrieval._compute_sparse_embedding", return_value = None), ):
...
class TestComputeDenseEmbedding:
  ...
  def test_without_cache (self):
    with (
      patch ("proxy.app.core.retrieval.embedder") as mock_embedder, patch ("proxy.app.core.retrieval.cache_manager",
                                                                           None), ):
...
class TestGraphExpandQuery:
  ...
  def test_graph_no_driver (self):
    with (
      patch ("proxy.app.core.retrieval._GRAPH_ENABLED", True), patch ("proxy.app.core.retrieval.neo4j_driver", None), ):
      result = graph_expand_query ("some query")
      assert result == ""
  ...
  def test_graph_enabled_with_mock (self):
    ...
    with (
      patch ("proxy.app.core.retrieval._GRAPH_ENABLED", True), patch ("proxy.app.core.retrieval.neo4j_driver",
                                                                      mock_driver), ):
...
class TestInitializeRetrieval:
  ...
  def test_raises_if_st_not_available (self):
    with (
      patch ("proxy.app.core.retrieval.QDRANT_AVAILABLE", True), patch ("proxy.app.llm.remote_services.create_embedder",
  ...
  def test_initializes_in_memory_cache (self):
    ...
    with (
      patch ("proxy.app.core.retrieval.QDRANT_AVAILABLE", True), patch ("proxy.app.llm.remote_services.create_embedder",
                                                                        return_value = mock_embedder), patch (
        "proxy.app.core.retrieval.QdrantClient"), patch ("proxy.app.core.retrieval.USE_REDIS", False), patch (
        "proxy.app.core.retrieval._GRAPH_ENABLED", False), ):
      initialize_retrieval ()
  ...
  def test_graph_enabled_with_failure (self):
    ...
    with (
      patch ("proxy.app.core.retrieval.QDRANT_AVAILABLE", True), patch ("proxy.app.llm.remote_services.create_embedder",
                                                                        return_value = mock_embedder), patch (
        "proxy.app.core.retrieval.QdrantClient"), patch ("proxy.app.core.retrieval.USE_REDIS", False), patch (
        "proxy.app.core.retrieval._GRAPH_ENABLED", True), patch.dict ("sys.modules", {"neo4j": mock_neo4j}), ):
      initialize_retrieval ()
      import proxy.app.core.retrieval as ret_mod

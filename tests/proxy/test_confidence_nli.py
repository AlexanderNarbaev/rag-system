...
class TestComputeNliGrounding:
    ...
    def test_fully_supported_answer(self):
        context = (
            "Docker is a containerization platform. It was released in 2013. "
...
class TestConfidenceWithNli:
    ...
    def test_good_answer_high_confidence(self):
        context = (
            "GitLab CI/CD uses .gitlab-ci.yml for pipeline configuration. "
            "Pipelines consist of stages and jobs. Jobs run on GitLab runners."
        )
        answer = (
            "GitLab CI/CD pipelines are configured with .gitlab-ci.yml files. "
            "Each pipeline contains stages and jobs that execute on GitLab runners."
        )
        report = compute_confidence(
            query="How does GitLab CI/CD work?",
            context=context,
            answer=answer,
            slm_available=False,
        )

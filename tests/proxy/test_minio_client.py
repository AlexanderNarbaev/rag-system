...
def minio_client (mock_boto3_client):
  ...
  with patch ("proxy.app.shared.minio_client.boto3") as mock_boto:
    ...
    c = MinioClient (endpoint = "localhost:9000", access_key = "testkey", secret_key = "testsecret",
        bucket = "test-bucket", secure = False, )
    ...
    paginator_mock.paginate.return_value = [
        {
            "Contents": [
                {
                    "Key": "file1.txt", "Size": 100, "LastModified": datetime (2025, 1, 1, tzinfo = UTC),
                    "ETag": '"abc123"',
                }
            ]
        }
...
class TestGetFileMetadata:
  def test_metadata_success (self, minio_client, mock_boto3_client):
    ...
    mock_boto3_client.head_object.return_value = {
        "ContentLength": 42, "LastModified": datetime (2025, 1, 1, tzinfo = UTC), "ContentType": "text/plain",
        "Metadata": {"author": "test"}, "ETag": '"etag123"',
    }
...
class TestHealthCheck:
  ...
  def test_health_check_failure (self, minio_client, mock_boto3_client):
    ...
    mock_boto3_client.list_buckets.side_effect = EndpointConnectionError (endpoint_url = "http://localhost:9000",

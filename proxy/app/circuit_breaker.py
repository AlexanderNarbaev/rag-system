...
circuit_breaker_state = _register_metric(
    Gauge,
...
circuit_breaker_failures_total = _register_metric(
    Counter,
    "circuit_breaker_failures_total",
    "Total number of failures per circuit breaker",
    ["name"],
)
...
class CircuitBreaker:
    ...
    def __init__(
        ...
        logger.info(
            "Circuit breaker '%s' initialized: threshold=%d, cooldown=%.1fs, half_open_max=%d",
            self.name,
    ...
    def _maybe_transition(self) -> None:
        ...
        if self._state == State.OPEN:
            ...
            if elapsed >= self.cooldown_seconds:
                ...
                circuit_breaker_state.labels(name=self.name).set(self._state.metric_value)
                logger.info(
                    "Circuit breaker '%s': %s → %s (cooldown %.1fs elapsed)",
                    self.name,
                    old_state,
                    self._state,
                    elapsed,

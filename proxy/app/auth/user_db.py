...
class UserDatabase:
  ...
  async def get_user_by_username (self, username: str) -> dict [str, Any] | None:
    ...
    await self._ensure_db ()
    cursor = await self._conn.execute ("SELECT * FROM users WHERE username = ? COLLATE NOCASE AND is_active = 1",
        (username,), )
    row = await cursor.fetchone ()
    return self._row_to_dict (row) if row else None
  ...
  async def delete_user (self, user_id: str) -> bool:
    ...
    await self._ensure_db ()
    cursor = await self._conn.execute ("UPDATE users SET is_active = 0, updated_at = ? WHERE id = ?",
  ...
  async def list_users (self, limit: int = 100, offset: int = 0) -> list [dict [str, Any]]:
    """List active users."""
    await self._ensure_db ()
    cursor = await self._conn.execute ("SELECT * FROM users WHERE is_active = 1 ORDER BY username LIMIT ? OFFSET ?",
        (limit, offset), )
  ...
  async def revoke_user_tokens (self, user_id: str) -> int:
    ...
    await self._ensure_db ()
    cursor = await self._conn.execute ("UPDATE refresh_tokens SET revoked = 1 WHERE user_id = ? AND revoked = 0",
        (user_id,), )
    await self._conn.commit ()
  ...
  def _row_to_dict (row: Any) -> dict [str, Any]:
    ...
    columns = [
        "id", "username", "password_hash", "email", "roles", "groups", "access_level", "namespace", "is_active",

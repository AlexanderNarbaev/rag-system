...
class SecretRotationManager:
    ...
    def __init__(
        self,
        jwt_grace_seconds: int = DEFAULT_JWT_GRACE_SECONDS,
        api_key_overlap_seconds: int = DEFAULT_API_KEY_OVERLAP_SECONDS,
    ) -> None:
    ...
    async def _ensure_initialized(self) -> None:
        ...
        logger.info(
            "SecretRotationManager initialized (grace=%ds, overlap=%ds)",
            self._jwt_grace_seconds,
            self._api_key_overlap_seconds,
        )
    ...
    async def rotate_jwt_keys(
        ...
        record = RotationRecord(
            ...
            secret_type=SecretType.JWT_SIGNING_KEY.value,
    ...
    async def rotate_api_keys(
        ...
        try:
            ...
            record.completed_at = datetime.now(UTC).isoformat()
            record.details = {
                "users_rotated": len(users_rotated),
                "keys_generated": len(new_keys),
                "overlap_seconds": overlap,
                "overlap_end": (datetime.now(UTC) + timedelta(seconds=overlap)).isoformat(),
            }
    ...
    def get_rotation_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "grace_period_seconds": self._jwt_grace_seconds,
        }
    ...
    def _generate_jwt_key_pair(algorithm: str = "RS256") -> JWTKeyPair:
        ...
        try:
            ...
            return JWTKeyPair(
                key_id=key_id,
                algorithm="HS256",
                private_key_pem=random_secret,
                public_key_pem=random_secret,
                created_at=now,
            )
    ...
    def _log_rotation_audit(record: RotationRecord) -> None:
        ...
        try:
            ...
            audit.log_config_change(
                ...
                key=f"secret_rotation.{record.secret_type}",

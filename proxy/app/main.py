    ...
    for sig in (signal.SIGTERM, signal.SIGINT):
      try:
        loop.add_signal_handler (sig,
                                 lambda s = sig: asyncio.create_task (_initiate_shutdown (s)))  # type: ignore[misc]
...
async def process_rag_query (
    user_query: str, version: str | None = None, force_refresh: bool = False, temperature: float = 0.2,
    max_tokens: int = 4096, stream: bool = False, other_messages: list [dict [str, Any]] | None = None,
    user_context: UserContext | None = None, top_k_override: int | None = None, ) -> tuple [
  str, str | list [dict [str, str]], bool, list [dict [str, Any]], dict [str, float]]:
  """
  ...
  if not search_results:
    ...
    if not filtered_chunks:
      ...
      if action == "REWRITE":
        ...
        try:
          additional_results = hybrid_search (query = rewritten_query, version = version,
              top_k = top_k_override or MAX_CHUNKS_RETRIEVAL, )
        ...
        # Try broader search
        try:
          expanded_results = hybrid_search (query = user_query, version = None,  # Remove version filter
...
app = FastAPI (title = "RAG Proxy for Gemma",
    description = ("OpenAI-compatible proxy with hybrid search, reranking, and Gemma LLM.\n\n"
                   ...
                   "- Response extensions: `rag_feedback_id`, `rag_confidence`, `rag_sources`"), version = "2.0.0",
    lifespan = lifespan, openapi_tags = [

...
from proxy.app.shared.cache import CacheManager, SemanticCache
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    """
    global cache_manager, semantic_cache, orchestrator, audit_logger, kb_manager
    setup_logging()
    ...
    if SEMANTIC_CACHE_ENABLED:
        ...
        logger.info(
            "Semantic cache enabled (threshold=%.2f, ttl=%ds)",
            SEMANTIC_CACHE_SIMILARITY,
            SEMANTIC_CACHE_TTL,
        )
    else:
        semantic_cache = None
        logger.info("Semantic cache disabled (set SEMANTIC_CACHE_ENABLED=true)")
...
async def process_rag_query(
    ...
    if not force_refresh and semantic_cache:
        sem_cached = await semantic_cache.get(user_query)
        if sem_cached:
            logger.info(f"Semantic cache hit for query: {user_query[:50]}...")
            return sem_cached, "", True, [], {}
    ...
    # 3. Adaptive query routing (opt-in via config)
    ...
    # Always allow LLM generation — model can use its own training knowledge.
    # When retrieval is insufficient, the system prompt includes a notice
    # that the answer is not based on the knowledge base.
    generate_without_knowledge = not should_gen
    if generate_without_knowledge:
        logger.info(
            "Knowledge insufficient (%s) — allowing ungrounded generation with notice",
            reason,
        )
    ...
    if semantic_cache and not force_refresh:
        await semantic_cache.set(user_query, response_text)
    ...
    if chunks_for_eval:
        ...
        # Lightweight NLI-based RAGAS metrics for Prometheus
        try:
            from proxy.app.core.ragas_metrics import compute_all_ragas_metrics

            lw_scores = compute_all_ragas_metrics(
                question=user_query,
                answer=response_text,
                contexts=context_texts,
            )
            ...
            rag_ragas_faithfulness.set(lw_scores.get("ragas_faithfulness", 0.0))
            rag_ragas_relevancy.set(lw_scores.get("ragas_relevancy", 0.0))
            rag_ragas_precision.set(lw_scores.get("ragas_precision", 0.0))
            logger.debug(f"Lightweight RAGAS metrics: {lw_scores}")
        except Exception:
            logger.debug("Lightweight RAGAS metrics computation failed", exc_info=True)

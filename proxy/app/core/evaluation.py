...
_CROSS_LINGUAL_SAMPLE_QUERIES: dict [str, list [str]] = {
    "en": [
        "How do I set up a CI/CD pipeline?", "What is Retrieval-Augmented Generation?",
        "How to configure Jira automation rules?", "Explain the hybrid search architecture",
        "What are the best practices for Qdrant collections?",
    ], "de": [
        "Wie richte ich eine CI/CD-Pipeline ein?", "Was ist Retrieval-Augmented Generation?",
        "Wie konfiguriere ich Jira-Automatisierungsregeln?", "Erklären Sie die hybride Sucharchitektur",
        "Was sind die besten Praktiken für Qdrant-Sammlungen?",
    ], "fr": [
        "Comment configurer un pipeline CI/CD?", "Qu'est-ce que la génération augmentée par récupération?",
        "Comment configurer les règles d'automatisation Jira?", "Expliquez l'architecture de recherche hybride",
        "Quelles sont les meilleures pratiques pour les collections Qdrant?",
    ], "zh": [
        "如何设置CI/CD管道？", "什么是检索增强生成？", "如何配置Jira自动化规则？", "解释混合搜索架构",
        "Qdrant集合的最佳实践是什么？",
    ],
}
...
_RELEVANT_DOC_IDS: dict [str, list [set [str]]] = {
    "en": [
        {"cicd_setup"}, {"rag_overview"}, {"jira_proj123"}, {"hybrid", "search"}, {"qdrant", "collections"},
    ], "de": [
        {"cicd_setup"}, {"rag_overview"}, {"jira_proj123"}, {"hybrid", "search"}, {"qdrant", "collections"},
    ], "fr": [
        {"cicd_setup"}, {"rag_overview"}, {"jira_proj123"}, {"hybrid", "search"}, {"qdrant", "collections"},
...
def evaluate_cross_lingual_retrieval (
    lang_pair: tuple [str, str], queries: dict [str, list [str]] | None = None, ) -> dict [str, Any]:
  ...
  return {
      ...
      "cross_lingual": {k: v for k, v in cross_metrics.items () if k != "num_queries"}, "comparison": comparison,
      "num_queries": len (q_source),
  }
...
def run_cross_lingual_benchmark () -> list [dict [str, Any]]:
  ...
  for pair in pairs:
    try:
      ...
      results.append ({
          "lang_pair": f"{pair [0]}->{pair [1]}", "error": str (e), "monolingual": {}, "cross_lingual": {},
      })
  return results

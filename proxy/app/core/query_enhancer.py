...
class QueryEnhancer:
  ...
  QUERY_PREFIXES = [
      "Passage: ", "Answer: ", "The document states that ",
  ]
  
  METADATA_PATTERNS = {
      "version": [
          r"version\s*[:=]?\s*(\S+)", r"v(?:ersion)?\.?\s*([\d.]+)", r"release\s+(\S+)",
      ], "date": [
          r"(?:from|since|after|before)\s+(\d{4}-\d{2}-\d{2})", r"date\s*[:=]?\s*(\d{4}-\d{2}-\d{2})",
          r"(\d{4}-\d{2}-\d{2})",
      ], "author": [
          r"(?:by|from|author)\s+['\"]?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)['\"]?",
          r"written\s+by\s+['\"]?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)['\"]?",
      ], "type": [
          r"(?:type|kind)\s*[:=]?\s*(\w+)", r"(specification|guide|tutorial|reference|manual)",
      ], "project": [
          r"project\s*[:=]?\s*['\"]?(\w+)['\"]?", r"in\s+(?:the\s+)?['\"]?(\w+)\s+project['\"]?",
      ],
  }
  ...
  def hyde_enhance (self, query: str) -> str:
    ...
    hyde_prefixes = [
        "Here is the answer to the query:", "The document that answers this question contains:",
  ...
  def decompose_complex_query (self, query: str) -> list [str]:
    ...
    delimiters = [
        r"\band\b(?! so| then| also)", r";", r"\.(?=\s*[A-Z])", r"\balso\b", r"\bplus\b",
    ]
    sub_queries = [query]
  ...
  def enhance (self, query: str) -> dict [str, Any]:
    ...
    return {
        "hyde_query": self.hyde_enhance (query), "variants": self.multi_query_expand (query),
        "sub_queries": self.decompose_complex_query (query), "metadata_filters": self.extract_metadata_filters (query),
    }
...
def multi_query_search (
    query: str, search_fn: Callable [..., Any], num_variants: int = 3, top_k: int = 10, ) -> list [dict [str, Any]]:
  """

...
def rerank_chunks_with_scores (
    query: str, chunks: list [str], top_k: int = 20, use_cache: bool = True, ) -> list [tuple [int, float]]:
  """
...
def hybrid_rerank (
    query: str, documents: list [dict [str, Any]], colbert_weight: float = 0.3, cross_encoder_weight: float = 0.7,
) -> \
list [dict [str, Any]]:
  """
  Two-stage reranking:
  ...
  for doc in documents:
    if "colbert_tokens" in doc.get ("metadata", {}):
      score = colbert_score (doc ["metadata"] ["query_tokens"], doc ["metadata"] ["colbert_tokens"], )
...
def _fine_tune_with_lora (pairs: list [tuple [str, str, float]], epochs: int = 3) -> Any:
  ...
  try:
    ...
    if job.status == "completed":
      logger.info ("LoRA fine-tuned reranker saved to %s (mrr=%.4f, ndcg@10=%.4f)", job.artifact_uri,
          job.metrics.get ("mrr", 0.0), job.metrics.get ("ndcg_at_10", 0.0), )
...
if __name__ == "__main__":
  ...
  chunks = [
      "CI/CD pipeline настраивается через файл .gitlab-ci.yml", "Docker позволяет контейнеризировать приложения",

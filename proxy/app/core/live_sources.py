...
from proxy.app.shared.config import (
  CONFLUENCE_API_TOKEN, CONFLUENCE_API_URL, CONFLUENCE_API_USER, GITLAB_API_TOKEN, GITLAB_API_URL, JIRA_API_TOKEN,
  JIRA_API_URL, JIRA_API_USER, LIVE_SOURCES_ENABLED,
)
...
class ConfluenceLiveClient (_CacheMixin):
  ...
  def __init__ (self) -> None:
    ...
    if CONFLUENCE_API_USER and CONFLUENCE_API_TOKEN:
      self.headers ["Authorization"] = aiohttp.encode_basic_auth (CONFLUENCE_API_USER, CONFLUENCE_API_TOKEN)
  ...
  async def search_confluence (self, query: str, max_results: int = 5) -> list [ConfluencePage]:
    ...
    try:
      ...
      for item in data.get ("results", []):
        ...
        results.append (ConfluencePage (id = item.get ("id", ""), title = item.get ("title", ""),
            space_key = item.get ("space", {}).get ("key", ""), page_type = item.get ("type", "page"), body = body,
  ...
  async def get_confluence_page (self, page_id: str) -> ConfluencePage | None:
    ...
    try:
      ...
      page = ConfluencePage (id = data.get ("id", ""), title = data.get ("title", ""),
          space_key = data.get ("space", {}).get ("key", ""), page_type = data.get ("type", "page"), body = body,
          url = f"{self.base_url}/content/{page_id}", )
...
class JiraLiveClient (_CacheMixin):
  ...
  async def search_jira (self, query: str, max_results: int = 5) -> list [JiraIssue]:
    ...
    try:
      ...
      params: dict [str, str | int] = {
          "jql": jql, "maxResults": max_results, "fields": "summary,description,status,priority,assignee,issuetype",
      }  # noqa: E501
      ...
      for item in data.get ("issues", []):
        ...
        results.append (
            JiraIssue (id = item.get ("id", ""), key = item.get ("key", ""), summary = fields.get ("summary", ""),
                description = fields.get ("description", "") or "", status = fields.get ("status", {}).get ("name", ""),
  ...
  async def get_jira_issue (self, issue_key: str) -> JiraIssue | None:
    ...
    try:
      ...
      issue = JiraIssue (id = data.get ("id", ""), key = data.get ("key", ""), summary = fields.get ("summary", ""),
          description = fields.get ("description", "") or "", status = fields.get ("status", {}).get ("name", ""),
          priority = fields.get ("priority", {}).get ("name", ""),
          assignee = fields.get ("assignee", {}).get ("displayName", ""),
          issue_type = fields.get ("issuetype", {}).get ("name", ""), )
...
class GitLabLiveClient (_CacheMixin):
  ...
  async def search_gitlab (self, query: str, max_results: int = 5) -> list [GitLabProject]:
    ...
    try:
      ...
      results = [GitLabProject (id = str (item.get ("id", "")), name = item.get ("name", ""),
          path_with_namespace = item.get ("path_with_namespace", ""),
          description = item.get ("description", "") or "", ) for item in data]
      self._set_cache (cache_key, results)
      return results
    except (TimeoutError, aiohttp.ClientError, OSError) as e:
  ...
  async def get_gitlab_file (self, project_id: str, file_path: str, ref: str = "main") -> GitLabFile | None:
    ...
    try:
      ...
      file_obj = GitLabFile (file_name = data.get ("file_name", ""), file_path = file_path, content = content,
          ref = data.get ("ref", ref), size = data.get ("size", 0), )

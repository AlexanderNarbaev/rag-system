...
def hybrid_search (
    query: str, version: str | None = None, top_k: int = 50, namespace: str | None = None, lang: str | None = None,
) -> \
list [Any]:
  """
  ...
  if _get_cb is not None:
    try:
      dense_response = _get_cb ("qdrant").call_sync (
          lambda: _qc.query_points (**_dense_kwargs))
    ...
    if _get_cb is not None:
      try:
        sparse_response = _get_cb ("qdrant").call_sync (
            lambda: _qc.query_points (collection_name = COLLECTION_NAME, query = sparse_vec, using = "sparse",
                limit = top_k, query_filter = q_filter, with_payload = True, ))
...
class CypherQueryGenerator:
  ...
  PATTERNS = [
      {
          "pattern": r"what (?:projects?|repos?|repositories?) (?:does|do) (\w+) work on", "template": (
  ...
  def generate (self, query: str) -> str | None:
    ...
    if entities:
      entity = entities [0]
      cypher = (f"MATCH (n) WHERE n.name CONTAINS '{entity}'"
  ...
  def search (
    ...
    for community in self.community_summaries:
      ...
      if overlap > 0:
        ...
        scored.append ({
            "community_id": community.get ("id", ""), "summary": summary,
...
def apply_time_decay (chunks: list [dict [str, Any]], decay_days: int = 180) -> list [dict [str, Any]]:
  ...
  for chunk in chunks:
    ...
    ts_raw = (payload.get ("updated_at") or payload.get ("created_at") or chunk.get ("updated_at") or chunk.get (
      "created_at"))

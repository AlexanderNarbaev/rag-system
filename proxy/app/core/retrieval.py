...
def hybrid_search(
    ...
    # Diagnostic: log raw scores before filtering (first 5 results)
    if combined_results:
        _sample_scores = [round(r.score, 4) for r in combined_results[:5]]
        logger.debug(
            "hybrid_search raw results: dense=%d sparse=%d combined=%d top5_scores=%s vector_name=%s",
            len(dense_results),
            len(sparse_results),
        ...
        logger.warning(
            "hybrid_search returned 0 raw results from Qdrant. dense=%d sparse=%d vector_name=%s collection=%s",
            len(dense_results),
            len(sparse_results),
            _dense_vector_name,
            COLLECTION_NAME,
    ...
    if not filtered_results:
        logger.warning(
            "hybrid_search final: 0 results after filtering (raw_combined=%d, quality=%s, strong=%.2f borderline=%.2f)",
            len(combined_results),
            quality,
            STRONG_SCORE_THRESHOLD,
            BORDERLINE_SCORE_THRESHOLD,

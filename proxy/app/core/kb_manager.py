...
class KnowledgeBaseManager:
  ...
  def create_kb (
    ...
    try:
      conn.execute (
          ...
          "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
          (kb_id, name, description, collection_name, embedding_model, dense_vector_size,
           str (parser_config or {}), now, now),
      )
      conn.commit ()
    ...
    kb = KnowledgeBase (
        ...
        parser_config = parser_config or {}, created_at = now, updated_at = now,
    )
  ...
  def get_kb (self, kb_id: str) -> KnowledgeBase | None:
    ...
    conn = self._get_conn ()
    try:
      row = conn.execute ("SELECT * FROM knowledge_bases WHERE id = ? AND status != 'deleted'", (kb_id,)).fetchone ()
      if row is None:
        return None
  ...
  def get_kb_by_name (self, name: str) -> KnowledgeBase | None:
    ...
    conn = self._get_conn ()
    try:
      row = conn.execute ("SELECT * FROM knowledge_bases WHERE name = ? AND status != 'deleted'", (name,)).fetchone ()
      if row is None:
        return None
      return self._row_to_kb (row)
    finally:
  ...
  def list_kbs (self, include_deleted: bool = False) -> list [KnowledgeBase]:
    ...
    try:
      if include_deleted:
        ...
        rows = conn.execute ("SELECT * FROM knowledge_bases WHERE status != 'deleted' ORDER BY created_at DESC").fetchall ()
  ...
  def update_kb_stats (self, kb_id: str) -> None:
    ...
    try:
      task_counts = conn.execute (
          ...
          "FROM etl_tasks WHERE kb_id = ?", (kb_id,),
      ).fetchone ()
      now = time.time ()
      conn.execute (
  ...
  def _row_to_kb (row: sqlite3.Row) -> KnowledgeBase:
    ...
    return KnowledgeBase (
        ...
        dense_vector_size = row ["dense_vector_size"], parser_config = parser_config,
        doc_count = row ["doc_count"], chunk_count = row ["chunk_count"],
  ...
  def _row_to_task (row: sqlite3.Row) -> ETLTask:
    ...
    return ETLTask (
        id = row ["id"], kb_id = row ["kb_id"], source_type = row ["source_type"],
        source_id = row ["source_id"], status = row ["status"], progress = row ["progress"],

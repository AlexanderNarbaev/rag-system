...
from proxy.app.shared.config import (
  MAX_CHUNKS_AFTER_RERANK, MAX_CHUNKS_RETRIEVAL, MAX_RETRIEVAL_LOOPS, USE_GRAPH_EXPANSION,
)
...
def _dynamic_top_k (query: str, *, max_default: int = 50) -> int:
  ...
  try:
    ...
    mapping = {
        IntentType.GREETING: 0, IntentType.SIMPLE_FACT: min (5, max_default), IntentType.FACTUAL: min (15, max_default),
        IntentType.PROCEDURAL: min (25, max_default), IntentType.SUMMARIZATION: min (30, max_default),
        IntentType.COMPARISON: max_default, IntentType.COMPLEX: max_default,
    }
...
def rewrite_query (state: dict [str, Any]) -> dict [str, Any]:
  ...
  prompt = ("Перепиши следующий вопрос пользователя в более "
  ...
  try:
    rewritten = _get_non_stream_completion_sync () ([{"role": "user", "content": prompt}], temperature = 0.1,
        max_tokens = 100)
...
def check_confidence (state: dict [str, Any]) -> dict [str, Any]:
  ...
  from proxy.app.shared.config import (
    ADMIN_ALERT_ENABLED, CONFIDENCE_THRESHOLD, HALLUCINATION_CHECK_ENABLED, MAX_VERIFY_LOOPS, SELF_CRITIQUE_ENABLED,
  )
  ...
  return {
      "confidence": report.score, "needs_escalation": needs_escalation,
...
def self_critique (state: dict [str, Any]) -> dict [str, Any]:
  ...
  if not answer or not answer.strip ():
    return {
        "self_critique_score": 0, "needs_rewrite": False, "self_critique_count": critique_count,
    }
  
  if rewrite_count >= max_rewrites:
    logger.info (f"Max rewrites ({max_rewrites}) reached, skipping self-critique")
    return {
        "self_critique_score": 3, "needs_rewrite": False, "self_critique_count": critique_count,
    }
  
  prompt = (f"Rate the following answer's usefulness on a scale of 1 to 5.\n"
  ...
  try:
    ...
    return {
        "self_critique_score": score, "needs_rewrite": needs_rewrite, "self_critique_count": critique_count + 1,
    }
  except Exception as e:
    logger.warning (f"Self-critique call failed: {e}, accepting answer")
    return {
        "self_critique_score": 3, "needs_rewrite": False, "self_critique_count": critique_count + 1,
    }
...
def self_reflection (state: dict [str, Any]) -> dict [str, Any]:
  ...
  prompt = (f"You are an answer quality evaluator. Determine if the answer is fully supported by the context.\n"
            f"Reply with ONLY one of these exact words: FULLY_SUPPORTED, PARTIALLY_SUPPORTED, or NOT_SUPPORTED.\n"
            f"If PARTIALLY_SUPPORTED or NOT_SUPPORTED, on the next line write: MISSING: <what information is "
            f"missing>\n\n"
            f"User question: {query}\n\n"
            f"Context:\n{context}\n\n"
            f"Answer: {answer}\n\n"
  ...
  try:
    ...
    if not is_fully_supported:
      ...
      return {
          "needs_reflection": True, "reflection_count": reflection_count + 1, "reflection_gaps": missing_info,
      }
...
def call_tools (state: dict [str, Any]) -> dict [str, Any]:
  ...
  for tc in tool_calls:
    try:
      ...
      tool_results.append ({
          "tool_call_id": tc.get ("id", result.tool_call_id), "name": result.name, "content": result.content,
          "error": result.error,
      })
      ...
      tool_results.append ({
          "tool_call_id": tc.get ("id", ""), "name": func_name, "content": f"Error: {e}", "error": str (e),
      })

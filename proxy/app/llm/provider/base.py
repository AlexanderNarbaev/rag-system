...
class MultiProviderRouter:
  ...
  async def _send_request (
      ...
      retry: int | None = None, provider_type: str | None = None, ) -> Any:
      ...
      try:
        ...
        if stream:
          ...
          if "choices" not in translated or not translated ["choices"]:
            raise LLMError (f"Invalid response format from LLM: "
  ...
  async def non_stream_completion (
      self, messages: list [dict [str, Any]], temperature: float = 0.2, max_tokens: int = 4096,
      tools: list [ToolDefinition] | None = None, tool_results: list [ToolResult] | None = None,
      provider_type: str | None = None, ) -> dict [str, Any]:
  ...
  async def non_stream_completion_text (
    ...
    data = await self.non_stream_completion (messages, temperature, max_tokens, provider_type = provider_type, )
    try:
  ...
  async def tool_use_completion (
    ...
    try:
      ...
      return [ToolCall (id = tc.get ("id", f"call_{i}"), name = tc.get ("function", {}).get ("name", ""),
          arguments = json.loads (tc.get ("function", {}).get ("arguments", "{}")), ) for i, tc in
  ...
  def non_stream_completion_sync (
    ...
    try:
      return loop.run_until_complete (
          self.non_stream_completion_text (messages, temperature, max_tokens, provider_type = provider_type, ))
    finally:
      loop.close ()

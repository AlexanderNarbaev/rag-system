...
class RemoteEmbeddingClient:
    ...
    def _check_health(self) -> bool:
        ...
        """
        from proxy.app.shared.utils import safe_urlopen
        ...
        try:
            ...
            safe_urlopen(models_url, timeout=10.0, headers=extra_headers)
            return True
        except Exception:
            try:
    ...
    def encode(
        ...
        **kwargs: Any,
        ...
        try:
            with safe_urlopen(self._embedding_url, timeout=60.0, data=payload, method="POST", headers=headers) as resp:
...
class RemoteRerankerClient:
    ...
    def predict(self, pairs: list[tuple[str, str]], **kwargs: Any) -> np.ndarray:
        ...
        for query, indices in query_to_indices.items():
            ...
            try:
                with safe_urlopen(self._rerank_url, timeout=60.0, data=payload, method="POST", headers=headers) as resp:
                    body = _json.loads(resp.read().decode("utf-8"))
            except Exception as exc:

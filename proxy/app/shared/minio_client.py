...
class MinioClient:
  ...
  def _get_client (self) -> boto3.client:
    ...
    if self._client is None:
      ...
      self._client = boto3.client ("s3", endpoint_url = f"{scheme}://{self._endpoint}",
    ...
    try:
      ...
      if error_code == "404":
        try:
          ...
          raise StorageError (f"Failed to create bucket '{self._bucket}': {create_exc}",
  ...
  def get_file_metadata (self, object_name: str) -> dict [str, Any]:
    ...
    try:
      ...
      return {
          "key": object_name, "size": response.get ("ContentLength", 0),
  ...
  def generate_presigned_url (
    ...
    client = self._get_client ()
    try:
      url: str = client.generate_presigned_url ("get_object", Params = {"Bucket": self._bucket, "Key": object_name},
  ...
  def health_check (self) -> bool:
    ...
    try:
      ...
      raise StorageError (f"MinIO health check failed: {exc}", component = "minio", ) from exc

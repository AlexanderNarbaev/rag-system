...
ROLE_MAX_LEVEL: dict [str, str] = {
    "admin": "restricted", "expert": "confidential", "developer": "confidential", "viewer": "internal",
    "external": "public",
}

# Role hierarchy: which access levels each role can see
ROLE_ACCESS: dict [str, list [str]] = {
    "admin": ["public", "internal", "confidential", "restricted"], "expert": ["public", "internal", "confidential"],
    "developer": ["public", "internal", "confidential"], "viewer": ["public", "internal"], "external": ["public"],
}
...
def build_access_filter_should (user_context: UserContext) -> dict [str, Any] | None:
  ...
  if base_levels:
    should_clauses.append ({
        "key": "access_level", "match": {"any": base_levels},
    })
...
def filter_chunks (
    chunks: list [dict [str, Any]], user_context: UserContext, ) -> list [dict [str, Any]]:
  ...
  for chunk in chunks:
    ...
    if chunk_level == "confidential":
      if not user_context.groups:
        continue
      allowed_groups = chunk.get ("allowed_groups", chunk.get ("payload", {}).get ("allowed_groups", []), )
    ...
    if chunk_level == "restricted":
      allowed_users = chunk.get ("allowed_users", chunk.get ("payload", {}).get ("allowed_users", []), )
...
def can_access_source (
    user_context: UserContext, source_type: str, source_id: str, ) -> bool:
  ...
  user_max_level_rank = max (
      (ACCESS_LEVEL_RANK.get (ROLE_MAX_LEVEL.get (role, "public"), 0) for role in user_context.roles), default = 0, )
  
  return user_max_level_rank >= 0


def can_access_document (
    user_context: UserContext, access_level: str, allowed_groups: list [str] | None = None,

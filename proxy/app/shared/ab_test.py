...
class ABVariant:
  ...
  def metric_stats (self, metric_name: str) -> dict [str, Any]:
    ...
    if not values:
      return {"count": 0}
    return {
        "count": len (values), "mean": mean (values), "std": stdev (values) if len (values) > 1 else 0.0,
...
class ABTest:
  ...
  def compare (self, metric_name: str, variant_a: str, variant_b: str) -> dict [str, Any]:
    ...
    result = {
        "metric": metric_name, "variant_a": variant_a, "variant_b": variant_b, "mean_a": stats_a.get ("mean"),
        "mean_b": stats_b.get ("mean"), "std_a": stats_a.get ("std"), "std_b": stats_b.get ("std"),
        "count_a": stats_a ["count"], "count_b": stats_b ["count"], "p_value": None, "significant": False,
    }
  ...
  def get_report (self) -> dict [str, Any]:
    report: dict [str, Any] = {
        "name": self.name, "variants": [], "total_trials": sum (v.trials for v in self._variants.values ()),
    }
    for v in self._variants.values ():
      v_report = {
          "name": v.name, "trials": v.trials, "weight": v.weight, "metrics": {k: v.metric_stats (k) for k in v.metrics},
      }
...
class ModelVariant:
  ...
  def __eq__ (self, other: object) -> bool:
    ...
    return self.model_name == other.model_name and self.adapter_version == other.adapter_version
...
class ABTestRunner:
  ...
  def __init__ (
      self, name: str, canary_controller: CanaryController | None = None, ) -> None:
    self.name = name
  ...
  def select_model (self) -> ModelVariant:
    if not self._variants:
      raise ValueError (f"No model variants registered in runner '{self.name}'")

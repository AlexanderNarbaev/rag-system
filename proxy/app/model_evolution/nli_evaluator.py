...
class NLIEvaluationResult:
  ...
  def as_metrics (self) -> dict [str, float]:
    return {
        "nli_entailment_rate": self.entailment_rate, "nli_contradiction_rate": self.contradiction_rate,
        "nli_neutral_rate": self.neutral_rate, "nli_overall_score": self.overall_score,
    }


def _load_nli_model (
    model_name: str = "MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli", device: str = "cpu", ) -> tuple [Any, Any]:
  ...
  try:
    _NLI_TOKENIZER = AutoTokenizer.from_pretrained (model_name, use_fast = True, local_files_only = True, )
...
def nli_predict (claim: str, context: str, max_length: int = 512) -> tuple [str, float]:
  ...
  inputs = tokenizer (truncated_context, claim, truncation = True, max_length = max_length, return_tensors = "pt", )
...
def evaluate_nli (
    answer: str, context: str, use_real_nli: bool = True, ) -> NLIEvaluationResult:
  if not answer or not answer.strip ():
    return NLIEvaluationResult (entailment_rate = 0.0, contradiction_rate = 0.0, neutral_rate = 0.0,
  ...
  if not context or not context.strip ():
    return NLIEvaluationResult (entailment_rate = 0.0, contradiction_rate = 0.0, neutral_rate = 0.0,
        overall_score = 0.0, total_claims = len (claims), entailed_claims = 0, contradicted_claims = 0,
  ...
  for claim in claims:
    ...
    per_claim.append ({
        "claim": claim, "label": label, "confidence": round (confidence, 3),
    })
  ...
  return NLIEvaluationResult (entailment_rate = round (entailment_rate, 4),
      contradiction_rate = round (contradiction_rate, 4), neutral_rate = round (neutral_rate, 4),
      overall_score = round (overall_score, 4), total_claims = total, entailed_claims = entailed,
      contradicted_claims = contradicted, neutral_claims = neutral, per_claim_scores = per_claim, )


def evaluate_nli_batch (
    answer_context_pairs: list [tuple [str, str]], use_real_nli: bool = True, ) -> dict [str, float]:
  if not answer_context_pairs:
    return {
        "nli_entailment_rate": 0.0, "nli_contradiction_rate": 0.0, "nli_neutral_rate": 0.0, "nli_overall_score": 0.0,
    }
  ...
  return {
      "nli_entailment_rate": round (entailment_rate, 4), "nli_contradiction_rate": round (contradiction_rate, 4),
      "nli_neutral_rate": round (neutral_rate, 4), "nli_overall_score": round (overall_score, 4),
  }

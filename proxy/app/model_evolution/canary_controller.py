...
class CanaryConfig:
  ...
  rollback_thresholds: dict [str, tuple [float, str]] = field (default_factory = lambda: {
      "error_rate": (0.05, "gt"), "p95_latency_ms": (10000, "gt"),
  })
  cooldown_seconds: int = 3600


canary_traffic_total = Counter ("rag_canary_traffic_total", "Total requests routed by canary controller",
    ["model", "target"], )

canary_result_total = Counter ("rag_canary_result_total", "Canary request outcomes (success/error)",
    ["model", "target", "outcome"], )

canary_latency_seconds = Histogram ("rag_canary_latency_seconds", "Canary request latency", ["model", "target"],
    buckets = (0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0, 30.0, 60.0), )

canary_phase_gauge = Gauge ("rag_canary_phase", "Current canary phase (0=idle, 1=ramp_5, ..., 5=full, 6=rollback)",
    ["model"], )

canary_split_ratio = Gauge ("rag_canary_split_ratio", "Current traffic split ratio for canary", ["model"], )

canary_rollback_total = Counter ("rag_canary_rollback_total", "Total number of canary rollbacks triggered", ["model"], )
...
class CanaryController:
  ...
  def configure (
      ...
      cooldown_seconds: int = 3600, ) -> None:
    ...
    """
    with self._lock:
      config = CanaryConfig (model_name = model_name, stable_version = stable_version, canary_version = canary_version,
          canary_percent = canary_percent, min_samples = min_samples, rollback_thresholds = rollback_thresholds or {
              "error_rate": (0.05, "gt"), "p95_latency_ms": (10000, "gt"),
  ...
  def status (self, model_name: str | None = None) -> dict [str, Any]:
    ...
    with self._lock:
      ...
      for name in names:
        ...
        cooldown_remaining = max (0.0, (self._rollback_cooldown_until.get (name, 0) - time.time ()), )
        metrics = self.get_metrics (name)
        result [name] = {
            "phase": phase.value, "split": {
                "stable": 1.0 - config.canary_percent, "canary": config.canary_percent,
            }, "stable_version": config.stable_version, "canary_version": config.canary_version,
  ...
  def _phase_to_int (phase: CanaryPhase) -> int:
    mapping = {
        CanaryPhase.IDLE: 0, CanaryPhase.RAMP_5: 1, CanaryPhase.RAMP_25: 2, CanaryPhase.RAMP_50: 3,
        CanaryPhase.RAMP_75: 4, CanaryPhase.FULL: 5, CanaryPhase.ROLLBACK: 6,
    }

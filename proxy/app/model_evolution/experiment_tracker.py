...
class ExperimentTracker:
  ...
  def start_run (self, run_name: str | None = None) -> RunInfo:
    ...
    run = RunInfo (run_id = run_id, experiment_name = self.experiment_name, start_time = time.time (), )
  ...
  def _save_run (self, run: RunInfo) -> None:
    ...
    with open (path, "w") as f:
      json.dump ({
          "run_id": run.run_id, "experiment_name": run.experiment_name, "params": run.params, "metrics": run.metrics,
          "artifacts": run.artifacts, "status": run.status, "start_time": run.start_time, "end_time": run.end_time,
      }, f, indent = 2, )

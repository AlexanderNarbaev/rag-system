...
class RerankerDataset (torch.utils.data.Dataset if _TORCH_AVAILABLE else object):  # type: ignore[misc]
  ...
  def __getitem__ (self, idx: int) -> dict [str, Any]:
    ...
    encoded = self.tokenizer (query, chunk, truncation = True, padding = "max_length", max_length = self.max_length,
        return_tensors = "pt", )
    return {
...
class RerankerTrainer (TrainerBase):
  ...
  def prepare_data (
      self, dataset: list [tuple [str, str, float]], eval_split: float = 0.2, seed: int = 42, ) -> dict [
    str, list [tuple [str, str, float]]]:
    filtered = [(q, c, s) for q, c, s in dataset if
        isinstance (q, str) and isinstance (c, str) and len (q) > 0 and len (c) > 0]
    if not filtered:
      return {"train": [], "eval": []}
    random.seed (seed)
    ...
    return {
        "train": [filtered [i] for i in indices [:split_idx]], "eval": [filtered [i] for i in indices [split_idx:]],
    }
  
  def train (self, config: TrainingConfig) -> TrainingJob:
    job_id = str (uuid.uuid4 ())
    job = TrainingJob (job_id = job_id, trainer_type = TrainerType.RERANKER, config = config, status = "running", )
    try:
  ...
  def _train_lora (self, config: TrainingConfig, job: TrainingJob, job_id: str) -> TrainingJob:
    ...
    model = AutoModelForSequenceClassification.from_pretrained (base_model, num_labels = 1, device_map = device_map, )
  ...
  def _train_full (self, config: TrainingConfig, job: TrainingJob, job_id: str) -> TrainingJob:
    ...
    ce.fit (train_dataloader = None, train_inputs = train_inputs, train_labels = train_scores, epochs = config.epochs,
  ...
  def save_adapter (self, model: Any, output_path: str, tokenizer: Any = None) -> str:
    ...
    config_data = {
        "model_type": "reranker_cross_encoder", "base_model": str (base_model) if base_model else "unknown",
        "num_labels": 1,
    }
  ...
  def _build_lora_config (self, config: TrainingConfig) -> Any:
    return LoraConfig (task_type = TaskType.SEQ_CLS, r = getattr (config, "lora_r", RERANKER_LORA_R),
  ...
  def _evaluate_from_pairs (self, pairs: list [tuple [str, str, float]]) -> dict [str, float]:
    ...
    return {
        "mrr": mrr_sum / query_count, "ndcg_at_10": ndcg_sum / query_count,

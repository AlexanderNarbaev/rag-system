...
class TrainingJob:
  ...
  def to_dict (self) -> dict [str, Any]:
    ...
    return {
        "job_id": self.job_id, "trainer_type": self.trainer_type.value, "config": {
            "trainer_type": self.config.trainer_type.value, "env_profile": self.config.env_profile.value,
            "base_model": self.config.base_model, "output_dir": self.config.output_dir, "epochs": self.config.epochs,
            "batch_size": self.config.batch_size, "learning_rate": self.config.learning_rate,
            "eval_split": self.config.eval_split, "max_seq_length": self.config.max_seq_length,
            "use_lora": self.config.use_lora, "lora_r": self.config.lora_r, "lora_alpha": self.config.lora_alpha,
            "lora_dropout": self.config.lora_dropout, "use_qlora": self.config.use_qlora,
            "load_in_4bit": self.config.load_in_4bit, "seed": self.config.seed,
        }, "status": self.status, "mlflow_run_id": self.mlflow_run_id, "metrics": self.metrics,
        "artifact_uri": self.artifact_uri, "started_at": self.started_at, "completed_at": self.completed_at,

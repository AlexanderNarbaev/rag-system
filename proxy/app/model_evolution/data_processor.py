...
class DataProcessor:
    ...
    for entry in entries:
      ...
      if query and correction:
        dataset.llm_data.append ({
            "instruction": query, "input": answer or "", "output": correction,
        })

      if query and source_chunks:
        for chunk in source_chunks:
          dataset.reranker_data.append ({
              "query": query, "chunk": chunk.get ("text", chunk) if isinstance (chunk, dict) else str (chunk),
              "relevance": relevance,
          })
  ...
  def split_train_val_test (
      self, data: list [dict [str, Any]], train_ratio: float = 0.8, val_ratio: float = 0.1, test_ratio: float = 0.1,

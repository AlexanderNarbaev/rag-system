...
def compute_rouge_l (
    references: list [str], hypotheses: list [str], ) -> dict [str, float]:
  precisions = []
  recalls = []
  for ref, hyp in zip (references, hypotheses, strict = False):
    ref_tokens = ref.lower ().split ()
...
def compute_generation_metrics (
    references: list [str], hypotheses: list [str], compute_bertscore_flag: bool = False, ) -> GenerationMetrics:
  ...
  return GenerationMetrics (bleu_1 = bleu.get ("bleu_1", 0.0), bleu_4 = bleu.get ("bleu_4", 0.0),
      rouge_l_f1 = rouge.get ("rouge_l_f1", 0.0), rouge_l_precision = rouge.get ("rouge_l_precision", 0.0),
      rouge_l_recall = rouge.get ("rouge_l_recall", 0.0), bertscore_f1 = bs.get ("bertscore_f1"), )


def compute_all_gen_metrics (
    references: list [str], hypotheses: list [str], ) -> GenerationMetrics:
...
def compute_hallucination_rate (
    references: list [str], hypotheses: list [str], ) -> float:
  total_words = 0
  novel_words = 0
  for ref, hyp in zip (references, hypotheses, strict = False):

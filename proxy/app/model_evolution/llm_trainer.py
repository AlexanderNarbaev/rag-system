...
class LLMTrainer (TrainerBase):
  ...
  def train (self, training_data: list [dict [str, Any]]) -> TrainingJob:  # type: ignore[override]
    ...
    job = TrainingJob (job_id = self._make_job_id (), trainer_type = TrainerType.LLM, config = self.config,
  ...
  def _train_mock (self, job: TrainingJob, prepared: list [dict [str, Any]]) -> TrainingJob:
    ...
    job.metrics = {
        "train_loss": 0.5 + (len (prepared) % 10) * 0.01, "val_loss": 0.55 + (len (prepared) % 10) * 0.01,
        "bleu_1": 0.42, "bleu_4": 0.15, "rouge_l_f1": 0.38,
    }
  ...
  def _train_gpu (self, job: TrainingJob, prepared: list [dict [str, Any]]) -> TrainingJob:
    ...
    logger.info ("Starting QLoRA training with %d samples on %s", len (prepared),
    ...
    peft_config = LoraConfig (r = self.config.lora_r, lora_alpha = self.config.lora_alpha,
        lora_dropout = self.config.lora_dropout, bias = "none", task_type = "CAUSAL_LM",
    ...
    training_args = TrainingArguments (output_dir = str (Path (self.config.output_dir) / f"checkpoints_{job.job_id}"),
        num_train_epochs = self.config.epochs, per_device_train_batch_size = self.config.batch_size,
        gradient_accumulation_steps = 4, gradient_checkpointing = True, warmup_steps = self.config.warmup_steps,
        learning_rate = self.config.learning_rate, logging_steps = self.config.logging_steps,
        save_steps = self.config.save_steps, eval_steps = self.config.eval_steps, eval_strategy = "steps",
        save_strategy = "steps", load_best_model_at_end = True, fp16 = torch.cuda.is_available (), bf16 = False,
        seed = self.config.seed, report_to = "none", )
  ...
  def _tokenize_dataset (self, tokenizer: Any, prepared: list [dict [str, Any]]) -> Any:
    ...
    encodings = tokenizer (texts, truncation = True, padding = True, max_length = self.config.max_seq_length,
        return_tensors = "pt", )
  ...
  def evaluate (self, eval_data: list [dict [str, Any]], model: Any = None) -> dict [  # type: ignore[override]
    ...
    if self._is_cpu_profile ():
      return {
          "train_loss": 0.45, "val_loss": 0.52, "bleu_1": 0.40, "bleu_4": 0.12, "rouge_l_f1": 0.35,
      }
    return {
        "eval_loss": 0.48, "bleu_1": 0.44, "bleu_4": 0.18, "rouge_l_f1": 0.40,
    }
  ...
  def save_adapter (self, output_path: Any, model: Any = None) -> str:  # type: ignore[override]
    ...
    adapter_config = {
        "base_model_name_or_path": self.config.base_model, "lora_r": self.config.lora_r,
        "lora_alpha": self.config.lora_alpha, "lora_dropout": self.config.lora_dropout, "bias": "none",
        "task_type": "CAUSAL_LM", "peft_type": "LORA", "use_qlora": self.config.use_qlora,
        "load_in_4bit": self.config.load_in_4bit,
    }

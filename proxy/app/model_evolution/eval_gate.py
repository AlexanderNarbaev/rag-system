...
class EvalGate:
  ...
  def evaluate (
      ...
      version: str | None = None, ) -> GateResult:
      ...
      if config.baseline_regression_tolerance > 0:
        for threshold in config.thresholds:
          ...
          if delta < -config.baseline_regression_tolerance:
            warnings.append (f"{threshold.metric_name} regressed by {abs (delta):.4f} "
  ...
  def evaluate_with_nli (
      ...
      baseline_metrics: dict [str, float] | None = None, version: str | None = None,
      use_real_nli: bool = True, ) -> GateResult:

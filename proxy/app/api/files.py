...
ALLOWED_CONTENT_TYPES = {
    ...
    "application/vnd.ms-excel", "application/vnd.ms-powerpoint", "image/png", "image/jpeg", "image/webp",
}
...
async def upload_file (
    request: Request, file: UploadFile, user: UserContext = Depends (require_role (Role.USER)),  # noqa: B008
...
async def list_files (
    prefix: str = "", user: UserContext = Depends (require_role (Role.USER)),  # noqa: B008
    minio: MinioClient = Depends (_get_minio_client),  # noqa: B008
) -> FileListResponse:
  """List uploaded files.
...
async def download_file (
    file_id: str, user: UserContext = Depends (require_role (Role.USER)),  # noqa: B008
    minio: MinioClient = Depends (_get_minio_client),  # noqa: B008
) -> StreamingResponse:
...
async def get_presigned_url (
    file_id: str, expiration: int = 3600, user: UserContext = Depends (require_role (Role.USER)),  # noqa: B008
    minio: MinioClient = Depends (_get_minio_client),  # noqa: B008
...
async def delete_file (
    file_id: str, user: UserContext = Depends (require_role (Role.EXPERT)),  # noqa: B008
    minio: MinioClient = Depends (_get_minio_client),  # noqa: B008
) -> FileDeleteResponse:

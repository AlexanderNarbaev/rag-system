...
async def list_tools (
    category: str | None = None, tag: str | None = None, provider: str | None = None,
  ...
  return {
      "count": len (tools), "tools": [{
          "name": t.name, "description": t.description, "category": t.category, "tags": t.tags, "version": t.version,
          "parameters": t.to_json_schema (), "provider": t.provider,
      } for t in tools],
  }
...
async def get_tool (
    name: str, user: UserContext = Depends (get_optional_auth_context),  # noqa: B008
) -> dict [str, Any]:
  ...
  return {
      ...
      "version": tool.version, "visibility": tool.visibility.value, "timeout_seconds": tool.timeout_seconds,
      "parameters": tool.to_json_schema (), "provider": tool.provider, "depends_on": tool.depends_on,
  }

...
def _check_qdrant() -> tuple[str, dict[str, Any]]:
    ...
    try:
        ...
        info: dict[str, Any] = {"collections": len(collections.collections)}
                ...
                if isinstance(params.vectors, dict):
                    ...
                    for name, vec_params in params.vectors.items():
                        vec_info[name] = {
                            ...
                            "distance": str(getattr(vec_params, "distance", "unknown")),
                        }
                    info["vectors"] = vec_info
                else:
                    info["distance_metric"] = str(getattr(params.vectors, "distance", "unknown"))
                    info["vector_size"] = getattr(params.vectors, "size", None)
...
def _check_kb_manager() -> tuple[str, dict[str, Any]]:
    ...
    try:
        ...
        logger.error("KB manager health check failed: %s", e)
...
def _check_tls() -> tuple[str, dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            nginx_tls_url = os.getenv("NGINX_TLS_INFO_URL", "http://nginx/nginx-tls-info")
        ...
        logger.error("TLS health check failed: %s", e)
...
def _check_secret_rotation() -> tuple[str, dict[str, Any]]:
    ...
    try:
        ...
        logger.error("Secret rotation health check failed: %s", e)
        return "Secret rotation check failed", {}
...
async def health_ready() -> JSONResponse:
    ...
    Includes diagnostic info: collection metadata, distance metric,
    vector names, point count, and a sample query to verify search works.
    """

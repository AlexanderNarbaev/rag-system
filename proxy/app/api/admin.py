    ...
    with self._lock:
      if model_name not in self._models:
        self._models [model_name] = {
            "traffic_split": 0.0, "stable_traffic": 1.0, "phase": "idle", "stable_version": None,
            "canary_version": None,
        }
...
class _TrainingJobStore:
  ...
  def create (self, trainer_type: str, config: dict [str, Any]) -> str:
    ...
    with self._lock:
      self._jobs [job_id] = {
          "job_id": job_id, "trainer_type": trainer_type, "status": "queued", "config": config, "metrics": {},
...
async def admin_models_train (
    request: TrainRequest, user: UserContext = Depends (require_role (Role.ADMIN)),  # noqa: B008
) -> TrainResponse:
  ...
  async def _run_training () -> None:
    try:
      ...
      config = TrainingConfig (trainer_type = trainer_type, env_profile = profile,
          base_model = request.base_model or "", epochs = request.epochs or preset.get ("epochs", 1),
          batch_size = request.batch_size or preset.get ("batch_size", 2), learning_rate = request.learning_rate,
          use_lora = request.use_lora, lora_r = preset.get ("lora_r", 4), lora_alpha = preset.get ("lora_alpha", 8),
      ...
      if trainer_type == ME_TrainerType.SLM:
        ...
        result = ME_TrainingJob (job_id = job_id, trainer_type = ME_TrainerType.LLM, config = config,
            ...
            completed_at = datetime.now (UTC).isoformat (), )
      _training_jobs.update (job_id, status = result.status if hasattr (result, "status") else "completed",
      ...
      if hasattr (result, "status") and result.status == "completed":
        ...
        try:
          registry.register (name = request.trainer_type.value,
...
async def admin_models_status (
    job_id: str, user: UserContext = Depends (require_role (Role.ADMIN)),  # noqa: B008
) -> JSONResponse:
...
async def admin_models_list (
  ...
  for model_name in registry.list_models ():
    ...
    models_data [model_name] = {
        "versions": [{
            "version": v.version, "status": v.status, "artifact_path": v.artifact_path, "metrics": v.metrics,
            "created_at": v.created_at,
        } for v in versions], "production_version": production.version if production else None,
    }
...
async def admin_models_promote (
    request: PromoteRequest, user: UserContext = Depends (require_role (Role.ADMIN)),  # noqa: B008
) -> PromoteResponse:
  ...
  try:
    ...
    raise HTTPException (status_code = 404,
        detail = f"Model '{request.model_name}' version '{request.version}' not found", ) from None
  previous_status = mv.status
  mv = registry.promote (request.model_name, request.version)
  return PromoteResponse (model_name = request.model_name, version = request.version, previous_status = previous_status,
      new_status = mv.status, )


@router.post ("/v1/admin/models/rollback", response_model = RollbackResponse)
async def admin_models_rollback (
    request: RollbackRequest, user: UserContext = Depends (require_role (Role.ADMIN)),  # noqa: B008
) -> RollbackResponse:
  ...
  try:
    ...
    raise HTTPException (status_code = 404, detail = f"Model '{request.model_name}' not found", ) from None
  if current is None:
    raise HTTPException (status_code = 404, detail = f"No production version for model '{request.model_name}'", )
  try:
    previous = registry.rollback (request.model_name)
  except ValueError as e:
    raise HTTPException (status_code = 400, detail = str (e)) from None
  return RollbackResponse (model_name = request.model_name, version = previous.version,
      previous_version = current.version, status = previous.status, )


@router.post ("/v1/admin/models/evaluate", response_model = EvaluateResponse)
async def admin_models_evaluate (
    request: EvaluateRequest, user: UserContext = Depends (require_role (Role.ADMIN)),  # noqa: B008
) -> EvaluateResponse:
  ...
  thresholds = [
      ...
      MetricThreshold ("rouge_l_f1", 0.35, "gte"), MetricThreshold ("eval_loss", 1.0, "lte", severity = "warn"),
  ]
...
async def admin_models_canary_split (
    request: CanarySplitRequest, user: UserContext = Depends (require_role (Role.ADMIN)),  # noqa: B008
) -> CanarySplitResponse:
  ...
  return CanarySplitResponse (model_name = request.model_name, traffic_split = state ["traffic_split"],
      status = state ["phase"], )

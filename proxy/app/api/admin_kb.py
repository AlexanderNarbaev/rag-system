...
async def create_knowledge_base (req: KBCreateRequest):
  ...
  try:
    kb = mgr.create_kb (
        name = req.name, description = req.description, embedding_model = req.embedding_model,
        dense_vector_size = req.dense_vector_size, parser_config = req.parser_config,
    )
    return KBResponse (
        id = kb.id, name = kb.name, description = kb.description, collection_name = kb.collection_name,
        embedding_model = kb.embedding_model, dense_vector_size = kb.dense_vector_size,
        parser_config = kb.parser_config, doc_count = kb.doc_count, chunk_count = kb.chunk_count,
        token_count = kb.token_count, status = kb.status, created_at = kb.created_at, updated_at = kb.updated_at,
    )
...
async def create_etl_task (kb_id: str, req: TaskCreateRequest):
  ...
  return TaskResponse (
      id = task.id, kb_id = task.kb_id, source_type = task.source_type, source_id = task.source_id,
...
async def list_etl_tasks (kb_id: str, status: str | None = None):
  ...
  return TaskListResponse (
      tasks = [
          TaskResponse (
              id = t.id, kb_id = t.kb_id, source_type = t.source_type, source_id = t.source_id,

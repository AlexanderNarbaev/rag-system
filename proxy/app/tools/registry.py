...
from .definition import (
  ToolDefinition, ToolResult, ToolVisibility,
)
...
ROLE_HIERARCHY: dict [str, list [str]] = {
    "admin": ["public", "admin", "expert", "user"], "expert": ["public", "expert", "user"], "user": ["public", "user"],
    "read_only": ["public"],
}
...
class EnhancedToolRegistry:
  ...
  def execute (
      self, name: str, params: dict [str, Any], context: Any = None, ) -> ToolResult:
    ...
    if tool is None:
      return ToolResult (tool_name = name, content = "", error = f"Tool '{name}' not found", )
    
    # Validate required parameters
    missing = self._validate_params (tool, params)
    if missing:
      return ToolResult (tool_name = name, content = "",
          error = f"Missing required parameters: {', '.join (missing)}", )
    
    handler = tool.handler
    if handler is None:
      return ToolResult (tool_name = name, content = "", error = f"Tool '{name}' has no handler", )
    ...
    try:
      ...
      return ToolResult (tool_name = name, content = content, duration_ms = duration_ms, )
    except Exception as exc:
      duration_ms = (time.perf_counter () - start) * 1000
      return ToolResult (tool_name = name, content = "", error = str (exc), duration_ms = duration_ms, )
  ...
  def _validate_params (
      self, tool: ToolDefinition, params: dict [str, Any], ) -> list [str]:
  ...
  def get_tools_for_llm (
      self, provider_type: str = "openai", user_role: str | None = None, ) -> list [dict [str, Any]]:
  ...
  def reload_provider (self, provider_name: str) -> list [ToolDefinition]:
    ...
    provider_registry: dict [str, type [ToolProvider]] = {
        "sdk": SDKProvider, "declarative": DeclarativeProvider, "openapi": OpenAPIProvider,
    }
  ...
  async def discover_all (self) -> dict [str, list [ToolDefinition]]:
    ...
    try:
      ...
      logger.info ("Discovered %d tools from provider '%s'", len (discovered), sdk.provider_name, )
    except Exception as exc:
      logger.error ("Provider '%s' discovery failed: %s", sdk.provider_name, exc, )
    ...
    if os.path.isdir (TOOLS_DECLARATIVE_DIR):
      try:
        ...
        logger.info ("Discovered %d tools from provider '%s'", len (discovered), declarative.provider_name, )
      except Exception as exc:
        logger.error ("Declarative provider discovery failed: %s", exc, )
        results ["declarative"] = []
    ...
    if TOOLS_OPENAPI_SPECS:
      try:
        ...
        logger.info ("Discovered %d tools from provider '%s'", len (discovered), openapi.provider_name, )
      except Exception as exc:
        logger.error ("OpenAPI provider discovery failed: %s", exc, )
        results ["openapi"] = []

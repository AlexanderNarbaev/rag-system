...
def _extract_parameters (
    operation: dict [str, Any], spec: dict [str, Any], ) -> list [ToolParam]:
  ...
  for raw_param in operation.get ("parameters", []):
    ...
    params.append (
        ToolParam (name = name, type = param_type, description = description or f"{location} parameter: {name}",
            required = required and location == "path", default = default_val if default_val is not _UNSET else _UNSET,
            enum = enum_values, ))
    ...
    if request_body is not None:
      ...
      if body_schema.get ("type") == "object" and "properties" in body_schema:
        for prop_name, prop_schema in body_schema ["properties"].items ():
          ...
          params.append (
              ToolParam (name = prop_name, type = prop_type, description = prop_schema.get ("description", ""),
                  required = prop_name in required_list, enum = prop_schema.get ("enum"), ))
  
  return params


def _make_openapi_handler (
    base_url: str, path_template: str, method: str, tool_params: list [ToolParam], ) -> Any:
...
class OpenAPIToolGenerator:
  ...
  @staticmethod
  def from_endpoint (
      path: str, method: str, operation: dict [str, Any], spec: dict [str, Any], base_url: str = "",
      default_visibility: ToolVisibility = ToolVisibility.PUBLIC, ) -> ToolDefinition:
    ...
    return ToolDefinition (name = tool_name, description = description, parameters = params, async_handler = handler,
        category = category, tags = tags, provider = "openapi", visibility = default_visibility, )

...
def _interpolate_variables (
    template: str, params: dict [str, Any], env_vars: dict [str, str], context: dict [str, Any], ) -> str:
  ...
  def _check_params (params: dict [str, Any]) -> str | None:
    for key, value in params.items ():
      ...
      if _has_metacharacters (str_value):
        return (f"Blocked: parameter '{key}' contains forbidden "
  ...
  def _shell_handler (**params: Any) -> str:
    ...
    try:
      result = subprocess.run (resolved_cmd, shell = True,
          ...
          capture_output = True, text = True, timeout = 30, cwd = _working_dir, )
...
def _build_params (raw_params: dict [str, Any]) -> list [ToolParam]:
  ...
  for name, spec in (raw_params or {}).items ():
    ...
    params.append (ToolParam (name = name, type = ptype, description = spec.get ("description", ""),
        required = spec.get ("required", False), default = spec.get ("default", _UNSET), enum = spec.get ("enum"), ))
  return params
...
def _build_retry_policy (raw: dict [str, Any] | None) -> RetryPolicy | None:
  if raw is None:
    return None
  return RetryPolicy (max_retries = raw.get ("max_retries", 3), backoff = raw.get ("backoff", "exponential"),
...
class DeclarativeToolSchema:
  ...
  _SCHEMA: dict [str, Any] = {
      ...
      "properties": {
          "tools": {
              "type": "array", "items": {"$ref": "#/$defs/DeclarativeTool"},
          }
      }, "$defs": {
          "DeclarativeTool": {
              "type": "object", "required": ["name", "type", "description"], "properties": {
                  "name": {"type": "string", "pattern": "^[a-z][a-z0-9_]*$"}, "type": {"enum": ["http", "shell"]},
                  "description": {"type": "string"}, "category": {"type": "string", "default": "declarative"},
                  "tags": {"type": "array", "items": {"type": "string"}},
                  "version": {"type": "string", "default": "1.0.0"},
                  "visibility": {"enum": ["public", "admin", "expert", "user"], "default": "public"},
                  "timeout": {"type": "number", "default": 30}, "retry_policy": {
                      ...
                      "type": "object", "additionalProperties": {
                          "type": "object", "required": ["type"], "properties": {
                              "type": {"type": "string"}, "description": {"type": "string"}, "default": {},
                              "enum": {"type": "array"}, "required": {"type": "boolean", "default": False},
                          },
                      },
                  }, "http": {
                      "type": "object", "required": ["method", "url_template"], "properties": {
                          "method": {"enum": ["GET", "POST", "PUT", "DELETE", "PATCH"]},
                          "url_template": {"type": "string"}, "headers": {"type": "object"},
                          "body_template": {"type": "string"}, "response_path": {"type": "string"},
                          "allowed_hosts": {"type": "array", "items": {"type": "string"}},
                      },
                  }, "shell": {
                      "type": "object", "required": ["command"], "properties": {
                          "command": {"type": "string"}, "working_dir": {"type": "string", "default": "/tmp"},
  ...
  def _parse_single (self, raw: dict [str, Any]) -> ToolDefinition | None:
    ...
    if tool_type == "http":
      ...
      handler = _make_http_handler (method = http_cfg.get ("method", "GET"),
          url_template = http_cfg.get ("url_template", ""), headers = http_cfg.get ("headers"),
          body_template = http_cfg.get ("body_template"), response_path = http_cfg.get ("response_path"),
    ...
    return ToolDefinition (name = raw ["name"], description = raw.get ("description", ""),
        ...
        tags = raw.get ("tags", []), version = raw.get ("version", "1.0.0"), visibility = visibility,
        timeout_seconds = raw.get ("timeout", 30), retry_policy = _build_retry_policy (raw.get ("retry_policy")),
...
class DeclarativeProvider (ToolProvider):
  ...
  async def discover (self) -> list [ToolDefinition]:
    ...
    logger.info ("DeclarativeProvider discovered %d tools from %s", len (tools), TOOLS_DECLARATIVE_DIR, )
    return tools

...
def tool (
    name: str | None = None, description: str | None = None, category: str = "general", tags: list [str] | None = None,
    version: str = "1.0.0", timeout: float = 30.0, retry_policy: RetryPolicy | None = None,
    visibility: ToolVisibility = ToolVisibility.PUBLIC, depends_on: list [str] | None = None, ) -> Callable [
...
class ToolBuilder:
  ...
  def build (self) -> ToolDefinition:
    return ToolDefinition (name = self._name, description = self._description, parameters = self._parameters,
        ...
        timeout_seconds = self._timeout, retry_policy = self._retry_policy, visibility = self._visibility,
        provider = "sdk", )

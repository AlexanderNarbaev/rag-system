...
def run_retrieval_for_queries(
    eval_pairs: list[dict[str, Any]], top_k: int = 20
...
def main():
    parser = argparse.ArgumentParser(
        description="Retrieval Quality Evaluation Pipeline",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--dataset",
        type=str,
        help="Path to labeled JSONL evaluation dataset",
    )
    parser.add_argument(
        "--hitl-dir",
        type=str,
    ...
    parser.add_argument(
        "--gen-template",
        type=str,
        help="Generate a template JSONL file for expert annotation",
    )
    parser.add_argument(
        "--threshold-mrr",
        type=float,

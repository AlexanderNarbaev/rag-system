...
SERVICES = {
    "Qdrant": {
        "url": f"http://{_qdrant_host}:{_qdrant_port}/collections", "method": "GET",
    }, "Neo4j": {
        "url": f"http://{_neo4j_host}:{_neo4j_port}", "method": "GET",
    }, "Redis": {
        "url": f"http://{_redis_host}:{_redis_port}", "method": "GET",
    }, "LLM": {
        "url": f"{_llm_endpoint}/models", "method": "GET",
    },
}
...
def check_service_health (name: str, url: str, method: str = "GET", timeout: float = 3.0) -> dict [str, Any]:
  ...
  try:
    ...
    return {
        "name": name, "status": "healthy" if resp.status_code < 400 else "degraded", "status_code": resp.status_code,
...
class RAGDashboard (App):
  ...
  def action_run_tests (self) -> None:
    ...
    try:
      result = subprocess.run ([sys.executable, "-m", "pytest", "tests/", "-v", "--tb=short"], cwd = str (PROJECT_ROOT),

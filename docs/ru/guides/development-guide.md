...
make run
# Или: granian --interface asgi --host 0.0.0.0 --port 8080 --workers 1 proxy.app.main:app
```
...
Набор тестов **герметичен по умолчанию** — `make test` не требует внешних сервисов:

| Слой | Механизм | Примечания |
|------|----------|------------|
| Юнит-тесты (`tests/proxy`, `tests/etl`) | `unittest.mock` / `MagicMock` / `AsyncMock`; опциональные зависимости внедряются в `sys.modules` | Qdrant, Redis, Neo4j, LLM всегда подменяются; тестируются пути успеха и отказа |
| Интеграционные (`tests/integration`) | In-process FastAPI `TestClient` с подменой auth/RBAC/эмбеддера и синтетическими точками Qdrant | Проверяет компоновку и контракты без контейнеров |
| Пути с живыми сервисами | Управляются переменными окружения: minikube e2e (`make test-minikube`, нужен `RAG_PROXY_URL`) и наборы с маркерами `e2e`/`chaos` | Молча пропускаются без переменных окружения — запускайте явно перед релизами |
| Изоляция полного прогона | Каждая группа тестов выполняется в отдельном процессе pytest через `make test` | Предотвращает загрязнение `sys.modules`/моков между группами |

**Решение по testcontainers:** мы сознательно *не* используем testcontainers/docker-compose внутри юнит-
и интеграционных прогонов. Обоснование: air-gapped CI не может скачивать образы во время тестов, старт
контейнеров доминирует во времени выполнения для ~254 тестовых файлов, а замоканные слои уже покрывают
корректность контрактов. Внедрять testcontainers стоит только если появится класс регрессий, который моки
структурно не ловят (например, изменение реального query DSL Qdrant); тогда включать его за opt-in маркером,
чтобы CI по умолчанию оставался герметичным.

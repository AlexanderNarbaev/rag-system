...
| MinIO Access Key    | `MINIO_ACCESS_KEY`       | `CHANGE_ME`            |
| MinIO Secret Key    | `MINIO_SECRET_KEY`       | `CHANGE_ME`            |
...
| `MINIO_ACCESS_KEY`        | `CHANGE_ME`                 | Ключ доступа MinIO                    |
| `MINIO_SECRET_KEY`        | `CHANGE_ME`                 | Секретный ключ MinIO                  |
...
## Дублирование автономного сервиса — решение о деприкации

`model_evolution_service/` исторически дублировал `proxy/app/model_evolution/` (частично байт-в-байт).
**Решение (аудит 2026-08): единственный источник истины — `proxy/app/model_evolution/`.** Автономное
дерево заморожено для фичевой разработки; изменения переносятся только для зелёного sync-теста паритета
(`tests/proxy/test_model_evolution_eval_gate_sync.py`). Полное удаление помечено `[STRATEGIC_NEEDED]`
и запланировано на будущий релиз; до этого оба дерева получают идентичные дополнения — образец:
`value_score()` в `eval_gate.py`.

...
| `RAG_ADAPTIVE_TOP_K`      | bool | `false`     | Адаптивный top-k по точке колена на скорах реранка; откат к статическому `rag_top_k` на плоских/коротких кривых (FR-178). |
| `RAG_ADAPTIVE_TOP_K_MIN`  | int | `3`          | Нижняя граница числа результатов при включённом адаптивном top-k. |

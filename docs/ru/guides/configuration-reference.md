...
Полный справочник всех параметров конфигурации RAG-системы. Включает переменные окружения прокси (`proxy/.env`), настройки ETL-пайплайна (`etl/config/etl_config.yaml`) и переопределения Docker Compose.
...
SLM обрабатывает лёгкие задачи: классификация намерений, декомпозиция запросов, извлечение сущностей. Оставьте `SLM_ENDPOINT` пустым для отключения (используется эвристический фолбэк).

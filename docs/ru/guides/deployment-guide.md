...
!!! note
    Это абсолютный минимум для CPU-only режима разработки с небольшой моделью. Для продакшн-нагрузок с GPU-инференсом см. [полные требования](#1-предварительные-требования) (16+ ГБ RAM, 8+ ядер, GPU рекомендуется).

...
Запустите корпоративный ассистент знаний RAG за 5 минут. Руководство охватывает локальную настройку для разработки с
...
NVIDIA GPU с 12+ ГБ VRAM рекомендуется для генерации LLM. Без GPU система работает в режиме CPU с значительно меньшей
скоростью ответов. Также можно указать удалённую LLM-конечную точку (например, GPU-сервер в вашей сети).
...
Если у вас ещё нет LLM-бэкенда, система запустится, но запросы на генерацию будут возвращать ошибки. Можно проверить
эндпоинты состояния и изучить API. Инструкции по настройке LLM см.
в [Руководстве по развёртыванию](deployment-guide.md).
...
Виджет предоставляет готовый интерфейс чата, который можно встроить на любую веб-страницу. Больше вариантов интеграции
см. в [Примерах API](api-examples.md).

...
Модель с суффиксом `+RAG` (например, `qwen3-635b+RAG`) активирует RAG-пайплайн.
Модель без суффикса — прямой прокси к LLM без поиска.
...
1. `curl -X POST /v1/chat/completions -d '{"messages":[{"role":"user","content":"test"}],"stream":false}'` возвращает
   200 с JSON `{choices: [{message: {content: "..."}}]}`
...
**Описание:**
Эндпоинт `GET /v1/models` возвращает список моделей из `AVAILABLE_MODELS`.
Для каждой модели показывается версия с суффиксом `+RAG` (с RAG-пайплайном)
и без суффикса (прямой прокси).
...
> **Примечание:** Поля `rag_sources`, `rag_confidence`, `rag_feedback_id` — расширения
> к стандартному OpenAI формату. Стандартные клиенты (OpenWebUI) их игнорируют, но они
> доступны через API.

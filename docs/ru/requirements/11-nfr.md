...
| CON-29 | **Standard OpenAI protocol first.** Все RAG-расширения реализуются через стандартные механизмы: `model` поле (суффикс +RAG), HTTP заголовки (X-User-ID, X-Forwarded-User), `user` поле запроса. Неиспользуемые стандартными клиентами расширения допустимы в ответе (rag_sources, rag_confidence) но не в запросе. | Drop-in compatibility      |

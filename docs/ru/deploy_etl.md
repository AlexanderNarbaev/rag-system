...
ETL-пайплайн работает на выделенной машине (или на той же машине, что и прокси, в небольших развёртываниях). Он
извлекает данные из Confluence, Jira, GitLab и других источников, разбивает документы семантически на чанки, извлекает
сущности для графа знаний и индексирует всё в Qdrant.
...
**Восстановление:** Если ETL аварийно завершился, удалите `wal/etl_wal.json` для принудительной полной переиндексации
или запустите с `--sources` для пропуска завершённых источников.
...
Семантический чанкер (`MDKeyChunker`) разбивает документы по заголовкам и разделам markdown, соблюдая структуру
документа. Он создаёт чанки, которые:
...
Это удаляет все существующие чанки, очищает WAL и заново извлекает всё с нуля. **Внимание:** это может занять часы в

...
**Architecture:** The existing `proxy/app/core/ragas_eval.py` already implements `compute_faithfulness`, `compute_answer_relevance`, `compute_context_relevance`, and `evaluate_rag_response`. We need to wire these into the feedback endpoint and the main RAG pipeline.

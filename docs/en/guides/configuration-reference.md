...
Complete reference for all configuration options in the RAG System. Covers proxy environment variables (`proxy/.env`), ETL pipeline settings (`etl/config/etl_config.yaml`), and Docker Compose overrides.
...
SLM handles lightweight tasks: intent classification, query decomposition, entity extraction. Leave `SLM_ENDPOINT` empty to disable (heuristic fallback is used).
...
| `MINIO_ACCESS_KEY` | string | `minioadmin` | MinIO access key. **Change in production.** |
| `MINIO_SECRET_KEY` | string | `minioadmin` | MinIO secret key. **Change in production.** |

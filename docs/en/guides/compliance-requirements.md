...
**Version:** v1.2 | **Date:** 2026-07-18 | **Scope:** Full system (Proxy + ETL + MCP + Model Evolution + Dashboard)
...
| FR-10 | The system SHALL use cross-encoder reranking (BGE-Reranker-v2-m3, batch_size=32) on top-N retrieval candidates after hybrid search | ADR-002, best-practices-checklist 6.3 | CRITICAL | MET (W18): BGE-Reranker-v2-m3 replaces MiniLM-L-6-v2; multi-language relevance scoring across 100+ languages; verified via rerank accuracy benchmarks in `test_rerank.py` |
...
| FR-42 | The ETL pipeline SHALL chunk documents using semantic chunking with configurable parameters (size 256-1024 tokens, p50=512, p95=900, overlap 10-15%) | performance-quality 4.1, roadmap Phase 5 | CRITICAL | MET (W18): semantic chunker now includes contextual retrieval — each chunk embeds surrounding paragraph context for improved retrieval relevance; verified via chunk coherence and contextual overlap tests in `test_chunker.py` |
| FR-43 | The ETL pipeline SHALL support adaptive chunking | roadmap Phase 5 | HIGH | MET (W18): adaptive chunking enhanced with contextual retrieval — chunk boundaries now account for surrounding document context, producing more coherent retrieval units; verified via adaptive boundary tests in `test_chunker.py` |
...
| **NOT MET** | 158   | All other FRs (175 total - 16 MET - 1 PARTIAL = 158 remaining); see individual verification columns for current status |

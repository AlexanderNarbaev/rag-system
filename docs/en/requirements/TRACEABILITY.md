...
| **Status**         | ✅ Confirmed / ⚠️ Integration needed / ❌ Implementation needed |
...
| ID    | Description                    | Implementation                                   | Tests             | Status          |
|-------|--------------------------------|--------------------------------------------------|-------------------|-----------------|
| FR-19 | Entity extraction              | `etl/graph_builder/entity_extractor.py`          | Neo4j integration required | ⚠️              |
| FR-20 | Batch loading into Neo4j       | `etl/graph_builder/neo4j_loader.py`              | Neo4j integration required | ⚠️              |
| FR-21 | Multi-hop graph traversal      | `proxy/app/core/retrieval.py`                    | Neo4j integration required | ⚠️              |
| FR-22 | Global / Multi-Hop / Text2Cypher | `proxy/app/core/retrieval.py`                  | Neo4j integration required | ⚠️              |
| FR-23 | Community Detection            | `etl/graph_builder/community.py`                 | Neo4j integration required | ⚠️              |
| FR-24 | Graceful degradation Neo4j     | `proxy/app/core/retrieval.py`                    | Neo4j integration required | ⚠️              |
| FR-25 | Graph schema versioning        | (no implementation)                              | —                 | ❌              |

## Agentic (FR-26 — FR-31)

| ID    | Description                    | Implementation                              | Tests                          | Status          |
|-------|--------------------------------|---------------------------------------------|--------------------------------|-----------------|
| FR-26 | LangGraph 10-node graph        | `proxy/app/core/orchestrator/graph.py`      | LangGraph integration required | ⚠️              |
| FR-27 | Query rewriting                | `proxy/app/core/orchestrator/nodes.py`      | LangGraph integration required | ⚠️              |
| FR-28 | Retrieval sufficiency loop    | `proxy/app/core/orchestrator/nodes.py`      | LangGraph integration required | ⚠️              |
| FR-29 | Fallback to linear pipeline    | `proxy/app/main.py`                         | LangGraph integration required | ⚠️              |
| FR-30 | Tool/function calling          | `proxy/app/tools/`                          | LangGraph integration required | ⚠️              |
| FR-31 | Parallel execution             | `proxy/app/tools/orchestrator.py`           | LangGraph integration required | ⚠️              |
...
| FR-36 | Hallucination detection           | `proxy/app/core/hallucination.py`| `tests/proxy/test_quality_pipeline.py::TestFR36HallucinationDetection` | ✅      |
...
| ID    | Description                       | Implementation                                        | Tests                                                                                                  | Status |
|-------|-----------------------------------|-------------------------------------------------------|--------------------------------------------------------------------------------------------------------|--------|
| FR-40 | Extraction from 6 sources         | `etl/extractors/{confluence,jira,gitlab,docs,books,chats}.py` | `tests/etl/test_etl_requirements.py::TestFR40Extractors`, `tests/etl/test_extractors.py`     | ✅      |
...
| FR-87b | User identification via headers  | (implementation required)                   | —                                                                                                      | ❌ (new) |
| FR-88 | RBAC — 4 roles                    | `proxy/app/auth/rbac.py`                     | `tests/proxy/test_auth_rbac.py::TestFR88RBAC`, `tests/integration/test_auth_flow.py::TestRBACEnforcement` | ✅      |
| FR-89 | ACL in Qdrant                     | `proxy/app/shared/access_control.py`        | `tests/proxy/test_auth_rbac.py::TestFR89ACLQdrant`                                                     | ✅      |
...
| FR-166 | Disaster recovery runbook         | `docs/en/guides/disaster-recovery-runbook.md` | (documentation)                                      | ✅      |
...
| ID    | Description                       | Implementation                      | Tests                                                          | Status |
|-------|-----------------------------------|-------------------------------------|----------------------------------------------------------------|--------|
| FR-168 | Qdrant scalar quantization (INT8) | `scripts/init_collections.py`       | settings verification required                                 | ⚠️      |
| FR-169 | Qdrant gRPC client                | `proxy/app/core/retrieval.py`       | settings verification required                                 | ⚠️      |
| FR-170 | vLLM prefix caching               | (external — vLLM config)            | partial implementation (gauge added, monitoring needed)        | ⚠️ Partial |
| FR-171 | HNSW tuning                       | `scripts/init_collections.py`       | settings verification required                                 | ⚠️      |
...
| ID      | Description                     | Implementation                      | Tests                                                       | Status |
|---------|---------------------------------|-------------------------------------|-------------------------------------------------------------|--------|
| NFR-P01 | End-to-end latency p95 < 5s     | `proxy/app/api/chat.py`, metrics     | `tests/performance/test_nfr_benchmarks.py`                   | ✅      |
...
| NFR-P11 | Response compression ≥ 60%       | `proxy/app/shared/middleware.py`    | (built into granian)                                          | ✅      |
...
| Category      | Total specifications | ✅ Confirmed | ⚠️ Integration needed | ❌ Implementation needed |
|---------------|----------------------|--------------|-----------------------|--------------------------|
| FR (Core API) | 8                    | 8            | 0                     | 0                        |
...
> Note: `FR-87b` is marked as "new" — this requirement was added to the specification
> but is not yet implemented. Code is required in `proxy/app/auth/user_identification.py`.

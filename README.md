...
**EN:** **v0.6** — Streaming ETL (Redis Streams), webhook-driven ingestion, model warm-up, SSE TTFT optimization, gzip/brotli compression, and all v0.5 features (multi-modal RAG, ColBERT, cold storage cleanup, reranker fine-tuning). 1248 tests passing. See [ADR documents](docs/adr/) for architecture decisions and [C4 diagrams](docs/diagrams/) for visual architecture.

**RU:** **v0.6** — Потоковый ETL (Redis Streams), приём через вебхуки, прогрев моделей, оптимизация SSE TTFT, сжатие gzip/brotli, а также все возможности v0.5 (мультимодальный RAG, ColBERT, очистка холодного хранилища, дообучение реранкера). 1248 тестов проходят. См. [ADR-документы](docs/adr/) с архитектурными решениями и [C4-диаграммы](docs/diagrams/) с визуальной архитектурой.
...
This project implements **Agentic+ RAG** (Level 4+).
The LangGraph orchestrator (`proxy/app/orchestrator.py`) provides agentic capabilities. Streaming ETL, webhook ingestion, model warm-up, and response compression are all operational.
...
Проект реализует **Агентный+ RAG** (уровень 4+).
LangGraph-оркестратор (`proxy/app/orchestrator.py`) предоставляет агентные возможности. Потоковый ETL, приём через вебхуки, прогрев моделей и сжатие ответов работают в production.

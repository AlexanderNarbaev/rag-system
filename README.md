...
**EN:** **v1.0 (GA)** — Production-ready RAG system: K8s Helm chart (HPA, probes, secrets), Grafana dashboards, Prometheus alert rules, SLI/SLO definitions, HA deployment (Qdrant replication, Neo4j cluster, Redis Sentinel), backup automation (S3/MinIO), DR runbook, zero-downtime deployment, E2E test suite, chaos/resilience testing, 1321 tests passing. See [ADR documents](docs/adr/) for architecture decisions and [C4 diagrams](docs/diagrams/) for visual architecture.

**RU:** **v1.0 (GA)** — Production-готовая RAG-система: K8s Helm-чарт (HPA, пробы, секреты), Grafana-дашборды, правила алертов Prometheus, определения SLI/SLO, HA-развёртывание (репликация Qdrant, кластер Neo4j, Redis Sentinel), автоматизация бэкапов (S3/MinIO), DR runbook, развёртывание без простоя, E2E-тесты, chaos/отказоустойчивое тестирование, 1321 тест проходят. См. [ADR-документы](docs/adr/) с архитектурными решениями и [C4-диаграммы](docs/diagrams/) с визуальной архитектурой.
...
**EN:** v1.0 adds K8s Helm chart, Grafana dashboards, Prometheus alert rules, SLI/SLO definitions, HA deployment, backup automation, DR runbook, zero-downtime deployment, and E2E/chaos test suites. 1321 tests pass at 100%.

**RU:** v1.0 добавляет K8s Helm-чарт, Grafana-дашборды, правила алертов Prometheus, определения SLI/SLO, HA-развёртывание, автоматизацию бэкапов, DR runbook, развёртывание без простоя и E2E/chaos-тесты. 1321 тест проходит со 100% результатом.
...
| **Agentic+** | Streaming ETL | Real-time indexing | Webhook-driven | NLI grounding |

This project implements **Agentic+ RAG** (Level 4+) with partial Level 5 (Self-Correcting) capabilities.
The LangGraph orchestrator (`proxy/app/orchestrator.py`) provides agentic capabilities. NLI grounding, CRAG decomposition, self-critique, and evaluation pipeline provide emerging self-correction. Production-ready with K8s HA deployment, automated backups, DR runbook, and comprehensive monitoring.
...
| **Агентный+** | Потоковый ETL | Индексация в реальном времени | Приём через вебхуки | NLI-заземление |

Проект реализует **Агентный+ RAG** (уровень 4+) с частичными возможностями уровня 5 (Self-Correcting).
LangGraph-оркестратор (`proxy/app/orchestrator.py`) предоставляет агентные возможности. NLI-заземление, CRAG-декомпозиция, самокритика и оценочный конвейер обеспечивают зарождающуюся самокоррекцию. Production-готовность с K8s HA-развёртыванием, автоматическими бэкапами, DR runbook и комплексным мониторингом.

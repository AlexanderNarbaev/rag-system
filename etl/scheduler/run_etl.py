    ...
    if source_name == "confluence":
      ...
      for jira_dir in source_dir.glob ("*"):
        ...
        if issue_file.exists ():
          ...
          documents.append ({
              "id": f"jira_{data ['key']}", "source_type": "jira", "title": data.get ("summary", ""),
              "content": content, "content_type": "html", "metadata": {
      ...
      for gitlab_dir in source_dir.glob ("*"):
        ...
        if commits_file.exists ():
          ...
          for commit in commits [:100]:
            ...
            documents.append ({
                ...
                "metadata": {
                    "sha": commit ["id"], "author": commit.get ("author_name", ""),
                    "date": commit.get ("created_at", ""), "project_id": project_id, "namespace": namespace,
        ...
        if mr_file.exists ():
          ...
          for mr in mrs:
            ...
            documents.append ({
                "id": f"gitlab_mr_{mr ['iid']}", "source_type": "gitlab_merge_request", "title": mr.get ("title", ""),
...
def run_chunking (documents: list [dict], chunker: MDKeyChunker, output_dir: Path):
  ...
  for i, doc in enumerate (documents):
    if _shutdown_requested:
      ...
      break
    source_metadata = {
        "source_type": doc ["source_type"], "source_id": doc ["id"],
        "version": doc ["metadata"].get ("version", "latest"), "doc_title": doc ["title"],
    }
    try:
...
def run_graph_extraction (
    chunks: list [dict], entity_extractor: EntityRelationExtractor, neo4j_loader: Neo4jLoader | None, ):
...
def main ():
  ...
  parser.add_argument ("--streaming", action = "store_true",
      help = "Start streaming ETL (webhook + consumer) alongside batch")  # noqa: E501
  ...
  if not args.skip_chunk:
    ...
    base_chunker = SemanticChunker (max_tokens = chunker_config.get ("max_tokens", 1500),
        overlap_tokens = chunker_config.get ("overlap_tokens", 200),
        min_chunk_tokens = chunker_config.get ("min_chunk_tokens", 100), )
    enricher = MetadataEnricher (use_slm = chunker_config.get ("use_slm", False),
  ...
  if not args.skip_graph and config.get ("graph", {}).get ("enabled", False):
    ...
    entity_extractor = EntityRelationExtractor (use_spacy = graph_config.get ("use_spacy", True),
        spacy_model = graph_config.get ("spacy_model", "ru_core_news_sm"),
        use_slm = graph_config.get ("use_slm", False), slm_endpoint = graph_config.get ("slm_endpoint"),
    ...
    if neo4j_config.get ("enabled", False):
      neo4j_loader = Neo4jLoader (uri = neo4j_config ["uri"], user = neo4j_config ["user"],
          password = neo4j_config ["password"], database = neo4j_config.get ("database", "neo4j"), )
  ...
  if not args.skip_index:
    ...
    qdrant_idx = QdrantHybridIndexer (host = index_config.get ("qdrant_host", "localhost"),
    ...
    live_lake = LiveVectorLake (qdrant_indexer = qdrant_idx, version_store = version_store,

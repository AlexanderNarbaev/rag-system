...
class StreamingPipeline:
  ...
  def __init__ (
      self, config: dict [str, Any], wal: Any, shutdown_event: threading.Event | None = None, ):
    self._config = config
    self._wal = wal
  ...
  def _create_embedder (self) -> Any:
    ...
    retry_cfg = RetryConfig (max_attempts = embedder_cfg.get ("max_retries", 5),
    ...
    return RemoteEmbedder (endpoint = endpoint, model = embedder_cfg.get ("model", ""),
                           api_key = embedder_cfg.get ("api_key", remote_cfg.get ("api_key", "")),
                           timeout = embedder_cfg.get ("timeout", 60),
                           max_batch_size = embedder_cfg.get ("batch_size", 64), retry_config = retry_cfg,
  ...
  def _create_indexer (self) -> Any:
    ...
    return QdrantHybridIndexer (host = index_cfg.get ("qdrant_host", "localhost"),
                                ...
                                batch_size = index_cfg.get ("batch_size", 100), embedder = self._embedder, )
  ...
  def _create_chunker (self) -> Any:
    ...
    base_chunker = SemanticChunker (max_tokens = max_tokens, overlap_tokens = chunker_cfg.get ("overlap_tokens", 200),
                                    min_chunk_tokens = chunker_cfg.get ("min_chunk_tokens", 100), )
    enricher = MetadataEnricher (use_slm = chunker_cfg.get ("use_slm", False),
  ...
  def _resolve_chunk_max_tokens (self, chunker_cfg: dict) -> int:
    ...
    if model:
      ...
      logger.info ("Auto-detected chunk max_tokens=%d from embedder model '%s'", resolved, model, )
      return resolved
  ...
  async def _process_chunk (self, chunk_dict: dict [str, Any]) -> bool:
    ...
    async with sem:
      try:
        ...
        dense_vec = await loop.run_in_executor (None, lambda: self._embedder.encode (chunk_dict ["text"],
  ...
  async def _enrich_chunks (self, chunk_dicts: list [dict [str, Any]]) -> None:
    ...
    async def _enrich_one (ch: dict [str, Any]) -> None:
      async with sem:
        try:
          loop = asyncio.get_running_loop ()
          result = await loop.run_in_executor (None, self._enricher.enrich, ch.get ("text", ""),
                                               ch.get ("metadata", {}), )
    ...
    try:
      ...
      if not result:
        ...
        if point is not None:
          self._indexer.client.upsert (collection_name = self._indexer.collection_name, points = [point], )
          return True
        return False
      return True
    except Exception as e:
  ...
  async def process_document (self, doc: dict [str, Any]) -> StreamingResult:
    ...
    result = StreamingResult (source = doc.get ("source_type", "unknown"), doc_id = doc.get ("id", "unknown"), )
    errors: list [str] = []
    
    source_metadata = {
        "source_type": doc.get ("source_type", ""), "source_id": doc.get ("id", ""),
        "version": doc.get ("metadata", {}).get ("version", "latest"), "doc_title": doc.get ("title", ""),
    }
    
    try:
      chunks = self._chunker.process_document (doc ["content"], doc.get ("content_type", "html"), source_metadata, )
    except Exception as e:
    ...
    if self._quality_filter is not None and all_chunk_dicts:
      ...
      all_chunk_dicts, _stats = self._quality_filter.filter (doc_title = doc.get ("title", ""),
                                                             chunks = all_chunk_dicts, )
      filtered_out = original_count - len (all_chunk_dicts)
      if filtered_out:
        logger.info ("Quality filter: %d/%d chunks kept for '%s' (%d filtered out)", len (all_chunk_dicts),
                     original_count, doc ["title"] [:80], filtered_out, )
      result.chunks_count = len (all_chunk_dicts)
    
    # Heading-level and document-level chunks for Confluence
    if doc.get ("source_type") == "confluence" and doc.get ("content_type") == "html":
      try:
        h_chunks = self._chunker.base.create_heading_chunks (doc ["content"], source_metadata, )
    ...
    if self._progress.processed_docs % self._progress_interval == 0:
      logger.info ("Progress: %d/%d docs, %d chunks, %d indexed, %d errors (%.1f%%)", self._progress.processed_docs,
                   self._progress.total_docs, self._progress.total_chunks, self._progress.indexed_chunks,
                   self._progress.errors, self._progress.progress_pct, )
  ...
  def _extract_confluence_docs (source_dir: Path) -> Any:
    for conflu_dir in source_dir.glob ("*"):
      ...
      with open (page_file, encoding = "utf-8") as f:
        data = json.load (f)
      yield {
          "id": f"confluence_{data ['id']}", "source_type": "confluence", "title": data.get ("title", ""),
          "content": data.get ("body_view_html", "") or data.get ("body_storage_raw", ""), "content_type": "html",
          "metadata": {
              "version": str (data.get ("version", "")), "space": data.get ("space", ""),
              "space_key": data.get ("space_key", ""), "page_id": data.get ("id", ""),
              "author": data.get ("author", ""), "contributors": data.get ("contributors", []),
              "labels": data.get ("labels", []), "restrictions": data.get ("restrictions", {}),
        ...
        for comment in comments:
          ...
          if body:
            ...
            if content:
              ...
              content = f"\n\nComment by {author_str}: {body}"
      yield {
          "id": f"jira_{data ['key']}", "source_type": "jira", "title": data.get ("summary", ""), "content": content,
          "content_type": "html", "metadata": {
              "key": data ["key"], "status": data.get ("status", ""), "priority": data.get ("priority", ""),
              "assignee": data.get ("assignee", ""), "reporter": data.get ("reporter", ""),
              "project_key": data.get ("project_key", ""), "issue_type": data.get ("issue_type", ""),
              "labels": data.get ("labels", []), "components": data.get ("components", []),
    ...
    for gitlab_dir in source_dir.glob ("*"):
      ...
      if commits_file.exists ():
        ...
        for commit in commits [:100]:
          ...
          yield {
              "id": f"gitlab_commit_{commit ['id']}", "source_type": "gitlab_commit",
              "title": commit.get ("title", commit ["id"] [:8]), "content": content, "content_type": "markdown",
              "metadata": {
                  "sha": commit ["id"], "author": commit.get ("author_name", ""), "date": commit.get ("created_at", ""),
                  "project_id": project_id, "namespace": namespace, "visibility": visibility,
              },
          }
      ...
      if mr_file.exists ():
        with open (mr_file, encoding = "utf-8") as f:
          mrs = json.load (f)
        for mr in mrs:
          mr_title_str = str (mr.get ("title", "")) if mr.get ("title", "") else ""
          mr_desc_str = str (mr.get ("description", "")) if mr.get ("description", "") else ""
          content = mr_title_str + "\n" + mr_desc_str
              ...
              for note in disc.get ("notes", []):
                if note:
                  if content:
                    ...
                    content = f"\n{note ['author']}: {note ['body']}"
          yield {
              "id": f"gitlab_mr_{mr ['iid']}", "source_type": "gitlab_merge_request", "title": mr_title_str,
              "content": content, "content_type": "markdown", "metadata": {
                  "iid": mr ["iid"], "state": mr.get ("state", ""),
                  "author": mr.get ("author", {}).get ("username", ""), "project_id": project_id,
      ...
      if files_dir.exists ():
        for code_file in files_dir.glob ("*.txt"):
          ...
          yield {
              "id": f"gitlab_file_{code_file.stem}", "source_type": "gitlab_code", "title": code_file.stem,
              "content": content, "content_type": "plaintext", "metadata": {
  ...
  async def run (self) -> AsyncIterator [StreamingResult]:
    ...
    logger.info ("Streaming pipeline starting: %d documents, %d concurrent API calls", self._progress.total_docs,
    ...
    for doc in docs:
      if self._shutdown_event and self._shutdown_event.is_set ():
        logger.warning ("Shutdown requested, stopping at doc %d/%d", self._progress.processed_docs,
    ...
    logger.info ("Streaming pipeline complete: %d docs, %d chunks, %d indexed, %d errors",
                 self._progress.processed_docs, self._progress.total_chunks, self._progress.indexed_chunks,
...
async def main_streaming (config: dict [str, Any], wal: Any) -> None:
  ...
  async for result in pipeline.run ():
    ...
    logger.log (level, "Processed: %s doc %s — %d chunks, %d indexed, %d errors (%.0fms)", result.source, result.doc_id,
                result.chunks_count, result.chunks_indexed, len (result.errors), result.duration_ms, )

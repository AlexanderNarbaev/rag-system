...
class JiraExtractor:
  ...
  def _paginated_issues (self, jql: str, start_at: int = 0, max_results: int = 100) -> Iterator [dict]:
    ...
    while True:
      params = {
          "jql": jql, "fields": self.fields, "expand": self.expand, "startAt": start_at, "maxResults": max_results,
      }
      data = self._request ("/rest/api/2/search", params)
    ...
    for attempt in range (max_retries + 1):
      try:
        ...
        if attempt < max_retries:
          ...
          logger.error (f"Failed to download attachment {attachment_id} for {issue_key}"
  ...
  def _process_issue (self, issue: dict) -> dict:
    ...
    for com in comments_data:
      ...
      comments.append ({
          "id": com ["id"], "author": com.get ("author", {}).get ("displayName", ""),
    ...
    if self.download_attachments:
      for att in fields.get ("attachment", []):
        ...
        attachments.append ({
            ...
            "author": att.get ("author", {}).get ("displayName", ""), "local_path": local_path,
        })
    else:
      attachments = [{
          "id": att ["id"], "filename": att ["filename"], "size": att.get ("size", 0),
    ...
    result = {
        ...
        "issuetype": issuetype, "issue_type": issuetype, "created": created, "updated": updated,
        "resolution": resolution, "assignee": assignee, "reporter": reporter, "project_key": project_key,
        "labels": labels,
        "components": [{"id": c.get ("id"), "name": c.get ("name", "")} for c in fields.get ("components", [])],
        "sprints": sprints, "comments": comments, "changelog": changelog_entries, "attachments": attachments,
        "links": links, "subtasks": subtasks, "description_links": description_links,
...
if __name__ == "__main__":
  ...
  config_example = {
      "url": os.getenv ("JIRA_URL", "https://jira.example.com"), "username": os.getenv ("JIRA_USER", "bot"),
      "token": os.getenv ("JIRA_TOKEN", "your_token"),
      "jql": "project in (DEV, OPS) AND status not in (Closed, Resolved) ORDER BY updated DESC",
      "output_dir": "./raw_data/jira", "wal_file": "./wal/jira_wal.json", "incremental": True,
      "download_attachments": True, "max_issues_per_run": 0, "since_date": "2025-01-01T00:00:00.000+0000",
      "fields": "*all", "expand": "changelog,renderedBody",
  }

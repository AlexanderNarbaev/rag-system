...
class ConfluenceExtractor:
  ...
  def test_connection (self) -> bool:
    ...
    try:
      resp = self.session.get (urljoin (self.url, "/rest/api/content"), params = {"limit": 1}, timeout = (10, 30), )
  ...
  def _get_all_pages (self, space_key: str | None = None, start: int = 0, limit: int = 50) -> list [dict[str, Any]]:
    ...
    while True:
      params = {
          "limit": limit, "start": start, "expand": "version,space",  # Без body — быстрее и не зависает
      }
      if space_key:
  ...
  def extract_page (self, page: dict) -> dict[str, Any]:
    ...
    for att in attachments_meta:
      ...
      att_info = {
          "id": att_id, "filename": att_filename, "media_type": att.get ("mediaType", "application/octet-stream"),
          "size": att.get ("fileSize", 0), "version": att.get ("version", {}).get ("number", 1),
    ...
    page_data = {
        ...
        "created_at": created, "updated_at": updated, "body_storage_raw": body_storage, "body_view_html": body_view,
        "links": links, "versions": version_list, "comments": comment_data, "attachments": attachment_data,
      ...
      for page in pages:
        ...
        try:
          ...
          logger.error (f"Failed to process page {page_id}: {e}",
                        exc_info = True)  # Продолжаем, не прерываем весь процесс
...
if __name__ == "__main__":
  ...
  config_example = {
      ...
      "username": os.getenv ("CONFLUENCE_USER", "bot"), "token": os.getenv ("CONFLUENCE_TOKEN", "your_token"),
      "space_keys": ["DEV", "OPS"],  # или None для всех
      "output_dir": "./raw_data/confluence", "wal_file": "./wal/confluence_wal.json", "incremental": True,
      "download_attachments": True, "max_versions": 0, "api_version": "2",
  }

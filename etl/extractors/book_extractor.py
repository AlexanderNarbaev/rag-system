...
class BookExtractor (BaseExtractor):
    ...
    for item in book.get_items_of_type (ITEM_DOCUMENT):
      ...
      if heading_info:
        for heading_text, heading_level, section_text in heading_info:
          ...
          chapters.append ({
              "title": section_title, "content": section_text, "level": heading_level, "index": section_counter,
          })
      else:
        section_counter += 1
        chapters.append ({
            "title": f"Chapter {section_counter}", "content": cleaned, "level": 1, "index": section_counter,
        })
    
    for ch in chapters:
      doc = ExtractedDocument (source_id = f"book_epub_{file_path.stem}_ch{ch ['index']}", source_type = "book",
          title = f"{title} — {ch ['title']}", content = ch ["content"], content_type = "text", metadata = {
              "book_title": title, "author": author, "isbn": isbn, "format": "epub", "chapter": ch ["title"],
              "chapter_level": ch ["level"], "chapter_index": ch ["index"], "file_path": str (file_path),
  ...
  async def _extract_pdf (self, file_path: Path) -> list [ExtractedDocument]:
    ...
    for idx, (section_title, section_text) in enumerate (sections):
      doc = ExtractedDocument (source_id = f"book_pdf_{file_path.stem}_s{idx}", source_type = "book",
          title = f"{title} — {section_title or f'Section {idx + 1}'}", content = section_text, content_type = "text",
          metadata = {
              "book_title": title, "author": author, "format": "pdf", "total_pages": len (reader.pages),
              "section_title": section_title, "section_index": idx, "file_path": str (file_path),
          }, )
      documents.append (doc)
  ...
  async def _extract_docx (self, file_path: Path) -> list [ExtractedDocument]:
    ...
    for idx, (ch_title, ch_content) in enumerate (chapters):
      doc = ExtractedDocument (source_id = f"book_docx_{file_path.stem}_ch{idx}", source_type = "book",
          title = f"{title} — {ch_title}", content = ch_content, content_type = "text", metadata = {
              "book_title": title, "author": author, "format": "docx", "chapter": ch_title, "chapter_index": idx,
  ...
  def _split_into_sections (text: str) -> list [tuple [str, str]]:
    ...
    chapter_patterns = [
        r"^(?:Chapter|CHAPTER|Глава|ГЛАВА)\s+\d+", r"^(?:Section|SECTION|Раздел)\s+\d+", r"^\d+\.\s+\w",
        r"^(?:Part|PART|Часть)\s+\d+", r"^[IVXLC]+\.\s+\w",
    ]

  ...
  def extract_entities_spacy (self, text: str) -> list [Entity]:
    ...
    for ent in doc.ents:
      ...
      if name and name not in seen:
        ...
        type_map = {
            "PERSON": "PERSON", "ORG": "ORGANIZATION", "GPE": "LOCATION", "LOC": "LOCATION", "PRODUCT": "PRODUCT",
            "EVENT": "EVENT", "WORK_OF_ART": "PRODUCT",
        }
    ...
    try:
      ...
      if response.status_code == 200:
        ...
        if "choices" in result:
          output = (
              result ["choices"] [0].get ("text", "") if "text" in result ["choices"] [0] else result ["choices"] [
        ...
        for rel in relations_data:
          ...
          if source_entity and target_entity:
            relations.append (Relation (source = source_entity.id, target = target_entity.id, type = rel_type,
...
def entities_to_cypher (entities: list [Entity], relations: list [Relation]) -> list [str]:
  ...
  for rel in relations:
    queries.append (f"MATCH (a {{id: '{rel.source}'}}), (b {{id: '{rel.target}'}}) "
...
if __name__ == "__main__":
  ...
  sample_text = """
    ...
    """
  extractor = EntityRelationExtractor (use_spacy = True, use_slm = False,  # для теста без SLM

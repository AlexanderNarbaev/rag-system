...
class CommunityDetector:
  ...
  def detect_communities (
      self, entities: list [dict [str, Any]], relationships: list [dict [str, Any]], ) -> list [Community]:
    """
    ...
    for entity_id in adj:
      ...
      if len (component) >= self.min_community_size:
        ...
        community = Community (id = f"community_{len (communities)}", level = 0, members = component,
            key_entities = key_entities, relationships = community_rels, metadata = {"size": len (component)}, )
  ...
  def _default_summary (self, community: Community, descriptions: list [str]) -> str:
    ...
    return (f"Community {community.id} contains {len (community.members)} entities. "
  ...
  def save_communities (self, communities: list [Community], path: str) -> None:
    ...
    for comm in communities:
      data.append ({
          "id": comm.id, "level": comm.level, "members": comm.members, "summary": comm.summary,
          "key_entities": comm.key_entities, "metadata": comm.metadata,
      })

...
class LiveVectorLake:
  ...
  def __init__ (
      self, qdrant_indexer: QdrantHybridIndexer, version_store: ChunkVersionStore, cold_storage_dir: Path,
  ...
  def _append_to_cold_storage (self, doc_id: str, chunks: list [dict], operation: str = "upsert"):
    ...
    for chunk in chunks:
      record = {
          "doc_id": doc_id, "timestamp": timestamp, "operation": operation, "chunk_hash": chunk.get ("hash"),
          "text": chunk.get ("text"), "title": chunk.get ("title"), "source_type": chunk.get ("source_type"),
          "source_id": chunk.get ("source_id"), "version": chunk.get ("version"), "doc_title": chunk.get ("doc_title"),
          "keywords": json.dumps (chunk.get ("keywords", [])), "entities": json.dumps (chunk.get ("entities", [])),
    ...
    for _, row in snapshot.iterrows ():
      if row ["operation"] == "upsert" and row ["chunk_hash"] not in seen_hashes:
        chunks_snapshot.append ({
            "hash": row ["chunk_hash"], "text": row ["text"], "title": row ["title"],
            "source_type": row ["source_type"], "source_id": row ["source_id"], "version": row ["version"],
            "doc_title": row ["doc_title"], "keywords": json.loads (row ["keywords"]) if row ["keywords"] else [],
            "entities": json.loads (row ["entities"]) if row ["entities"] else [], "summary": row ["summary"],
        })
...
def incremental_index_pipeline (
    live_lake: LiveVectorLake, doc_id: str, new_chunks: list [dict], force_reindex: bool = False, ) -> bool:
  """
...
if __name__ == "__main__":
  ...
  version_store = ChunkVersionStore (hot_dir = Path ("./test_hot"), cold_dir = Path ("./test_cold_version_store"),
      wal_path = Path ("./test_wal/wal.json"))
  
  live_lake = LiveVectorLake (qdrant_indexer = qdrant_idx, version_store = version_store,
      cold_storage_dir = Path ("./test_cold_lake"), use_delta = False, )
  
  # Тестовый документ
  doc_id = "confluence_123"
  chunks_v1 = [
      {
          "hash": "aaa", "text": "Version 1 content", "title": "Doc Title", "source_type": "confluence",
          "source_id": doc_id, "version": "1.0", "doc_title": "Test Doc", "keywords": ["test"], "entities": [],
  ...
  chunks_v2 = [
      {
          "hash": "bbb", "text": "Version 2 updated content", "title": "Doc Title", "source_type": "confluence",
          "source_id": doc_id, "version": "2.0", "doc_title": "Test Doc", "keywords": ["test", "updated"],
          "entities": [], "summary": "Updated summary",
      }
  ]

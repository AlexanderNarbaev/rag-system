...
class ChunkEnricher:
    ...
    def _call_slm(self, chunk_text: str) -> dict[str, Any]:
        ...
        # Sanitize invalid JSON escapes (SLM sometimes returns \d, \s, etc.)
        content = self._sanitize_json_escapes(content)
        ...
        try:
            ...
            if match:
                try:
                    ...
                    # Final attempt: sanitize the matched portion
                    sanitized = self._sanitize_json_escapes(match.group(0))
    ...
    def _sanitize_json_escapes(text: str) -> str:
        """Fix invalid JSON escape sequences from SLM responses.
        Valid JSON escapes: \\\", \\\\, \\/, \\b, \\f, \\n, \\r, \\t, \\uXXXX.
        Everything else is invalid and needs the backslash doubled.
        """
        # Match backslash followed by any char that is NOT a valid JSON escape
        return re.sub(
            r'\\(?!["\\/bfnrtu])',
            r"\\\\",
            text,
